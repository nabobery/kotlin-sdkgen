...
| --- | --- |
| Parser and OpenAPI 3.1 fidelity | Foundation Evaluation benchmark, not a preference decision |
| Litote reuse | Foundation Evaluation benchmark |
| Fabrikt reuse | Foundation Evaluation benchmark |
...
| KotlinPoet emitter | Foundation Evaluation benchmark; KotlinPoet remains the recommended final emitter |
| Missing versus null | Resolved: exact three-state wire presence |
| Mixed primitive/object unions | Partially resolved: closed `oneOf` uses adaptive sealed cases and strict unique matching; Foundation Evaluation selects a lossless representation for multi-match `anyOf` |
...
| Quantitative performance budgets | Set from Foundation Evaluation measurements |
| Artifact split | Architectural split resolved; exact publication granularity validated in Foundation Evaluation |
| Published ABI validation tool | Foundation Evaluation comparison of the experimental KGP DSL and maintenance-mode standalone validator against actual JVM/KMP publications |
