...
Accepted with Generator Alpha conformance gates.

**Updated 2026-07-16:** the RFC 9535 conformance gate
...
replaces SnackJson. The conformance suite and its machine-readable CTS matrix remain the
...
Overlay 1.1 uses RFC 9535 JSONPath and defines `update`, `remove`, and `copy`. The Foundation Evaluation prototype proved `update` and `remove`; it did not implement `copy`. SnackJson provides an RFC-named mode but still accepts some non-standard operators.
...
> SnackJson 4.0.54's explicit IETF mode still accepted the non-RFC Jayway `in` operator, so the prototype rejected known extensions before evaluation and required a full grammar/corpus gate for Generator Alpha.
...
- Retain tests for Overlay 1.1 `copy`, including zero-match, multiple source/target, conflict, and evidence behavior.
- Keep the RFC 9535 corpus pinned and reject non-standard grammar before evaluation.
