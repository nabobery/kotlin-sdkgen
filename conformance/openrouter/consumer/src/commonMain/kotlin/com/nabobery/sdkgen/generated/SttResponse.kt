...
public class SttResponse(
   ...
   */
  public val segments: List<SttSegment>? = null,
  /**
   ...
   */
  public val words: List<SttWord>? = null,
) {
  public class Builder {
     ...
     * Timestamped transcript segments, present when response_format is verbose_json
     */
    public var segments: List<SttSegment>? = null
     ...
     * Timestamped words, present when the provider returns word-level timestamps
     */
    public var words: List<SttWord>? = null
  ...
  public object Serializer : KSerializer<SttResponse> {
    ...
    override fun deserialize(decoder: Decoder): SttResponse {
      ...
      val raw = jsonDecoder.decodeJsonElement() as? JsonObject ?:
        throw SerializationException("SttResponse must be a JSON object")
      val text = json.decodeRequired<String>(raw, "text")
      return SttResponse(
        text = text,
        duration = raw["duration"]?.let { json.decodeFromJsonElement<Double>(it) },
        language = raw["language"]?.let { json.decodeFromJsonElement<String>(it) },
        segments = raw["segments"]?.let { json.decodeFromJsonElement<List<SttSegment>>(it) },
        task = raw["task"]?.let { json.decodeFromJsonElement<String>(it) },
        usage = raw["usage"]?.let { json.decodeFromJsonElement<SttUsage>(it) },
        words = raw["words"]?.let { json.decodeFromJsonElement<List<SttWord>>(it) },
      )
    }
