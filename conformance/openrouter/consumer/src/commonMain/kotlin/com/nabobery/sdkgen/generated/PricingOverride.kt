...
public class PricingOverride(
  ...
  public object Serializer : KSerializer<PricingOverride> {
    ...
    override fun deserialize(decoder: Decoder): PricingOverride {
      ...
      val raw = jsonDecoder.decodeJsonElement() as? JsonObject ?:
        throw SerializationException("PricingOverride must be a JSON object")
      return PricingOverride(
        ...
        minPromptTokens = raw["min_prompt_tokens"]?.let { json.decodeFromJsonElement<Double>(it) },
        prompt = raw["prompt"]?.let { json.decodeFromJsonElement<String>(it) },
        utcEnd = raw["utc_end"]?.let { json.decodeFromJsonElement<Double>(it) },
        utcStart = raw["utc_start"]?.let { json.decodeFromJsonElement<Double>(it) },
      )
    }
