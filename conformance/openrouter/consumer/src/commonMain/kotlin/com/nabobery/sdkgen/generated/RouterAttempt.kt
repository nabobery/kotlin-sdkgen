...
public class RouterAttempt(
  ...
  public object Serializer : KSerializer<RouterAttempt> {
    ...
    override fun deserialize(decoder: Decoder): RouterAttempt {
      ...
      val raw = jsonDecoder.decodeJsonElement() as? JsonObject ?:
        throw SerializationException("RouterAttempt must be a JSON object")
      val model = json.decodeRequired<String>(raw, "model")
      val provider = json.decodeRequired<String>(raw, "provider")
      val status = json.decodeRequired<Int>(raw, "status")
      return RouterAttempt(
        model = model,
        provider = provider,
        status = status,
