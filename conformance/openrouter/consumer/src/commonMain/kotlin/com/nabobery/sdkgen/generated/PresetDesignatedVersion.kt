...
public class PresetDesignatedVersion(
  ...
  public object Serializer : KSerializer<PresetDesignatedVersion> {
    ...
    override fun deserialize(decoder: Decoder): PresetDesignatedVersion {
      ...
      val raw = jsonDecoder.decodeJsonElement() as? JsonObject ?:
        throw SerializationException("PresetDesignatedVersion must be a JSON object")
      val config = json.decodeRequired<Map<String, JsonElement?>>(raw, "config")
      val createdAt = json.decodeRequired<String>(raw, "created_at")
      val creatorId = json.decodeRequired<String>(raw, "creator_id")
      val id = json.decodeRequired<String>(raw, "id")
      val presetId = json.decodeRequired<String>(raw, "preset_id")
      val updatedAt = json.decodeRequired<String>(raw, "updated_at")
      val version = json.decodeRequired<Int>(raw, "version")
      if (!raw.containsKey("system_prompt")) {
        throw SerializationException("PresetDesignatedVersion is missing required property 'system_prompt'")
      }
      val systemPrompt = raw["system_prompt"].let { element -> if (element == JsonNull) null else json
...
public fun presetDesignatedVersion(block: PresetDesignatedVersion.Builder.() -> Unit): PresetDesignatedVersion =
  PresetDesignatedVersion.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("PresetDesignatedVersion is missing required property '" +
    name + "'")
