...
public class ChatStreamChoice internal constructor(
  ...
  public object Serializer : KSerializer<ChatStreamChoice> {
    ...
    override fun deserialize(decoder: Decoder): ChatStreamChoice {
      ...
      val raw = jsonDecoder.decodeJsonElement() as? JsonObject ?:
        throw SerializationException("ChatStreamChoice must be a JSON object")
      val delta = json.decodeRequired<ChatStreamDelta>(raw, "delta")
      ...
      val finishReason = raw["finish_reason"].let { element -> if (element == JsonNull) null else json
        .decodeFromJsonElement<ChatFinishReasonEnum?>(requireNotNull(element)) }
      return ChatStreamChoice(
        delta = delta,
        finishReason = finishReason,
        index = index,
        logprobsState = json.decodeOptional(raw, "logprobs", nullable = true),
      )
    }
...
public fun chatStreamChoice(block: ChatStreamChoice.Builder.() -> Unit): ChatStreamChoice = ChatStreamChoice
  .build(block)
