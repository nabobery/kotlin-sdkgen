...
public class ModelReasoning(
   ...
   */
  public val supportedEfforts: List<ReasoningEffort?>? = null,
  /**
  ...
  public class Builder {
     ...
     */
    public var supportedEfforts: List<ReasoningEffort?>? = null
  ...
  public object Serializer : KSerializer<ModelReasoning> {
    ...
    override fun deserialize(decoder: Decoder): ModelReasoning {
      ...
      val raw = jsonDecoder.decodeJsonElement() as? JsonObject ?:
        throw SerializationException("ModelReasoning must be a JSON object")
      val mandatory = json.decodeRequired<Boolean>(raw, "mandatory")
      return ModelReasoning(
        mandatory = mandatory,
        defaultEffort = raw["default_effort"]?.let { element -> if (element == JsonNull) null else json
          .decodeFromJsonElement<ReasoningEffort?>(element) },
        defaultEnabled = raw["default_enabled"]?.let { json.decodeFromJsonElement<Boolean>(it) },
        supportedEfforts = raw["supported_efforts"]?.let { element -> if (element == JsonNull) null else json
          .decodeFromJsonElement<List<ReasoningEffort?>?>(element) },
        supportsMaxTokens = raw["supports_max_tokens"]?.let { json.decodeFromJsonElement<Boolean>(it) },
      )
    }
