...
public class AnthropicPlainTextSource(
  ...
  public class Builder {
    ...
    private var mediaTypeValue: InlineComponentsSchemasAnthropicPlainTextSourcePropertiesMediaType?
        = null

    public var mediaType: InlineComponentsSchemasAnthropicPlainTextSourcePropertiesMediaType
      get() = requireNotNull(mediaTypeValue) { "mediaType is required" }
      set(`value`) {
        mediaTypeValue = value
      }

    private var typeValue: InlineComponentsSchemasAnthropicPlainTextSourcePropertiesType? = null

    public var type: InlineComponentsSchemasAnthropicPlainTextSourcePropertiesType
  ...
  public object Serializer : KSerializer<AnthropicPlainTextSource> {
    ...
    override fun deserialize(decoder: Decoder): AnthropicPlainTextSource {
      ...
      val raw = jsonDecoder.decodeJsonElement() as? JsonObject ?:
        throw SerializationException("AnthropicPlainTextSource must be a JSON object")
      val data = json.decodeRequired<String>(raw, "data")
      val mediaType = json.decodeRequired<InlineComponentsSchemasAnthropicPlainTextSourcePropertiesMediaType>(raw,
        "media_type")
      val type = json.decodeRequired<InlineComponentsSchemasAnthropicPlainTextSourcePropertiesType>(raw, "type")
      return AnthropicPlainTextSource(
        data = data,
        mediaType = mediaType,
        type = type,
...
public fun anthropicPlainTextSource(block: AnthropicPlainTextSource.Builder.() -> Unit): AnthropicPlainTextSource =
  AnthropicPlainTextSource.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("AnthropicPlainTextSource is missing required property '" +
    name + "'")
