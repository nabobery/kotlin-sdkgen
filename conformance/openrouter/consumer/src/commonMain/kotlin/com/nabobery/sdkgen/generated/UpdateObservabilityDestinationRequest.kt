...
public class UpdateObservabilityDestinationRequest internal constructor(
  ...
  public object Serializer : KSerializer<UpdateObservabilityDestinationRequest> {
    ...
    override fun deserialize(decoder: Decoder): UpdateObservabilityDestinationRequest {
      ...
      val raw = jsonDecoder.decodeJsonElement() as? JsonObject ?:
        throw SerializationException("UpdateObservabilityDestinationRequest must be a JSON object")
      return UpdateObservabilityDestinationRequest(
        apiKeyHashesState = json.decodeOptional(raw, "api_key_hashes", nullable = true),
        configState = json.decodeOptional(raw, "config", nullable = false),
        enabledState = json.decodeOptional(raw, "enabled", nullable = false),
        filterRulesState = json.decodeOptional(raw, "filter_rules", nullable = true),
        nameState = json.decodeOptional(raw, "name", nullable = false),
        privacyModeState = json.decodeOptional(raw, "privacy_mode", nullable = false),
        samplingRateState = json.decodeOptional(raw, "sampling_rate", nullable = false),
      )
    }
...
public fun updateObservabilityDestinationRequest(block: UpdateObservabilityDestinationRequest.Builder.() ->
  Unit): UpdateObservabilityDestinationRequest = UpdateObservabilityDestinationRequest.build(block)
...
private inline fun <reified T> Json.decodeOptional(
  ...
  if (element == JsonNull) {
    if (!nullable) throw SerializationException("UpdateObservabilityDestinationRequest property '" + name +
      "' is not nullable")
