...
public class OutputBrowserUseServerToolItem(
  ...
  public val type: InlineComponentsSchemasOutputBrowserUseServerToolItemPropertiesType,
  ...
  public class Builder {
    ...
    private var typeValue: InlineComponentsSchemasOutputBrowserUseServerToolItemPropertiesType? =
        null

    public var type: InlineComponentsSchemasOutputBrowserUseServerToolItemPropertiesType
  ...
  public object Serializer : KSerializer<OutputBrowserUseServerToolItem> {
    ...
    override fun deserialize(decoder: Decoder): OutputBrowserUseServerToolItem {
      ...
      val raw = jsonDecoder.decodeJsonElement() as? JsonObject ?:
        throw SerializationException("OutputBrowserUseServerToolItem must be a JSON object")
      val status = json.decodeRequired<ToolCallStatus>(raw, "status")
      val type = json.decodeRequired<InlineComponentsSchemasOutputBrowserUseServerToolItemPropertiesType>(raw, "type")
      return OutputBrowserUseServerToolItem(
        status = status,
        type = type,
        action = raw["action"]?.let { json.decodeFromJsonElement<String>(it) },
        id = raw["id"]?.let { json.decodeFromJsonElement<String>(it) },
        screenshotB64 = raw["screenshotB64"]?.let { json.decodeFromJsonElement<String>(it) },
      )
    }
...
public fun outputBrowserUseServerToolItem(block: OutputBrowserUseServerToolItem.Builder.() ->
  Unit): OutputBrowserUseServerToolItem = OutputBrowserUseServerToolItem.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?:
    throw SerializationException("OutputBrowserUseServerToolItem is missing required property '" + name + "'")
