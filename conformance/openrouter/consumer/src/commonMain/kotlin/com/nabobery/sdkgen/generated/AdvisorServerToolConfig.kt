...
public class AdvisorServerToolConfig(
   ...
   */
  public val tools: List<AdvisorNestedTool>? = null,
) {
  public class Builder {
     ...
     * list must not include the advisor tool itself.
     */
    public var tools: List<AdvisorNestedTool>? = null
  ...
  public object Serializer : KSerializer<AdvisorServerToolConfig> {
    ...
    override fun deserialize(decoder: Decoder): AdvisorServerToolConfig {
      ...
      val raw = jsonDecoder.decodeJsonElement() as? JsonObject ?:
        throw SerializationException("AdvisorServerToolConfig must be a JSON object")
      return AdvisorServerToolConfig(
        forwardTranscript = raw["forward_transcript"]?.let { json.decodeFromJsonElement<Boolean>(it) },
        ...
        reasoning = raw["reasoning"]?.let { json.decodeFromJsonElement<AdvisorReasoning>(it) },
        ...
        tools = raw["tools"]?.let { json.decodeFromJsonElement<List<AdvisorNestedTool>>(it) },
      )
    }
...
public fun advisorServerToolConfig(block: AdvisorServerToolConfig.Builder.() -> Unit): AdvisorServerToolConfig =
  AdvisorServerToolConfig.build(block)
