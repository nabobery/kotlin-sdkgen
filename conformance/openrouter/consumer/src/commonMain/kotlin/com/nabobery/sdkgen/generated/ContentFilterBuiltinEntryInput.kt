...
public class ContentFilterBuiltinEntryInput internal constructor(
  ...
  public object Serializer : KSerializer<ContentFilterBuiltinEntryInput> {
    ...
    override fun deserialize(decoder: Decoder): ContentFilterBuiltinEntryInput {
      ...
      val raw = jsonDecoder.decodeJsonElement() as? JsonObject ?:
        throw SerializationException("ContentFilterBuiltinEntryInput must be a JSON object")
      val action = json.decodeRequired<ContentFilterBuiltinAction>(raw, "action")
      val slug = json.decodeRequired<ContentFilterBuiltinSlug>(raw, "slug")
      return ContentFilterBuiltinEntryInput(
        action = action,
        slug = slug,
        labelState = json.decodeOptional(raw, "label", nullable = false),
        scanScopeState = json.decodeOptional(raw, "scan_scope", nullable = false),
      )
    }
...
public fun contentFilterBuiltinEntryInput(block: ContentFilterBuiltinEntryInput.Builder.() ->
  Unit): ContentFilterBuiltinEntryInput = ContentFilterBuiltinEntryInput.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?:
    throw SerializationException("ContentFilterBuiltinEntryInput is missing required property '" + name + "'")
...
private inline fun <reified T> Json.decodeOptional(
  ...
  if (element == JsonNull) {
    if (!nullable) throw SerializationException("ContentFilterBuiltinEntryInput property '" + name +
      "' is not nullable")
