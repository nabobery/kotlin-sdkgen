...
public class AnthropicWebFetchToolResultError(
  ...
  public class Builder {
    private var errorCodeValue:
        InlineComponentsSchemasAnthropicWebFetchToolResultErrorPropertiesErrorCode? = null

    public var errorCode: InlineComponentsSchemasAnthropicWebFetchToolResultErrorPropertiesErrorCode
      get() = requireNotNull(errorCodeValue) { "errorCode is required" }
      set(`value`) {
        errorCodeValue = value
      }

    private var typeValue: InlineComponentsSchemasAnthropicWebFetchToolResultErrorPropertiesType? =
        null

    public var type: InlineComponentsSchemasAnthropicWebFetchToolResultErrorPropertiesType
  ...
  public object Serializer : KSerializer<AnthropicWebFetchToolResultError> {
    ...
    override fun deserialize(decoder: Decoder): AnthropicWebFetchToolResultError {
      ...
      val raw = jsonDecoder.decodeJsonElement() as? JsonObject ?:
        throw SerializationException("AnthropicWebFetchToolResultError must be a JSON object")
      val errorCode = json
        .decodeRequired<InlineComponentsSchemasAnthropicWebFetchToolResultErrorPropertiesErrorCode>(raw, "error_code")
      val type = json.decodeRequired<InlineComponentsSchemasAnthropicWebFetchToolResultErrorPropertiesType>(raw, "type")
      return AnthropicWebFetchToolResultError(
        errorCode = errorCode,
        type = type,
      )
...
public fun anthropicWebFetchToolResultError(block: AnthropicWebFetchToolResultError.Builder.() ->
  Unit): AnthropicWebFetchToolResultError = AnthropicWebFetchToolResultError.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?:
    throw SerializationException("AnthropicWebFetchToolResultError is missing required property '" + name + "'")
