...
public class AnthropicUsage(
  ...
  public object Serializer : KSerializer<AnthropicUsage> {
    ...
    override fun deserialize(decoder: Decoder): AnthropicUsage {
      ...
      val raw = jsonDecoder.decodeJsonElement() as? JsonObject ?:
        throw SerializationException("AnthropicUsage must be a JSON object")
      val inputTokens = json.decodeRequired<Int>(raw, "input_tokens")
      val outputTokens = json.decodeRequired<Int>(raw, "output_tokens")
      if (!raw.containsKey("cache_creation")) {
        throw SerializationException("AnthropicUsage is missing required property 'cache_creation'")
      }
      val cacheCreation = raw["cache_creation"].let { element -> if (element == JsonNull) null else json
        .decodeFromJsonElement<AnthropicCacheCreation?>(requireNotNull(element)) }
      ...
      val cacheReadInputTokens = raw["cache_read_input_tokens"].let { element ->
        if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(requireNotNull(element)) }
      if (!raw.containsKey("inference_geo")) {
        throw SerializationException("AnthropicUsage is missing required property 'inference_geo'")
      }
      val inferenceGeo = raw["inference_geo"].let { element -> if (element == JsonNull) null else json
      ...
      val serverToolUse = raw["server_tool_use"].let { element -> if (element == JsonNull) null else json
        .decodeFromJsonElement<AnthropicServerToolUsage?>(requireNotNull(element)) }
      ...
      val serviceTier = raw["service_tier"].let { element -> if (element == JsonNull) null else json
        .decodeFromJsonElement<AnthropicServiceTier?>(requireNotNull(element)) }
      return AnthropicUsage(
