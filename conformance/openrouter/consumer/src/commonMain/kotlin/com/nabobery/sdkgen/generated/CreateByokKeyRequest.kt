...
public class CreateByokKeyRequest(
  ...
  public class Builder {
     ...
     * Optional allowlist of user IDs that may use this credential. `null` means no restriction.
     */
    public var allowedUserIds: List<String>? = null
  ...
  public object Serializer : KSerializer<CreateByokKeyRequest> {
    ...
    override fun deserialize(decoder: Decoder): CreateByokKeyRequest {
      ...
      val raw = jsonDecoder.decodeJsonElement() as? JsonObject ?:
        throw SerializationException("CreateByokKeyRequest must be a JSON object")
      ...
      return CreateByokKeyRequest(
        ...
        allowedUserIds = raw["allowed_user_ids"]?.let { element -> if (element == JsonNull) null else json
          .decodeFromJsonElement<List<String>?>(element) },
        disabled = raw["disabled"]?.let { json.decodeFromJsonElement<Boolean>(it) },
        isFallback = raw["is_fallback"]?.let { json.decodeFromJsonElement<Boolean>(it) },
        name = raw["name"]?.let { element -> if (element == JsonNull) null else json
...
public fun createByokKeyRequest(block: CreateByokKeyRequest.Builder.() -> Unit): CreateByokKeyRequest =
  CreateByokKeyRequest.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("CreateByokKeyRequest is missing required property '" +
    name + "'")
