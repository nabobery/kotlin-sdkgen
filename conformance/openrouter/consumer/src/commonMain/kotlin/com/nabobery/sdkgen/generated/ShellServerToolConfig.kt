...
public class ShellServerToolConfig(
  ...
  public object Serializer : KSerializer<ShellServerToolConfig> {
    ...
    override fun deserialize(decoder: Decoder): ShellServerToolConfig {
      ...
      val raw = jsonDecoder.decodeJsonElement() as? JsonObject ?:
        throw SerializationException("ShellServerToolConfig must be a JSON object")
      return ShellServerToolConfig(
        engine = raw["engine"]?.let { json.decodeFromJsonElement<ShellServerToolEngine>(it) },
        environment = raw["environment"]?.let { json.decodeFromJsonElement<ShellServerToolEnvironment>(it) },
        sleepAfterSeconds = raw["sleep_after_seconds"]?.let { json.decodeFromJsonElement<Int>(it) },
      )
    }
...
public fun shellServerToolConfig(block: ShellServerToolConfig.Builder.() -> Unit): ShellServerToolConfig =
  ShellServerToolConfig.build(block)
