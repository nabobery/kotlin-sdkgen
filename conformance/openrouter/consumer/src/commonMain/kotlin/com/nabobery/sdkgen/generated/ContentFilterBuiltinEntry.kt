...
public class ContentFilterBuiltinEntry(
  ...
  public object Serializer : KSerializer<ContentFilterBuiltinEntry> {
    ...
    override fun deserialize(decoder: Decoder): ContentFilterBuiltinEntry {
      ...
      val raw = jsonDecoder.decodeJsonElement() as? JsonObject ?:
        throw SerializationException("ContentFilterBuiltinEntry must be a JSON object")
      ...
      return ContentFilterBuiltinEntry(
        action = action,
        slug = slug,
        label = raw["label"]?.let { json.decodeFromJsonElement<String>(it) },
        scanScope = raw["scan_scope"]?.let { json.decodeFromJsonElement<PromptInjectionScanScope>(it) },
      )
    }
...
public fun contentFilterBuiltinEntry(block: ContentFilterBuiltinEntry.Builder.() -> Unit): ContentFilterBuiltinEntry =
  ContentFilterBuiltinEntry.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("ContentFilterBuiltinEntry is missing required property '" +
    name + "'")
