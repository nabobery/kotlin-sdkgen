...
public class Model internal constructor(
  ...
  public class Builder {
    ...
    public var supportedVoices: List<String>?
      get() = supportedVoicesState.valueOrNull()
      set(`value`) {
        supportedVoicesState = value.toNullableFieldState()
      }
  ...
  public object Serializer : KSerializer<Model> {
    ...
    override fun deserialize(decoder: Decoder): Model {
      ...
      val raw = jsonDecoder.decodeJsonElement() as? JsonObject ?:
        throw SerializationException("Model must be a JSON object")
      val architecture = json.decodeRequired<ModelArchitecture>(raw, "architecture")
      ...
      val links = json.decodeRequired<ModelLinks>(raw, "links")
      val name = json.decodeRequired<String>(raw, "name")
      val pricing = json.decodeRequired<PublicPricing>(raw, "pricing")
      val supportedParameters = json.decodeRequired<List<Parameter>>(raw, "supported_parameters")
      val topProvider = json.decodeRequired<TopProviderInfo>(raw, "top_provider")
      if (!raw.containsKey("context_length")) {
        throw SerializationException("Model is missing required property 'context_length'")
      }
      val contextLength = raw["context_length"].let { element -> if (element == JsonNull) null else json
        .decodeFromJsonElement<Int?>(requireNotNull(element)) }
      if (!raw.containsKey("default_parameters")) {
        throw SerializationException("Model is missing required property 'default_parameters'")
      }
      val defaultParameters = raw["default_parameters"].let { element -> if (element == JsonNull) null else json
        .decodeFromJsonElement<DefaultParameters?>(requireNotNull(element)) }
      if (!raw.containsKey("per_request_limits")) {
        throw SerializationException("Model is missing required property 'per_request_limits'")
      }
      val perRequestLimits = raw["per_request_limits"].let { element -> if (element == JsonNull) null else json
        .decodeFromJsonElement<PerRequestLimits?>(requireNotNull(element)) }
      if (!raw.containsKey("supported_voices")) {
        throw SerializationException("Model is missing required property 'supported_voices'")
      }
      val supportedVoices = raw["supported_voices"].let { element -> if (element == JsonNull) null else json
      ...
      return Model(
        ...
        supportedVoices = supportedVoices,
        topProvider = topProvider,
        benchmarksState = json.decodeOptional(raw, "benchmarks", nullable = false),
        descriptionState = json.decodeOptional(raw, "description", nullable = false),
        expirationDateState = json.decodeOptional(raw, "expiration_date", nullable = true),
        huggingFaceIdState = json.decodeOptional(raw, "hugging_face_id", nullable = true),
        knowledgeCutoffState = json.decodeOptional(raw, "knowledge_cutoff", nullable = true),
        reasoningState = json.decodeOptional(raw, "reasoning", nullable = false),
      )
    }
