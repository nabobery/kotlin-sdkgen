 ...
 *
 * Source: sdkgen://source/openapi.yaml#/components/schemas/FileParserPlugin
 */
...
public class FileParserPlugin(
  ...
  public class Builder {
    private var idValue: InlineFileParserPluginIdX43ae2ffb? = null

    public var id: InlineFileParserPluginIdX43ae2ffb
  ...
  internal object Serializer : KSerializer<FileParserPlugin> {
    ...
    override fun deserialize(decoder: Decoder): FileParserPlugin {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("FileParserPlugin must be a JSON object")
      val id = json.decodeRequired<InlineFileParserPluginIdX43ae2ffb>(rawObject, "id")
      return FileParserPlugin(
        id = id,
        enabled = rawObject["enabled"]?.let { json.decodeFromJsonElement<Boolean>(it) },
        pdf = rawObject["pdf"]?.let { json.decodeFromJsonElement<PdfParserOptions>(it) },
      )
    }
...
public fun fileParserPlugin(block: FileParserPlugin.Builder.() -> Unit): FileParserPlugin = FileParserPlugin.build(block)
