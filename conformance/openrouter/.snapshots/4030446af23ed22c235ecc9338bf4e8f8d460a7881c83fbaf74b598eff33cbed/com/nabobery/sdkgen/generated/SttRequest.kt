 ...
 *
 * Source: sdkgen://source/openapi.yaml#/components/schemas/STTRequest
 */
...
public class SttRequest(
   ...
   */
  public val provider: InlineSttRequestProviderXce1a370d? = null,
  /**
   ...
   */
  public val responseFormat: InlineSttRequestResponseFormatX293f617e? = null,
  /**
  ...
  timestampGranularities: List<SttTimestampGranularity>? = null,
) {
  /**
   ...
   * "verbose_json".
   */
  public val timestampGranularities: List<SttTimestampGranularity>? =
  ...
  public class Builder {
     ...
     * Provider-specific passthrough configuration
     */
    public var provider: InlineSttRequestProviderXce1a370d? = null

    /**
     * Output format. "json" (default) returns { text, usage }. "verbose_json" additionally returns task, language,
     * duration, and segment-level timestamps; only supported by OpenAI-compatible providers.
     */
    public var responseFormat: InlineSttRequestResponseFormatX293f617e? = null
    ...
    private var timestampGranularitiesValue: List<SttTimestampGranularity>? = null
     ...
     * is "verbose_json".
     */
    public var timestampGranularities: List<SttTimestampGranularity>?
  ...
  internal object Serializer : KSerializer<SttRequest> {
    ...
    override fun deserialize(decoder: Decoder): SttRequest {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("SttRequest must be a JSON object")
      val inputAudio = json.decodeRequired<SttInputAudio>(rawObject, "input_audio")
      val model = json.decodeRequired<String>(rawObject, "model")
      return SttRequest(
        inputAudio = inputAudio,
        model = model,
        language = rawObject["language"]?.let { json.decodeFromJsonElement<String>(it) },
        provider = rawObject["provider"]?.let { json.decodeFromJsonElement<InlineSttRequestProviderXce1a370d>(it) },
        responseFormat = rawObject["response_format"]?.let { json.decodeFromJsonElement<InlineSttRequestResponseFormatX293f617e>(it) },
        temperature = rawObject["temperature"]?.let { json.decodeFromJsonElement<Double>(it) },
        timestampGranularities = rawObject["timestamp_granularities"]?.let { json.decodeFromJsonElement<List<SttTimestampGranularity>>(it) },
      )
    }
