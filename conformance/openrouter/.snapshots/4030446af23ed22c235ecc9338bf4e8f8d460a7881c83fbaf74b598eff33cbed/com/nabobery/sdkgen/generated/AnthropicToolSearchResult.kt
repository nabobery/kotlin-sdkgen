 ...
 *
 * Source: sdkgen://source/openapi.yaml#/components/schemas/AnthropicToolSearchResult
 */
...
public class AnthropicToolSearchResult(
  ...
  public class Builder {
    ...
    public var toolReferences: List<AnthropicToolReference>
      get() = requireNotNull(toolReferencesValue) { "toolReferences is required" }.toList()
      set(`value`) {
        toolReferencesValue = value.toList()
      }

    private var typeValue: InlineAnthropicToolSearchResultTypeX9de5669c? = null

    public var type: InlineAnthropicToolSearchResultTypeX9de5669c
  ...
  internal object Serializer : KSerializer<AnthropicToolSearchResult> {
    ...
    override fun deserialize(decoder: Decoder): AnthropicToolSearchResult {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("AnthropicToolSearchResult must be a JSON object")
      val toolReferences = json.decodeRequired<List<AnthropicToolReference>>(rawObject, "tool_references")
      val type = json.decodeRequired<InlineAnthropicToolSearchResultTypeX9de5669c>(rawObject, "type")
      return AnthropicToolSearchResult(
        toolReferences = toolReferences,
        type = type,
      )
...
public fun anthropicToolSearchResult(block: AnthropicToolSearchResult.Builder.() -> Unit): AnthropicToolSearchResult = AnthropicToolSearchResult.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("AnthropicToolSearchResult is missing required property '" + name + "'")
