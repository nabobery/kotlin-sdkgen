 ...
 *
 * Source: sdkgen://source/openapi.yaml#/components/schemas/OutputFilesServerToolItem
 */
...
public class OutputFilesServerToolItem(
  ...
  public class Builder {
    ...
    private var typeValue: InlineOutputFilesServerToolItemTypeX5fc5dc4b? = null

    public var type: InlineOutputFilesServerToolItemTypeX5fc5dc4b
  ...
  internal object Serializer : KSerializer<OutputFilesServerToolItem> {
    ...
    override fun deserialize(decoder: Decoder): OutputFilesServerToolItem {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("OutputFilesServerToolItem must be a JSON object")
      val status = json.decodeRequired<ToolCallStatus>(rawObject, "status")
      val type = json.decodeRequired<InlineOutputFilesServerToolItemTypeX5fc5dc4b>(rawObject, "type")
      return OutputFilesServerToolItem(
        status = status,
        type = type,
        ...
        operation = rawObject["operation"]?.let { json.decodeFromJsonElement<String>(it) },
        result = rawObject["result"]?.let { json.decodeFromJsonElement<String>(it) },
      )
    }
...
public fun outputFilesServerToolItem(block: OutputFilesServerToolItem.Builder.() -> Unit): OutputFilesServerToolItem = OutputFilesServerToolItem.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("OutputFilesServerToolItem is missing required property '" + name + "'")
