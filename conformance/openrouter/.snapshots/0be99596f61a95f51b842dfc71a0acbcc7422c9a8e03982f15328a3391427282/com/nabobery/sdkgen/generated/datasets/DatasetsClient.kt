...
import com.nabobery.sdkgen.generated.AppRankingsItem
...
public class DatasetsClient(
  ...
  ): Page<AppRankingsResponse, AppRankingsItem> {
    val engine = PaginationEngine<AppRankingsResponse, AppRankingsItem>(
          descriptor = requireNotNull(getAppRankingsMetadata.pagination as? PaginationDescriptor.OffsetLimit),
          operationId = getAppRankingsMetadata.operationId,
          requestedPageSize = limit,
          initialOffset = offset?.toLong() ?: 0L,
        )
    return engine.firstPage { pageRequest -> fetchgetAppRankingsPage(Unit, category, endDate, limit, offset, sort, startDate, subcategory, httpReferer, xOpenRouterCategories, xOpenRouterTitle, pageRequest, options) }
  }
  ...
  /**
   * Returns a cold page flow for getAppRankings.
   *
   * @param category Marketplace category group to filter by (e.g. `coding`). Only apps tagged with a subcategory inside
   * this group are returned. Mutually combinable with `subcategory` — when both are supplied the `subcategory` must
   * belong to the `category` group.
   ...
   * @param options Execution options, including pagination bounds.
   */
  public fun getAppRankingsPages(
    category: InlineDatasetsAppRankingsGetParameterX9098751d? = null,
    endDate: String? = null,
    limit: Int? = null,
    offset: Int? = null,
    ...
    xOpenRouterTitle: String? = null,
    options: CallOptions = CallOptions(),
  ): Flow<Page<AppRankingsResponse, AppRankingsItem>> = PaginationEngine<AppRankingsResponse, AppRankingsItem>(
    descriptor = requireNotNull(getAppRankingsMetadata.pagination as? PaginationDescriptor.OffsetLimit),
    operationId = getAppRankingsMetadata.operationId,
    requestedPageSize = limit,
    initialOffset = offset?.toLong() ?: 0L,
  ).pages(fetch = { pageRequest -> fetchgetAppRankingsPage(Unit, category, endDate, limit, offset, sort, startDate, subcategory, httpReferer, xOpenRouterCategories, xOpenRouterTitle, pageRequest, options) }, pagination = options.pagination)

  /**
   * Returns a cold item flow for getAppRankings.
   *
   * @param options Execution options, including pagination bounds.
   */
  public fun getAppRankingsItems(
    category: InlineDatasetsAppRankingsGetParameterX9098751d? = null,
    endDate: String? = null,
    limit: Int? = null,
    offset: Int? = null,
    ...
    xOpenRouterTitle: String? = null,
    options: CallOptions = CallOptions(),
  ): Flow<AppRankingsItem> = PaginationEngine<AppRankingsResponse, AppRankingsItem>(
    descriptor = requireNotNull(getAppRankingsMetadata.pagination as? PaginationDescriptor.OffsetLimit),
    operationId = getAppRankingsMetadata.operationId,
    requestedPageSize = limit,
    initialOffset = offset?.toLong() ?: 0L,
  ).items(fetch = { pageRequest -> fetchgetAppRankingsPage(Unit, category, endDate, limit, offset, sort, startDate, subcategory, httpReferer, xOpenRouterCategories, xOpenRouterTitle, pageRequest, options) }, pagination = options.pagination)

  private fun metadataForGetAppRankingsPage(
    ...
    xOpenRouterCategories: String? = null,
    xOpenRouterTitle: String? = null,
  ): OperationMetadata = getAppRankingsMetadata

  private suspend fun fetchgetAppRankingsPage(
    request: Unit,
    category: InlineDatasetsAppRankingsGetParameterX9098751d? = null,
    endDate: String? = null,
    limit: Int? = null,
    ...
    pageRequest: PageRequest,
    options: CallOptions,
  ): PageEnvelope<AppRankingsResponse, AppRankingsItem> {
    ...
    val pageMetadata = metadataForGetAppRankingsPage(pageRequest, category, endDate, limit, offset, sort, startDate, subcategory, httpReferer, xOpenRouterCategories, xOpenRouterTitle)
    val response = executor.execute<Unit, AppRankingsResponse>(SdkExecutionRequest(pageMetadata, baseUri, pageRequestValue, emptyList(), buildList {
      add(SdkRequestParameter(location = SdkParameterLocation.QUERY, name = "category", values = category?.let { listOf(it.toString()) }.orEmpty()))
      add(SdkRequestParameter(location = SdkParameterLocation.QUERY, name = "end_date", values = endDate?.let { listOf(it.toString()) }.orEmpty()))
      add(SdkRequestParameter(location = SdkParameterLocation.QUERY, name = "limit", values = limit?.let { listOf(it.toString()) }.orEmpty()))
      ...
      add(SdkRequestParameter(location = SdkParameterLocation.HEADER, name = "X-OpenRouter-Categories", values = xOpenRouterCategories?.let { listOf(it.toString()) }.orEmpty()))
      add(SdkRequestParameter(location = SdkParameterLocation.HEADER, name = "X-OpenRouter-Title", values = xOpenRouterTitle?.let { listOf(it.toString()) }.orEmpty()))
    }), listOf(DatasetsCodecs.GETAPPRANKINGS_RESPONSE_CODEC_ID), DatasetsCodecs.getAppRankingsRequestCodecRegistry, DatasetsCodecs.getAppRankingsResponseCodecRegistry, options)
