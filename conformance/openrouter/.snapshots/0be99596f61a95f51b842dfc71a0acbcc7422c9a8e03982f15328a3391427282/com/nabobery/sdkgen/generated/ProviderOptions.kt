...
/**
 * Provider-specific options keyed by provider slug. Only options for the matched provider are forwarded; the rest are
 * ignored. Unrecognized keys are silently dropped.
 *
 * Source: sdkgen://source/openapi.yaml#/components/schemas/ProviderOptions
 */
@Serializable(with = ProviderOptions.Serializer::class)
public class ProviderOptions(
  ...
  public class Builder {
    ...
    public fun build(): ProviderOptions = ProviderOptions(
      _01ai = _01ai,
      ai21 = ai21,
      aionLabs = aionLabs,
      akashml = akashml,
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): ProviderOptions = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<ProviderOptions> {
    override val descriptor: SerialDescriptor = JsonElement.serializer().descriptor

    override fun deserialize(decoder: Decoder): ProviderOptions {
      val jsonDecoder = decoder.requireJsonDecoder("ProviderOptions")
      val json = jsonDecoder.json
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("ProviderOptions must be a JSON object")
    ...
    override fun serialize(encoder: Encoder, `value`: ProviderOptions) {
      val jsonEncoder = encoder.requireJsonEncoder("ProviderOptions")
...
public fun providerOptions(block: ProviderOptions.Builder.() -> Unit): ProviderOptions = ProviderOptions.build(block)
