package io.github.nabobery.sdkgen.generated.embeddings
...
import io.github.nabobery.sdkgen.generated.InlineEmbeddingsPostRequestJsonX45746ee0
import io.github.nabobery.sdkgen.generated.InlineEmbeddingsPostResponse200JsonX0f6ccfc4
import io.github.nabobery.sdkgen.generated.InternalServerResponse
import io.github.nabobery.sdkgen.generated.ModelsListResponse
...
public class EmbeddingsClient(
   ...
   * @param options Execution options.
   * @return Buffered response body.
   * @throws ListEmbeddingsModelsApiException When the service returns a declared non-success response; its `error`
   * property exposes the decoded ListEmbeddingsModelsError payload.
   * @throws SdkSerializationException When a request or response cannot be serialized.
   * @throws SdkTransportException When transport execution fails.
   */
  public suspend fun listEmbeddingsModels(
    ...
    xOpenRouterTitle: String? = null,
    options: CallOptions = CallOptions(),
  ): ModelsListResponse = executor.executeWithTypedErrors<Unit, ListEmbeddingsModelsResponse, ModelsListResponse>(
    request = SdkExecutionRequest(listEmbeddingsModelsMetadata, baseUri, Unit, emptyList(), buildList {
    ...
    requestCodecs = EmbeddingsCodecs.listEmbeddingsModelsRequestCodecRegistry,
    responseDecoder = ListEmbeddingsModelsResponseDecoder,
    mapSuccess = { response ->
      when (response) {
        is ListEmbeddingsModelsResponse.SuccessJson -> response.json
        is ListEmbeddingsModelsResponse.Http400Json -> error("Runtime selected a non-success response for success mapping.")
        is ListEmbeddingsModelsResponse.Http500Json -> error("Runtime selected a non-success response for success mapping.")
        is ListEmbeddingsModelsResponse.Unknown -> error("Runtime returned an unmatched response through the typed success path.")
      }
    },
    mapError = { response, statusCode, headers ->
      when (response) {
        is ListEmbeddingsModelsResponse.SuccessJson -> error("Runtime selected a success response for error mapping.")
        is ListEmbeddingsModelsResponse.Http400Json -> ListEmbeddingsModelsApiException(response, statusCode, headers)
        is ListEmbeddingsModelsResponse.Http500Json -> ListEmbeddingsModelsApiException(response, statusCode, headers)
        is ListEmbeddingsModelsResponse.Unknown -> error("Runtime returned an unmatched response through the typed error path.")
      }
    },
    options = options,
  )
