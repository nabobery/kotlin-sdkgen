...
/**
 * Generated model for sdkgen://source/openapi.yaml#/components/schemas/Guardrail.
 *
 * Source: sdkgen://source/openapi.yaml#/components/schemas/Guardrail
 */
@Serializable(with = Guardrail.Serializer::class)
public class Guardrail(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): Guardrail = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<Guardrail> {
    override val descriptor: SerialDescriptor = JsonElement.serializer().descriptor

    override fun deserialize(decoder: Decoder): Guardrail {
      val jsonDecoder = decoder.requireJsonDecoder("Guardrail")
      val json = jsonDecoder.json
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("Guardrail must be a JSON object")
    ...
    override fun serialize(encoder: Encoder, `value`: Guardrail) {
      val jsonEncoder = encoder.requireJsonEncoder("Guardrail")
...
public fun guardrail(block: Guardrail.Builder.() -> Unit): Guardrail = Guardrail.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("Guardrail is missing required property '" + name + "'")
