 ...
 *
 * Source: sdkgen://source/openapi.yaml#/components/schemas/ContextCompressionPlugin
 */
...
public class ContextCompressionPlugin(
  ...
  public class Builder {
    private var idValue: InlineContextCompressionPluginIdX80e8af94? = null

    public var id: InlineContextCompressionPluginIdX80e8af94
  ...
  internal object Serializer : KSerializer<ContextCompressionPlugin> {
    ...
    override fun deserialize(decoder: Decoder): ContextCompressionPlugin {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("ContextCompressionPlugin must be a JSON object")
      val id = json.decodeRequired<InlineContextCompressionPluginIdX80e8af94>(rawObject, "id")
      return ContextCompressionPlugin(
        id = id,
        enabled = rawObject["enabled"]?.let { json.decodeFromJsonElement<Boolean>(it) },
        engine = rawObject["engine"]?.let { json.decodeFromJsonElement<ContextCompressionEngine>(it) },
      )
    }
...
public fun contextCompressionPlugin(block: ContextCompressionPlugin.Builder.() -> Unit): ContextCompressionPlugin = ContextCompressionPlugin.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("ContextCompressionPlugin is missing required property '" + name + "'")
