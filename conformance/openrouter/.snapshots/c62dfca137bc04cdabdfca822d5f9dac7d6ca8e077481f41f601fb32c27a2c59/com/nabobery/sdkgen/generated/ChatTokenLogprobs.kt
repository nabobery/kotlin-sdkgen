 ...
 *
 * Source: sdkgen://source/openapi.yaml#/components/schemas/ChatTokenLogprobs
 */
...
public class ChatTokenLogprobs internal constructor(
  ...
  refusalState: FieldState<List<ChatTokenLogprob>?>,
) {
  /**
   * Log probabilities for content tokens
   */
  public val content: List<ChatTokenLogprob>? = content?.let { collection0 -> collection0.toList() }

  private val refusalState: FieldState<List<ChatTokenLogprob>?> =
      refusalState.copyValue { fieldValue -> fieldValue?.let { collection0 -> collection0.toList() } }
  ...
  public class Builder {
    ...
    public var content: List<ChatTokenLogprob>?
      get() = contentState.valueOrNull()?.let { collection0 -> collection0.toList() }
      set(`value`) {
        contentState = value?.let { collection0 -> collection0.toList() }.toNullableFieldState()
      }
    ...
    public var refusal: List<ChatTokenLogprob>?
      get() = refusalState.valueOrNull()?.let { collection0 -> collection0.toList() }
      set(`value`) {
        refusalState = value?.let { collection0 -> collection0.toList() }.toNullableFieldState()
      }
  ...
  internal object Serializer : KSerializer<ChatTokenLogprobs> {
    ...
    override fun deserialize(decoder: Decoder): ChatTokenLogprobs {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("ChatTokenLogprobs must be a JSON object")
      if (!rawObject.containsKey("content")) {
        throw SerializationException("ChatTokenLogprobs is missing required property 'content'")
      }
      val content = rawObject["content"].let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<List<ChatTokenLogprob>?>(requireNotNull(element)) }
      return ChatTokenLogprobs(
        content = content,
        refusalState = json.decodeOptional(rawObject, "refusal", nullable = true),
      )
    }
...
public fun chatTokenLogprobs(block: ChatTokenLogprobs.Builder.() -> Unit): ChatTokenLogprobs = ChatTokenLogprobs.build(block)
