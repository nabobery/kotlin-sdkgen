...
import com.nabobery.sdkgen.generated.ImageStreamingResponse
...
public class ImagesClient(
   ...
   * Generates an image from a text prompt via the image generation router
   *
   * Streaming counterpart of this operation's buffered `createImages()`/`createImagesWithResponse()` methods: the
   ...
   *
   * Each event's `data` is decoded as `ImageStreamingResponse`; a declared terminal sentinel value ends the stream
   ...
   */
  public fun createImagesStream(
    request: ImageGenerationRequest,
    httpReferer: String? = null,
    xOpenRouterCategories: String? = null,
    xOpenRouterTitle: String? = null,
    options: CallOptions = CallOptions(),
  ): Flow<ImageStreamingResponse> = sseFlow(
    streamProvider = {
      executor.executeRawWithTypedErrors<ImageGenerationRequest, CreateImagesResponse>(
        request = SdkExecutionRequest(createImagesMetadataStream, baseUri, request, listOf(ImagesCodecs.CREATEIMAGES_REQUEST_CODEC_ID), buildList {
          add(SdkRequestParameter(location = SdkParameterLocation.HEADER, name = "HTTP-Referer", values = httpReferer?.let { listOf(it.toString()) }.orEmpty()))
          add(SdkRequestParameter(location = SdkParameterLocation.HEADER, name = "X-OpenRouter-Categories", values = xOpenRouterCategories?.let { listOf(it.toString()) }.orEmpty()))
          add(SdkRequestParameter(location = SdkParameterLocation.HEADER, name = "X-OpenRouter-Title", values = xOpenRouterTitle?.let { listOf(it.toString()) }.orEmpty()))
        }),
    ...
    descriptor = requireNotNull(createImagesMetadataStream.streaming as? StreamingDescriptor.ServerSentEvents),
  ).decodeData { data -> SdkJson.decodeFromString(ImageStreamingResponse.Serializer, data) }
  ...
  public companion object {
    ...
    internal val createImagesMetadataStream: OperationMetadata by
        lazy(LazyThreadSafetyMode.PUBLICATION) { OperationMetadata(
          operationId = "createImages",
          method = "POST",
          path = "/images",
