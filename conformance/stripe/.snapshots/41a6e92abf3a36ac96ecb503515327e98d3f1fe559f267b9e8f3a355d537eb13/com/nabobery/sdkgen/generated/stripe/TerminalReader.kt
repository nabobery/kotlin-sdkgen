...
@Serializable
public data class TerminalReaderView internal constructor(
...
public class TerminalReader(
  ...
  internal object Serializer : KSerializer<TerminalReader> {
    ...
    override fun deserialize(decoder: Decoder): TerminalReader {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("TerminalReader must be a JSON object")
      ...
      return TerminalReader(
        ...
        objectValue = objectValue,
        serialNumber = serialNumber,
        action = rawObject["action"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineTerminalReaderActionXc2e18526?>(element) },
        deviceSwVersion = rawObject["device_sw_version"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        ipAddress = rawObject["ip_address"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        lastSeenAt = rawObject["last_seen_at"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
        location = rawObject["location"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineTerminalReaderLocationX47458740?>(element) },
        status = rawObject["status"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineTerminalReaderStatusX06729bfe?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: TerminalReader) {
...
private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("TerminalReader is missing required property '" + name + "'")
