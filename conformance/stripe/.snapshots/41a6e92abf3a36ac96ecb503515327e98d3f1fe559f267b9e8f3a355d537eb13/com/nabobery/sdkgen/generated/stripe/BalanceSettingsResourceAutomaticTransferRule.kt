...
public class BalanceSettingsResourceAutomaticTransferRule(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): BalanceSettingsResourceAutomaticTransferRule = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<BalanceSettingsResourceAutomaticTransferRule> {
    ...
    override fun deserialize(decoder: Decoder): BalanceSettingsResourceAutomaticTransferRule {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("BalanceSettingsResourceAutomaticTransferRule must be a JSON object")
      val payoutMethod = json.decodeRequired<String>(rawObject, "payout_method")
      val type = json.decodeRequired<InlineBalanceSettingsResourceAutomaticTransferRuleTypeX33d26c37>(rawObject, "type")
      return BalanceSettingsResourceAutomaticTransferRule(
        payoutMethod = payoutMethod,
        type = type,
        transferUpToAmount = rawObject["transfer_up_to_amount"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: BalanceSettingsResourceAutomaticTransferRule) {
...
public fun balanceSettingsResourceAutomaticTransferRule(block: BalanceSettingsResourceAutomaticTransferRule.Builder.() -> Unit): BalanceSettingsResourceAutomaticTransferRule = BalanceSettingsResourceAutomaticTransferRule.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("BalanceSettingsResourceAutomaticTransferRule is missing required property '" + name + "'")
