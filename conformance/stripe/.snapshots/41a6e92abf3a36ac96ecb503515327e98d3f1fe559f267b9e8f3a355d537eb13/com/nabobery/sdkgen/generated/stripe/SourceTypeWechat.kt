...
public class SourceTypeWechat(
  ...
  public class Builder {
    ...
    public fun build(): SourceTypeWechat = SourceTypeWechat(
      prepayId = prepayId,
      qrCodeUrl = qrCodeUrl,
  ...
  internal object Serializer : KSerializer<SourceTypeWechat> {
    ...
    override fun deserialize(decoder: Decoder): SourceTypeWechat {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("SourceTypeWechat must be a JSON object")
      return SourceTypeWechat(
        prepayId = rawObject["prepay_id"]?.let { json.decodeFromJsonElement<String>(it) },
        qrCodeUrl = rawObject["qr_code_url"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        statementDescriptor = rawObject["statement_descriptor"]?.let { json.decodeFromJsonElement<String>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: SourceTypeWechat) {
...
public fun sourceTypeWechat(block: SourceTypeWechat.Builder.() -> Unit): SourceTypeWechat = SourceTypeWechat.build(block)
