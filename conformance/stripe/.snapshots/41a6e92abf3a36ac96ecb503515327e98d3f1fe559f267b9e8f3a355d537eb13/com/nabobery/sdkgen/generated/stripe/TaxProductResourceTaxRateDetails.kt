...
public class TaxProductResourceTaxRateDetails(
  ...
  internal object Serializer : KSerializer<TaxProductResourceTaxRateDetails> {
    ...
    override fun deserialize(decoder: Decoder): TaxProductResourceTaxRateDetails {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("TaxProductResourceTaxRateDetails must be a JSON object")
      ...
      return TaxProductResourceTaxRateDetails(
        ...
        flatAmount = rawObject["flat_amount"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineTaxProductResourceTaxRateDetailsFlatAmountXb1e8ba80?>(element) },
        rateType = rawObject["rate_type"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineTaxProductResourceTaxRateDetailsRateTypeX328c214b?>(element) },
        state = rawObject["state"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        taxType = rawObject["tax_type"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineTaxProductResourceTaxRateDetailsTaxTypeX29d1ece4?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: TaxProductResourceTaxRateDetails) {
...
public fun taxProductResourceTaxRateDetails(block: TaxProductResourceTaxRateDetails.Builder.() -> Unit): TaxProductResourceTaxRateDetails = TaxProductResourceTaxRateDetails.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("TaxProductResourceTaxRateDetails is missing required property '" + name + "'")
