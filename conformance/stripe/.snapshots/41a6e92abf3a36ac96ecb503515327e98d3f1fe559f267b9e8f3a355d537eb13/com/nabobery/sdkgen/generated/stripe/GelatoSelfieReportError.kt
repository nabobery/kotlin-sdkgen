...
public class GelatoSelfieReportError(
  ...
  public class Builder {
    ...
    public fun build(): GelatoSelfieReportError = GelatoSelfieReportError(
      code = code,
      reason = reason,
    )
  }
  ...
  internal object Serializer : KSerializer<GelatoSelfieReportError> {
    ...
    override fun deserialize(decoder: Decoder): GelatoSelfieReportError {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("GelatoSelfieReportError must be a JSON object")
      return GelatoSelfieReportError(
        code = rawObject["code"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineGelatoSelfieReportErrorCodeX38374890?>(element) },
        reason = rawObject["reason"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: GelatoSelfieReportError) {
...
public fun gelatoSelfieReportError(block: GelatoSelfieReportError.Builder.() -> Unit): GelatoSelfieReportError = GelatoSelfieReportError.build(block)
