...
public class DisputePaymentMethodDetailsCard(
  ...
  internal object Serializer : KSerializer<DisputePaymentMethodDetailsCard> {
    ...
    override fun deserialize(decoder: Decoder): DisputePaymentMethodDetailsCard {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("DisputePaymentMethodDetailsCard must be a JSON object")
      val brand = json.decodeRequired<String>(rawObject, "brand")
      val caseType = json.decodeRequired<InlineDisputePaymentMethodDetailsCardCaseTypeX14cff061>(rawObject, "case_type")
      return DisputePaymentMethodDetailsCard(
        brand = brand,
        caseType = caseType,
        networkReasonCode = rawObject["network_reason_code"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: DisputePaymentMethodDetailsCard) {
...
public fun disputePaymentMethodDetailsCard(block: DisputePaymentMethodDetailsCard.Builder.() -> Unit): DisputePaymentMethodDetailsCard = DisputePaymentMethodDetailsCard.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("DisputePaymentMethodDetailsCard is missing required property '" + name + "'")
