...
@Serializable
public data class AccountView internal constructor(
...
public class Account(
  ...
  internal object Serializer : KSerializer<Account> {
    override val descriptor: SerialDescriptor = JsonElement.serializer().descriptor

    override fun deserialize(decoder: Decoder): Account {
      val jsonDecoder = decoder.requireJsonDecoder("Account")
      val json = jsonDecoder.json
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("Account must be a JSON object")
      val id = json.decodeRequired<String>(rawObject, "id")
      val objectValue = json.decodeRequired<InlineAccountObjectValueX7a2c1f55>(rawObject, "object")
      return Account(
        id = id,
        objectValue = objectValue,
        businessProfile = rawObject["business_profile"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineAccountBusinessProfileX8962a2f6?>(element) },
        businessType = rawObject["business_type"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineAccountBusinessTypeX0dd32b67?>(element) },
        ...
        controller = rawObject["controller"]?.let { json.decodeFromJsonElement<AccountUnificationAccountController>(it) },
        ...
        externalAccounts = rawObject["external_accounts"]?.let { json.decodeFromJsonElement<InlineAccountExternalAccountsXace51fb0>(it) },
        futureRequirements = rawObject["future_requirements"]?.let { json.decodeFromJsonElement<AccountFutureRequirements>(it) },
        groups = rawObject["groups"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineAccountGroupsXb65c8b1c?>(element) },
        ...
        settings = rawObject["settings"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineAccountSettingsX8aa98c30?>(element) },
        tosAcceptance = rawObject["tos_acceptance"]?.let { json.decodeFromJsonElement<AccountTosAcceptance>(it) },
