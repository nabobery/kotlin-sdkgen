...
public class SourceTypeAcssDebit(
  ...
  public class Builder {
    ...
    public fun build(): SourceTypeAcssDebit = SourceTypeAcssDebit(
  ...
  internal object Serializer : KSerializer<SourceTypeAcssDebit> {
    ...
    override fun deserialize(decoder: Decoder): SourceTypeAcssDebit {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("SourceTypeAcssDebit must be a JSON object")
      return SourceTypeAcssDebit(
        bankAddressCity = rawObject["bank_address_city"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        bankAddressLine1 = rawObject["bank_address_line_1"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        bankAddressLine2 = rawObject["bank_address_line_2"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        bankAddressPostalCode = rawObject["bank_address_postal_code"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        bankName = rawObject["bank_name"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        category = rawObject["category"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
    ...
    override fun serialize(encoder: Encoder, `value`: SourceTypeAcssDebit) {
...
public fun sourceTypeAcssDebit(block: SourceTypeAcssDebit.Builder.() -> Unit): SourceTypeAcssDebit = SourceTypeAcssDebit.build(block)
