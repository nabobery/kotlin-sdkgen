...
public class InvoicePaymentMethodOptionsCard(
  ...
  public class Builder {
    ...
    public fun build(): InvoicePaymentMethodOptionsCard = InvoicePaymentMethodOptionsCard(
  ...
  internal object Serializer : KSerializer<InvoicePaymentMethodOptionsCard> {
    ...
    override fun deserialize(decoder: Decoder): InvoicePaymentMethodOptionsCard {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("InvoicePaymentMethodOptionsCard must be a JSON object")
      return InvoicePaymentMethodOptionsCard(
        installments = rawObject["installments"]?.let { json.decodeFromJsonElement<InvoiceInstallmentsCard>(it) },
        requestThreeDSecure = rawObject["request_three_d_secure"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineInvoicePaymentMethodOptionsCardRequestThreeDSecureX6b0f6085?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: InvoicePaymentMethodOptionsCard) {
...
public fun invoicePaymentMethodOptionsCard(block: InvoicePaymentMethodOptionsCard.Builder.() -> Unit): InvoicePaymentMethodOptionsCard = InvoicePaymentMethodOptionsCard.build(block)
