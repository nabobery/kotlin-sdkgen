...
public class IssuingDisputeCanceledEvidence(
  ...
  public class Builder {
    ...
    public fun build(): IssuingDisputeCanceledEvidence = IssuingDisputeCanceledEvidence(
  ...
  internal object Serializer : KSerializer<IssuingDisputeCanceledEvidence> {
    ...
    override fun deserialize(decoder: Decoder): IssuingDisputeCanceledEvidence {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("IssuingDisputeCanceledEvidence must be a JSON object")
      return IssuingDisputeCanceledEvidence(
        additionalDocumentation = rawObject["additional_documentation"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingDisputeCanceledEvidenceAdditionalDocumentationX7b60331e?>(element) },
        canceledAt = rawObject["canceled_at"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
        cancellationPolicyProvided = rawObject["cancellation_policy_provided"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Boolean?>(element) },
        ...
        expectedAt = rawObject["expected_at"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
        ...
        productType = rawObject["product_type"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingDisputeCanceledEvidenceProductTypeX3f171230?>(element) },
        returnStatus = rawObject["return_status"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingDisputeCanceledEvidenceReturnStatusXcb1fe3ee?>(element) },
        returnedAt = rawObject["returned_at"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: IssuingDisputeCanceledEvidence) {
...
public fun issuingDisputeCanceledEvidence(block: IssuingDisputeCanceledEvidence.Builder.() -> Unit): IssuingDisputeCanceledEvidence = IssuingDisputeCanceledEvidence.build(block)
