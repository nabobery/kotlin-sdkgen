...
public class IssuingNetworkTokenVisa(
  ...
  internal object Serializer : KSerializer<IssuingNetworkTokenVisa> {
    ...
    override fun deserialize(decoder: Decoder): IssuingNetworkTokenVisa {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("IssuingNetworkTokenVisa must be a JSON object")
      ...
      return IssuingNetworkTokenVisa(
        tokenReferenceId = tokenReferenceId,
        tokenRequestorId = tokenRequestorId,
        cardReferenceId = rawObject["card_reference_id"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        tokenRiskScore = rawObject["token_risk_score"]?.let { json.decodeFromJsonElement<String>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: IssuingNetworkTokenVisa) {
...
public fun issuingNetworkTokenVisa(block: IssuingNetworkTokenVisa.Builder.() -> Unit): IssuingNetworkTokenVisa = IssuingNetworkTokenVisa.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("IssuingNetworkTokenVisa is missing required property '" + name + "'")
