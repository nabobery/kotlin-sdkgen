...
public class FundingInstructions(
  ...
  internal object Serializer : KSerializer<FundingInstructions> {
    ...
    override fun deserialize(decoder: Decoder): FundingInstructions {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("FundingInstructions must be a JSON object")
      val bankTransfer = json.decodeRequired<FundingInstructionsBankTransfer>(rawObject, "bank_transfer")
      val currency = json.decodeRequired<String>(rawObject, "currency")
      val fundingType = json.decodeRequired<InlineFundingInstructionsFundingTypeX8e9a652f>(rawObject, "funding_type")
    ...
    override fun serialize(encoder: Encoder, `value`: FundingInstructions) {
...
public fun fundingInstructions(block: FundingInstructions.Builder.() -> Unit): FundingInstructions = FundingInstructions.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("FundingInstructions is missing required property '" + name + "'")
