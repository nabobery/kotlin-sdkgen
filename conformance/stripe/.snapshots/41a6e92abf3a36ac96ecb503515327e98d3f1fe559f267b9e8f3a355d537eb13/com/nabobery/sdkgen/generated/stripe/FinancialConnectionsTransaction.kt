...
public class FinancialConnectionsTransaction(
  ...
  internal object Serializer : KSerializer<FinancialConnectionsTransaction> {
    ...
    override fun deserialize(decoder: Decoder): FinancialConnectionsTransaction {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("FinancialConnectionsTransaction must be a JSON object")
      ...
      val objectValue = json.decodeRequired<InlineFinancialConnectionsTransactionObjectValueX76c5739e>(rawObject, "object")
      val status = json.decodeRequired<InlineFinancialConnectionsTransactionStatusXa15a6333>(rawObject, "status")
      val statusTransitions = json.decodeRequired<BankConnectionsResourceTransactionResourceStatusTransitions>(rawObject, "status_transitions")
    ...
    override fun serialize(encoder: Encoder, `value`: FinancialConnectionsTransaction) {
...
public fun financialConnectionsTransaction(block: FinancialConnectionsTransaction.Builder.() -> Unit): FinancialConnectionsTransaction = FinancialConnectionsTransaction.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("FinancialConnectionsTransaction is missing required property '" + name + "'")
