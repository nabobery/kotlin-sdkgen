...
public data class PaymentPagesCheckoutSessionConsentView(
  ...
  public val termsOfService:
      InlinePaymentPagesCheckoutSessionConsentTermsOfServiceXad324f3b? = null,
)
...
public class PaymentPagesCheckoutSessionConsent(
  ...
  public class Builder {
    ...
    public fun build(): PaymentPagesCheckoutSessionConsent = PaymentPagesCheckoutSessionConsent(
  ...
  internal object Serializer : KSerializer<PaymentPagesCheckoutSessionConsent> {
    ...
    override fun deserialize(decoder: Decoder): PaymentPagesCheckoutSessionConsent {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentPagesCheckoutSessionConsent must be a JSON object")
      return PaymentPagesCheckoutSessionConsent(
        promotions = rawObject["promotions"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentPagesCheckoutSessionConsentPromotionsX53836b17?>(element) },
        termsOfService = rawObject["terms_of_service"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentPagesCheckoutSessionConsentTermsOfServiceXad324f3b?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PaymentPagesCheckoutSessionConsent) {
...
public fun paymentPagesCheckoutSessionConsent(block: PaymentPagesCheckoutSessionConsent.Builder.() -> Unit): PaymentPagesCheckoutSessionConsent = PaymentPagesCheckoutSessionConsent.build(block)
