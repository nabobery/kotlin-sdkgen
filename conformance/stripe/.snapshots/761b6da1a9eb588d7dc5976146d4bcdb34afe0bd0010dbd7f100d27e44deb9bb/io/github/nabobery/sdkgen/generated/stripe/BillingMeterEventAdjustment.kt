...
public class BillingMeterEventAdjustment(
  ...
  internal object Serializer : KSerializer<BillingMeterEventAdjustment> {
    ...
    override fun deserialize(decoder: Decoder): BillingMeterEventAdjustment {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("BillingMeterEventAdjustment must be a JSON object")
      val eventName = json.decodeRequired<String>(rawObject, "event_name")
      val livemode = json.decodeRequired<Boolean>(rawObject, "livemode")
      val objectValue = json.decodeRequired<InlineBillingMeterEventAdjustmentObjectValueX9bd06423>(rawObject, "object")
      ...
      return BillingMeterEventAdjustment(
        ...
        status = status,
        type = type,
        cancel = rawObject["cancel"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineBillingMeterEventAdjustmentCancelX908730ed?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: BillingMeterEventAdjustment) {
...
public fun billingMeterEventAdjustment(block: BillingMeterEventAdjustment.Builder.() -> Unit): BillingMeterEventAdjustment = BillingMeterEventAdjustment.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("BillingMeterEventAdjustment is missing required property '" + name + "'")
