...
public class TerminalReaderReaderResourceProcessSetupIntentAction(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): TerminalReaderReaderResourceProcessSetupIntentAction = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<TerminalReaderReaderResourceProcessSetupIntentAction> {
    ...
    override fun deserialize(decoder: Decoder): TerminalReaderReaderResourceProcessSetupIntentAction {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("TerminalReaderReaderResourceProcessSetupIntentAction must be a JSON object")
      val setupIntent = json.decodeRequired<InlineTerminalReaderReaderResourceProcessSetupIntentActionSetupIntentXb1c809a3>(rawObject, "setup_intent")
      return TerminalReaderReaderResourceProcessSetupIntentAction(
        setupIntent = setupIntent,
        generatedCard = rawObject["generated_card"]?.let { json.decodeFromJsonElement<String>(it) },
        processConfig = rawObject["process_config"]?.let { json.decodeFromJsonElement<TerminalReaderReaderResourceProcessSetupConfig>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: TerminalReaderReaderResourceProcessSetupIntentAction) {
...
public fun terminalReaderReaderResourceProcessSetupIntentAction(block: TerminalReaderReaderResourceProcessSetupIntentAction.Builder.() -> Unit): TerminalReaderReaderResourceProcessSetupIntentAction = TerminalReaderReaderResourceProcessSetupIntentAction.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("TerminalReaderReaderResourceProcessSetupIntentAction is missing required property '" + name + "'")
