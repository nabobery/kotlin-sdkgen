...
@Serializable
public data class TopupView internal constructor(
...
public class Topup(
  ...
  internal object Serializer : KSerializer<Topup> {
    override val descriptor: SerialDescriptor = JsonElement.serializer().descriptor

    override fun deserialize(decoder: Decoder): Topup {
      val jsonDecoder = decoder.requireJsonDecoder("Topup")
      val json = jsonDecoder.json
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("Topup must be a JSON object")
      ...
      return Topup(
        ...
        objectValue = objectValue,
        status = status,
        balanceTransaction = rawObject["balance_transaction"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineTopupBalanceTransactionX090a295d?>(element) },
        description = rawObject["description"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        expectedAvailabilityDate = rawObject["expected_availability_date"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
        ...
        source = rawObject["source"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineTopupSourceX47d406a9?>(element) },
    ...
    override fun serialize(encoder: Encoder, `value`: Topup) {
      ...
      val raw = buildJsonObject {
        ...
        value.expectedAvailabilityDate?.let { put("expected_availability_date", json.encodeToJsonElement(it)) }
