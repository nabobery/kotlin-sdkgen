...
public class CreditNoteRefund(
  ...
  internal object Serializer : KSerializer<CreditNoteRefund> {
    ...
    override fun deserialize(decoder: Decoder): CreditNoteRefund {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("CreditNoteRefund must be a JSON object")
      ...
      return CreditNoteRefund(
        amountRefunded = amountRefunded,
        refund = refund,
        paymentRecordRefund = rawObject["payment_record_refund"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineCreditNoteRefundPaymentRecordRefundX09797674?>(element) },
        type = rawObject["type"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineCreditNoteRefundTypeXc2c9b186?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: CreditNoteRefund) {
...
public fun creditNoteRefund(block: CreditNoteRefund.Builder.() -> Unit): CreditNoteRefund = CreditNoteRefund.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("CreditNoteRefund is missing required property '" + name + "'")
