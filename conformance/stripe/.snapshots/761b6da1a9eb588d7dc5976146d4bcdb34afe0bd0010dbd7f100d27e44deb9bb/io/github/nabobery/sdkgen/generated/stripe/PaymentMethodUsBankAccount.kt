...
public class PaymentMethodUsBankAccount(
  ...
  public class Builder {
    ...
    public fun build(): PaymentMethodUsBankAccount = PaymentMethodUsBankAccount(
  ...
  internal object Serializer : KSerializer<PaymentMethodUsBankAccount> {
    ...
    override fun deserialize(decoder: Decoder): PaymentMethodUsBankAccount {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentMethodUsBankAccount must be a JSON object")
      return PaymentMethodUsBankAccount(
        accountHolderType = rawObject["account_holder_type"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentMethodUsBankAccountAccountHolderTypeX91c2d668?>(element) },
        accountType = rawObject["account_type"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentMethodUsBankAccountAccountTypeX72df841c?>(element) },
        bankName = rawObject["bank_name"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        financialConnectionsAccount = rawObject["financial_connections_account"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        ...
        networks = rawObject["networks"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentMethodUsBankAccountNetworksX15dfa0ca?>(element) },
        ...
        statusDetails = rawObject["status_details"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentMethodUsBankAccountStatusDetailsX20e39ea0?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PaymentMethodUsBankAccount) {
...
public fun paymentMethodUsBankAccount(block: PaymentMethodUsBankAccount.Builder.() -> Unit): PaymentMethodUsBankAccount = PaymentMethodUsBankAccount.build(block)
