...
public class BillingPortalSession(
  ...
  internal object Serializer : KSerializer<BillingPortalSession> {
    ...
    override fun deserialize(decoder: Decoder): BillingPortalSession {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("BillingPortalSession must be a JSON object")
      val configuration = json.decodeRequired<InlineBillingPortalSessionConfigurationX04be7630>(rawObject, "configuration")
      ...
      return BillingPortalSession(
        ...
        flow = rawObject["flow"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineBillingPortalSessionFlowX96cb984f?>(element) },
        locale = rawObject["locale"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineBillingPortalSessionLocaleXe8a0c699?>(element) },
    ...
    override fun serialize(encoder: Encoder, `value`: BillingPortalSession) {
...
public fun billingPortalSession(block: BillingPortalSession.Builder.() -> Unit): BillingPortalSession = BillingPortalSession.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("BillingPortalSession is missing required property '" + name + "'")
