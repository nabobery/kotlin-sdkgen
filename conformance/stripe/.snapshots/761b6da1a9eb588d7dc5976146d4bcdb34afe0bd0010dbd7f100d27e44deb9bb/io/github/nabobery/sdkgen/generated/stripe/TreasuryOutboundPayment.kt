...
public class TreasuryOutboundPayment(
  ...
  internal object Serializer : KSerializer<TreasuryOutboundPayment> {
    ...
    override fun deserialize(decoder: Decoder): TreasuryOutboundPayment {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("TreasuryOutboundPayment must be a JSON object")
      ...
      val objectValue = json.decodeRequired<InlineTreasuryOutboundPaymentObjectValueX17c509e1>(rawObject, "object")
      ...
      val statusTransitions = json.decodeRequired<TreasuryOutboundPaymentsResourceOutboundPaymentResourceStatusTransitions>(rawObject, "status_transitions")
      val transaction = json.decodeRequired<InlineTreasuryOutboundPaymentTransactionXdbbd9cc7>(rawObject, "transaction")
      return TreasuryOutboundPayment(
        amount = amount,
        cancelable = cancelable,
        created = created,
        ...
        destinationPaymentMethodDetails = rawObject["destination_payment_method_details"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineTreasuryOutboundPaymentDestinationPaymentMethodDetailsX5cf24d5d?>(element) },
        endUserDetails = rawObject["end_user_details"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineTreasuryOutboundPaymentEndUserDetailsX33dd7caf?>(element) },
        ...
        returnedDetails = rawObject["returned_details"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineTreasuryOutboundPaymentReturnedDetailsX599a78af?>(element) },
        trackingDetails = rawObject["tracking_details"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineTreasuryOutboundPaymentTrackingDetailsX7d731848?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: TreasuryOutboundPayment) {
      ...
      val raw = buildJsonObject {
        ...
        value.destinationPaymentMethodDetails?.let { put("destination_payment_method_details", json.encodeToJsonElement(it)) }
...
public fun treasuryOutboundPayment(block: TreasuryOutboundPayment.Builder.() -> Unit): TreasuryOutboundPayment = TreasuryOutboundPayment.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("TreasuryOutboundPayment is missing required property '" + name + "'")
