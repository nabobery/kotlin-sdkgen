...
public class IdentityVerificationReport(
  ...
  internal object Serializer : KSerializer<IdentityVerificationReport> {
    ...
    override fun deserialize(decoder: Decoder): IdentityVerificationReport {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("IdentityVerificationReport must be a JSON object")
      val created = json.decodeRequired<Int>(rawObject, "created")
      val id = json.decodeRequired<String>(rawObject, "id")
      val livemode = json.decodeRequired<Boolean>(rawObject, "livemode")
      val objectValue = json.decodeRequired<InlineIdentityVerificationReportObjectValueXbd208f05>(rawObject, "object")
      ...
      return IdentityVerificationReport(
        ...
        verificationSession = rawObject["verification_session"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: IdentityVerificationReport) {
...
public fun identityVerificationReport(block: IdentityVerificationReport.Builder.() -> Unit): IdentityVerificationReport = IdentityVerificationReport.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("IdentityVerificationReport is missing required property '" + name + "'")
