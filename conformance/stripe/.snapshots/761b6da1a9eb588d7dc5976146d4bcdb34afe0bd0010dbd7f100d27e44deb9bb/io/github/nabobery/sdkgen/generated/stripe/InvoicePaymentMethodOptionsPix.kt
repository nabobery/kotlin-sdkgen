...
public data class InvoicePaymentMethodOptionsPixView(
  ...
  public val amountIncludesIof:
      InlineInvoicePaymentMethodOptionsPixAmountIncludesIofX23578152? = null,
...
public class InvoicePaymentMethodOptionsPix(
  ...
  public class Builder {
    ...
    public fun build(): InvoicePaymentMethodOptionsPix = InvoicePaymentMethodOptionsPix(
  ...
  internal object Serializer : KSerializer<InvoicePaymentMethodOptionsPix> {
    ...
    override fun deserialize(decoder: Decoder): InvoicePaymentMethodOptionsPix {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("InvoicePaymentMethodOptionsPix must be a JSON object")
      return InvoicePaymentMethodOptionsPix(
        amountIncludesIof = rawObject["amount_includes_iof"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineInvoicePaymentMethodOptionsPixAmountIncludesIofX23578152?>(element) },
        expiresAfterSeconds = rawObject["expires_after_seconds"]?.let { json.decodeFromJsonElement<Int>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: InvoicePaymentMethodOptionsPix) {
...
public fun invoicePaymentMethodOptionsPix(block: InvoicePaymentMethodOptionsPix.Builder.() -> Unit): InvoicePaymentMethodOptionsPix = InvoicePaymentMethodOptionsPix.build(block)
