...
public class InlineV1ShippingRatesPostRequestFormXc4ae2656(
  ...
  public val deliveryEstimate:
      InlineV1ShippingRatesPostRequestFormDeliveryEstimateX04154e37? = null,
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): InlineV1ShippingRatesPostRequestFormXc4ae2656 = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<InlineV1ShippingRatesPostRequestFormXc4ae2656> {
    ...
    override fun deserialize(decoder: Decoder): InlineV1ShippingRatesPostRequestFormXc4ae2656 {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("InlineV1ShippingRatesPostRequestFormXc4ae2656 must be a JSON object")
      val displayName = json.decodeRequired<String>(rawObject, "display_name")
      return InlineV1ShippingRatesPostRequestFormXc4ae2656(
        displayName = displayName,
        deliveryEstimate = rawObject["delivery_estimate"]?.let { json.decodeFromJsonElement<InlineV1ShippingRatesPostRequestFormDeliveryEstimateX04154e37>(it) },
        expand = rawObject["expand"]?.let { json.decodeFromJsonElement<List<String>>(it) },
        fixedAmount = rawObject["fixed_amount"]?.let { json.decodeFromJsonElement<InlineV1ShippingRatesPostRequestFormFixedAmountX0bd5d011>(it) },
        metadata = rawObject["metadata"]?.let { json.decodeFromJsonElement<Map<String, String>>(it) },
        taxBehavior = rawObject["tax_behavior"]?.let { json.decodeFromJsonElement<InlineV1ShippingRatesPostRequestFormTaxBehaviorXb6d21e88>(it) },
        taxCode = rawObject["tax_code"]?.let { json.decodeFromJsonElement<String>(it) },
        type = rawObject["type"]?.let { json.decodeFromJsonElement<InlineV1ShippingRatesPostRequestFormTypeX43c932eb>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: InlineV1ShippingRatesPostRequestFormXc4ae2656) {
...
public fun inlineV1ShippingRatesPostRequestFormXc4ae2656(block: InlineV1ShippingRatesPostRequestFormXc4ae2656.Builder.() -> Unit): InlineV1ShippingRatesPostRequestFormXc4ae2656 = InlineV1ShippingRatesPostRequestFormXc4ae2656.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("InlineV1ShippingRatesPostRequestFormXc4ae2656 is missing required property '" + name + "'")
