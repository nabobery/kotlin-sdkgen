...
public class SourceReceiverFlow(
  ...
  internal object Serializer : KSerializer<SourceReceiverFlow> {
    ...
    override fun deserialize(decoder: Decoder): SourceReceiverFlow {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("SourceReceiverFlow must be a JSON object")
      ...
      return SourceReceiverFlow(
        ...
        refundAttributesMethod = refundAttributesMethod,
        refundAttributesStatus = refundAttributesStatus,
        address = rawObject["address"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: SourceReceiverFlow) {
...
public fun sourceReceiverFlow(block: SourceReceiverFlow.Builder.() -> Unit): SourceReceiverFlow = SourceReceiverFlow.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("SourceReceiverFlow is missing required property '" + name + "'")
