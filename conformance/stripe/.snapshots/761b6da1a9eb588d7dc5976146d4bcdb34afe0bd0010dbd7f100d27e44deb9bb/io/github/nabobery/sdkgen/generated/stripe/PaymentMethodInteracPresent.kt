...
public class PaymentMethodInteracPresent(
  ...
  internal object Serializer : KSerializer<PaymentMethodInteracPresent> {
    ...
    override fun deserialize(decoder: Decoder): PaymentMethodInteracPresent {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentMethodInteracPresent must be a JSON object")
      ...
      return PaymentMethodInteracPresent(
        ...
        networks = rawObject["networks"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentMethodInteracPresentNetworksX55abbf8b?>(element) },
        ...
        readMethod = rawObject["read_method"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentMethodInteracPresentReadMethodXb4618b29?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PaymentMethodInteracPresent) {
...
public fun paymentMethodInteracPresent(block: PaymentMethodInteracPresent.Builder.() -> Unit): PaymentMethodInteracPresent = PaymentMethodInteracPresent.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("PaymentMethodInteracPresent is missing required property '" + name + "'")
