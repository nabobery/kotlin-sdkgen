...
public class InlineV1PaymentIntentsPostRequestFormTransferDataXb1202e5d(
  ...
  public val paymentData:
      InlineV1PaymentIntentsPostRequestFormTransferDataPaymentDataX69b4db7a? = null,
) {
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): InlineV1PaymentIntentsPostRequestFormTransferDataXb1202e5d = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<InlineV1PaymentIntentsPostRequestFormTransferDataXb1202e5d> {
    ...
    override fun deserialize(decoder: Decoder): InlineV1PaymentIntentsPostRequestFormTransferDataXb1202e5d {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("InlineV1PaymentIntentsPostRequestFormTransferDataXb1202e5d must be a JSON object")
      val destination = json.decodeRequired<String>(rawObject, "destination")
      return InlineV1PaymentIntentsPostRequestFormTransferDataXb1202e5d(
        destination = destination,
        amount = rawObject["amount"]?.let { json.decodeFromJsonElement<Int>(it) },
        description = rawObject["description"]?.let { json.decodeFromJsonElement<String>(it) },
        metadata = rawObject["metadata"]?.let { json.decodeFromJsonElement<InlineV1PaymentIntentsPostRequestFormTransferDataMetadataX78f9e4ca>(it) },
        paymentData = rawObject["payment_data"]?.let { json.decodeFromJsonElement<InlineV1PaymentIntentsPostRequestFormTransferDataPaymentDataX69b4db7a>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: InlineV1PaymentIntentsPostRequestFormTransferDataXb1202e5d) {
...
public fun inlineV1PaymentIntentsPostRequestFormTransferDataXb1202e5d(block: InlineV1PaymentIntentsPostRequestFormTransferDataXb1202e5d.Builder.() -> Unit): InlineV1PaymentIntentsPostRequestFormTransferDataXb1202e5d = InlineV1PaymentIntentsPostRequestFormTransferDataXb1202e5d.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("InlineV1PaymentIntentsPostRequestFormTransferDataXb1202e5d is missing required property '" + name + "'")
