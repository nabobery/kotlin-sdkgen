...
public class CustomerBalanceResourceCashBalanceTransactionResourceAdjustedForOverdraft(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): CustomerBalanceResourceCashBalanceTransactionResourceAdjustedForOverdraft = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<CustomerBalanceResourceCashBalanceTransactionResourceAdjustedForOverdraft> {
    override val descriptor: SerialDescriptor = JsonElement.serializer().descriptor

    override fun deserialize(decoder: Decoder): CustomerBalanceResourceCashBalanceTransactionResourceAdjustedForOverdraft {
      val jsonDecoder = decoder.requireJsonDecoder("CustomerBalanceResourceCashBalanceTransactionResourceAdjustedForOverdraft")
      val json = jsonDecoder.json
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("CustomerBalanceResourceCashBalanceTransactionResourceAdjustedForOverdraft must be a JSON object")
      val balanceTransaction = json.decodeRequired<InlineCustomerBalanceResou7348BalanceTransactionX26d64799>(rawObject, "balance_transaction")
      val linkedTransaction = json.decodeRequired<InlineCustomerBalanceResou7348LinkedTransactionXd7ab40d7>(rawObject, "linked_transaction")
      return CustomerBalanceResourceCashBalanceTransactionResourceAdjustedForOverdraft(
        balanceTransaction = balanceTransaction,
        linkedTransaction = linkedTransaction,
      )
    ...
    override fun serialize(encoder: Encoder, `value`: CustomerBalanceResourceCashBalanceTransactionResourceAdjustedForOverdraft) {
      val jsonEncoder = encoder.requireJsonEncoder("CustomerBalanceResourceCashBalanceTransactionResourceAdjustedForOverdraft")
...
public fun customerBalanceResourceCashBalanceTransactionResourceAdjustedForOverdraft(block: CustomerBalanceResourceCashBalanceTransactionResourceAdjustedForOverdraft.Builder.() -> Unit): CustomerBalanceResourceCashBalanceTransactionResourceAdjustedForOverdraft = CustomerBalanceResourceCashBalanceTransactionResourceAdjustedForOverdraft.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("CustomerBalanceResourceCashBalanceTransactionResourceAdjustedForOverdraft is missing required property '" + name + "'")
