...
@Serializable
public data class CouponView internal constructor(
...
public class Coupon(
   ...
   */
  public val currency: String? = null,
  currencyOptions: Map<String, CouponCurrencyOption>? = null,
  /**
   ...
   */
  public val currencyOptions: Map<String, CouponCurrencyOption>? =
  ...
  public class Builder {
    ...
    private var currencyOptionsValue: Map<String, CouponCurrencyOption>? = null
     ...
     * currency](https://stripe.com/docs/currencies).
     */
    public var currencyOptions: Map<String, CouponCurrencyOption>?
  ...
  internal object Serializer : KSerializer<Coupon> {
    override val descriptor: SerialDescriptor = JsonElement.serializer().descriptor

    override fun deserialize(decoder: Decoder): Coupon {
      val jsonDecoder = decoder.requireJsonDecoder("Coupon")
      val json = jsonDecoder.json
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("Coupon must be a JSON object")
      ...
      return Coupon(
        ...
        timesRedeemed = timesRedeemed,
        valid = valid,
        amountOff = rawObject["amount_off"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
        ...
        currencyOptions = rawObject["currency_options"]?.let { json.decodeFromJsonElement<Map<String, CouponCurrencyOption>>(it) },
        durationInMonths = rawObject["duration_in_months"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
        ...
        percentOff = rawObject["percent_off"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Double?>(element) },
        redeemBy = rawObject["redeem_by"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: Coupon) {
