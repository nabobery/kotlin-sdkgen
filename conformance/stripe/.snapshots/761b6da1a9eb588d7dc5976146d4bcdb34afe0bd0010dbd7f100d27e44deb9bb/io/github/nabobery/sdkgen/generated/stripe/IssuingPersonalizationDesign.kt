...
@Serializable
public data class IssuingPersonalizationDesignView internal constructor(
...
public class IssuingPersonalizationDesign(
  ...
  internal object Serializer : KSerializer<IssuingPersonalizationDesign> {
    ...
    override fun deserialize(decoder: Decoder): IssuingPersonalizationDesign {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("IssuingPersonalizationDesign must be a JSON object")
      val created = json.decodeRequired<Int>(rawObject, "created")
      val id = json.decodeRequired<String>(rawObject, "id")
      val livemode = json.decodeRequired<Boolean>(rawObject, "livemode")
      val metadata = json.decodeRequired<Map<String, String>>(rawObject, "metadata")
      val objectValue = json.decodeRequired<InlineIssuingPersonalizationDesignObjectValueX8418564b>(rawObject, "object")
      val physicalBundle = json.decodeRequired<InlineIssuingPersonalizationDesignPhysicalBundleXdaa9c3d3>(rawObject, "physical_bundle")
      val preferences = json.decodeRequired<IssuingPersonalizationDesignPreferences>(rawObject, "preferences")
      val rejectionReasons = json.decodeRequired<IssuingPersonalizationDesignRejectionReasons>(rawObject, "rejection_reasons")
      ...
      return IssuingPersonalizationDesign(
        ...
        rejectionReasons = rejectionReasons,
        status = status,
        cardLogo = rawObject["card_logo"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingPersonalizationDesignCardLogoX9055fd98?>(element) },
        carrierText = rawObject["carrier_text"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingPersonalizationDesignCarrierTextX2a2ed5ae?>(element) },
    ...
    override fun serialize(encoder: Encoder, `value`: IssuingPersonalizationDesign) {
...
public fun issuingPersonalizationDesign(block: IssuingPersonalizationDesign.Builder.() -> Unit): IssuingPersonalizationDesign = IssuingPersonalizationDesign.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("IssuingPersonalizationDesign is missing required property '" + name + "'")
