...
public class PaymentMethodDomainResourcePaymentMethodStatus(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): PaymentMethodDomainResourcePaymentMethodStatus = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<PaymentMethodDomainResourcePaymentMethodStatus> {
    ...
    override fun deserialize(decoder: Decoder): PaymentMethodDomainResourcePaymentMethodStatus {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentMethodDomainResourcePaymentMethodStatus must be a JSON object")
      val status = json.decodeRequired<InlinePaymentMethodDomainResourcePaymentMethodStatusStatusXe5edfc9d>(rawObject, "status")
      return PaymentMethodDomainResourcePaymentMethodStatus(
        status = status,
        statusDetails = rawObject["status_details"]?.let { json.decodeFromJsonElement<PaymentMethodDomainResourcePaymentMethodStatusDetails>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PaymentMethodDomainResourcePaymentMethodStatus) {
...
public fun paymentMethodDomainResourcePaymentMethodStatus(block: PaymentMethodDomainResourcePaymentMethodStatus.Builder.() -> Unit): PaymentMethodDomainResourcePaymentMethodStatus = PaymentMethodDomainResourcePaymentMethodStatus.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("PaymentMethodDomainResourcePaymentMethodStatus is missing required property '" + name + "'")
