...
public class CustomUnitAmount(
  ...
  public class Builder {
    ...
    public fun build(): CustomUnitAmount = CustomUnitAmount(
      maximum = maximum,
      minimum = minimum,
      preset = preset,
    )
  ...
  internal object Serializer : KSerializer<CustomUnitAmount> {
    ...
    override fun deserialize(decoder: Decoder): CustomUnitAmount {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("CustomUnitAmount must be a JSON object")
      return CustomUnitAmount(
        ...
        preset = rawObject["preset"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: CustomUnitAmount) {
...
public fun customUnitAmount(block: CustomUnitAmount.Builder.() -> Unit): CustomUnitAmount = CustomUnitAmount.build(block)
