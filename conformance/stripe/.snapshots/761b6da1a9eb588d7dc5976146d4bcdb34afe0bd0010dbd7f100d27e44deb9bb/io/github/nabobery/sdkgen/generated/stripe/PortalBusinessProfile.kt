...
public class PortalBusinessProfile(
  ...
  public class Builder {
    ...
    public fun build(): PortalBusinessProfile = PortalBusinessProfile(
      headline = headline,
  ...
  internal object Serializer : KSerializer<PortalBusinessProfile> {
    ...
    override fun deserialize(decoder: Decoder): PortalBusinessProfile {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PortalBusinessProfile must be a JSON object")
      return PortalBusinessProfile(
        headline = rawObject["headline"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        privacyPolicyUrl = rawObject["privacy_policy_url"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        termsOfServiceUrl = rawObject["terms_of_service_url"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PortalBusinessProfile) {
...
public fun portalBusinessProfile(block: PortalBusinessProfile.Builder.() -> Unit): PortalBusinessProfile = PortalBusinessProfile.build(block)
