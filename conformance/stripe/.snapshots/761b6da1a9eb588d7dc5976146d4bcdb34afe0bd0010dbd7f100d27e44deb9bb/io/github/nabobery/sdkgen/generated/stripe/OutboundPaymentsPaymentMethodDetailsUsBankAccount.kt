...
public class OutboundPaymentsPaymentMethodDetailsUsBankAccount(
   ...
   */
  public val mandate:
      InlineOutboundPaymentsPaymentMethodDetailsUsBankAccountMandateX78d61086? = null,
  /**
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): OutboundPaymentsPaymentMethodDetailsUsBankAccount = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<OutboundPaymentsPaymentMethodDetailsUsBankAccount> {
    ...
    override fun deserialize(decoder: Decoder): OutboundPaymentsPaymentMethodDetailsUsBankAccount {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("OutboundPaymentsPaymentMethodDetailsUsBankAccount must be a JSON object")
      val network = json.decodeRequired<InlineOutboundPaymentsPaymentMethodDetailsUsBankAccountNetworkXe07bc35c>(rawObject, "network")
      return OutboundPaymentsPaymentMethodDetailsUsBankAccount(
        network = network,
        accountHolderType = rawObject["account_holder_type"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineOutboundPaymentsPaym8585AccountHolderTypeXfbf2fd38?>(element) },
        accountType = rawObject["account_type"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineOutboundPaymentsPaymentMethodDetailsUsBankAccountAccountTypeX04590928?>(element) },
        ...
        mandate = rawObject["mandate"]?.let { json.decodeFromJsonElement<InlineOutboundPaymentsPaymentMethodDetailsUsBankAccountMandateX78d61086>(it) },
    ...
    override fun serialize(encoder: Encoder, `value`: OutboundPaymentsPaymentMethodDetailsUsBankAccount) {
...
public fun outboundPaymentsPaymentMethodDetailsUsBankAccount(block: OutboundPaymentsPaymentMethodDetailsUsBankAccount.Builder.() -> Unit): OutboundPaymentsPaymentMethodDetailsUsBankAccount = OutboundPaymentsPaymentMethodDetailsUsBankAccount.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("OutboundPaymentsPaymentMethodDetailsUsBankAccount is missing required property '" + name + "'")
