...
public class AccountSettings(
  ...
  internal object Serializer : KSerializer<AccountSettings> {
    ...
    override fun deserialize(decoder: Decoder): AccountSettings {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("AccountSettings must be a JSON object")
      ...
      return AccountSettings(
        ...
        dashboard = dashboard,
        payments = payments,
        bacsDebitPayments = rawObject["bacs_debit_payments"]?.let { json.decodeFromJsonElement<AccountBacsDebitPaymentsSettings>(it) },
        cardIssuing = rawObject["card_issuing"]?.let { json.decodeFromJsonElement<AccountCardIssuingSettings>(it) },
        ...
        sepaDebitPayments = rawObject["sepa_debit_payments"]?.let { json.decodeFromJsonElement<AccountSepaDebitPaymentsSettings>(it) },
        treasury = rawObject["treasury"]?.let { json.decodeFromJsonElement<AccountTreasurySettings>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: AccountSettings) {
...
private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("AccountSettings is missing required property '" + name + "'")
