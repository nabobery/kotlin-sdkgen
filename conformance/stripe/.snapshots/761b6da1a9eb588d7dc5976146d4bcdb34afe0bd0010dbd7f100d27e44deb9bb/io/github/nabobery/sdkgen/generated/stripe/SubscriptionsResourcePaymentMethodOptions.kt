...
public data class SubscriptionsResourcePaymentMethodOptionsView(
  ...
  public val usBankAccount:
      InlineSubscriptionsResourcePaymentMethodOptionsUsBankAccountXd7ad0d47? = null,
)
...
public class SubscriptionsResourcePaymentMethodOptions(
  ...
  public class Builder {
    ...
    public fun build(): SubscriptionsResourcePaymentMethodOptions = SubscriptionsResourcePaymentMethodOptions(
      acssDebit = acssDebit,
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): SubscriptionsResourcePaymentMethodOptions = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<SubscriptionsResourcePaymentMethodOptions> {
    ...
    override fun deserialize(decoder: Decoder): SubscriptionsResourcePaymentMethodOptions {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("SubscriptionsResourcePaymentMethodOptions must be a JSON object")
      return SubscriptionsResourcePaymentMethodOptions(
        acssDebit = rawObject["acss_debit"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSubscriptionsResourcePaymentMethodOptionsAcssDebitXee7bc235?>(element) },
        bancontact = rawObject["bancontact"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSubscriptionsResourcePaymentMethodOptionsBancontactX9391c49c?>(element) },
        card = rawObject["card"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSubscriptionsResourcePaymentMethodOptionsCardX188087bc?>(element) },
        customerBalance = rawObject["customer_balance"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSubscriptionsResourcePaymentMethodOptionsCustomerBalanceX313818de?>(element) },
        konbini = rawObject["konbini"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSubscriptionsResourcePaymentMethodOptionsKonbiniXd507e5d1?>(element) },
        payto = rawObject["payto"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSubscriptionsResourcePaymentMethodOptionsPaytoX542135c1?>(element) },
        pix = rawObject["pix"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSubscriptionsResourcePaymentMethodOptionsPixX520b5633?>(element) },
        sepaDebit = rawObject["sepa_debit"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSubscriptionsResourcePaymentMethodOptionsSepaDebitX715a7c0b?>(element) },
        upi = rawObject["upi"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSubscriptionsResourcePaymentMethodOptionsUpiX35e798ed?>(element) },
        usBankAccount = rawObject["us_bank_account"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSubscriptionsResourcePaymentMethodOptionsUsBankAccountXd7ad0d47?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: SubscriptionsResourcePaymentMethodOptions) {
...
public fun subscriptionsResourcePaymentMethodOptions(block: SubscriptionsResourcePaymentMethodOptions.Builder.() -> Unit): SubscriptionsResourcePaymentMethodOptions = SubscriptionsResourcePaymentMethodOptions.build(block)
