...
public class PaymentPagesCheckoutSessionBrandingSettings(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): PaymentPagesCheckoutSessionBrandingSettings = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<PaymentPagesCheckoutSessionBrandingSettings> {
    ...
    override fun deserialize(decoder: Decoder): PaymentPagesCheckoutSessionBrandingSettings {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentPagesCheckoutSessionBrandingSettings must be a JSON object")
      val backgroundColor = json.decodeRequired<String>(rawObject, "background_color")
      val borderStyle = json.decodeRequired<InlinePaymentPagesCheckoutSessionBrandingSettingsBorderStyleXb8fbbe66>(rawObject, "border_style")
      ...
      return PaymentPagesCheckoutSessionBrandingSettings(
        ...
        displayName = displayName,
        fontFamily = fontFamily,
        icon = rawObject["icon"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentPagesCheckoutSessionBrandingSettingsIconX69a9ee01?>(element) },
        logo = rawObject["logo"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentPagesCheckoutSessionBrandingSettingsLogoX7c08eac8?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PaymentPagesCheckoutSessionBrandingSettings) {
...
public fun paymentPagesCheckoutSessionBrandingSettings(block: PaymentPagesCheckoutSessionBrandingSettings.Builder.() -> Unit): PaymentPagesCheckoutSessionBrandingSettings = PaymentPagesCheckoutSessionBrandingSettings.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("PaymentPagesCheckoutSessionBrandingSettings is missing required property '" + name + "'")
