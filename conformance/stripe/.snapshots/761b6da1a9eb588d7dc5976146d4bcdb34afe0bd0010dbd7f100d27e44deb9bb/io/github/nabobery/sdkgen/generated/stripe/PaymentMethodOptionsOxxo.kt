...
public class PaymentMethodOptionsOxxo(
  ...
  internal object Serializer : KSerializer<PaymentMethodOptionsOxxo> {
    ...
    override fun deserialize(decoder: Decoder): PaymentMethodOptionsOxxo {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentMethodOptionsOxxo must be a JSON object")
      val expiresAfterDays = json.decodeRequired<Int>(rawObject, "expires_after_days")
      return PaymentMethodOptionsOxxo(
        expiresAfterDays = expiresAfterDays,
        setupFutureUsage = rawObject["setup_future_usage"]?.let { json.decodeFromJsonElement<InlinePaymentMethodOptionsOxxoSetupFutureUsageXda07f017>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PaymentMethodOptionsOxxo) {
...
public fun paymentMethodOptionsOxxo(block: PaymentMethodOptionsOxxo.Builder.() -> Unit): PaymentMethodOptionsOxxo = PaymentMethodOptionsOxxo.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("PaymentMethodOptionsOxxo is missing required property '" + name + "'")
