...
public class ClimateRemovalsOrderDeliveries(
  ...
  internal object Serializer : KSerializer<ClimateRemovalsOrderDeliveries> {
    ...
    override fun deserialize(decoder: Decoder): ClimateRemovalsOrderDeliveries {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("ClimateRemovalsOrderDeliveries must be a JSON object")
      ...
      return ClimateRemovalsOrderDeliveries(
        ...
        metricTons = metricTons,
        supplier = supplier,
        location = rawObject["location"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineClimateRemovalsOrderDeliveriesLocationX4d122060?>(element) },
        registryUrl = rawObject["registry_url"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: ClimateRemovalsOrderDeliveries) {
...
public fun climateRemovalsOrderDeliveries(block: ClimateRemovalsOrderDeliveries.Builder.() -> Unit): ClimateRemovalsOrderDeliveries = ClimateRemovalsOrderDeliveries.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("ClimateRemovalsOrderDeliveries is missing required property '" + name + "'")
