...
public class PortalFlowsFlowAfterCompletion(
  ...
  public val hostedConfirmation:
      InlinePortalFlowsFlowAfterCompletionHostedConfirmationXcd4435c3? = null,
  /**
  ...
  internal object Serializer : KSerializer<PortalFlowsFlowAfterCompletion> {
    ...
    override fun deserialize(decoder: Decoder): PortalFlowsFlowAfterCompletion {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PortalFlowsFlowAfterCompletion must be a JSON object")
      val type = json.decodeRequired<InlinePortalFlowsFlowAfterCompletionTypeX05eee8d4>(rawObject, "type")
      return PortalFlowsFlowAfterCompletion(
        type = type,
        hostedConfirmation = rawObject["hosted_confirmation"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePortalFlowsFlowAfterCompletionHostedConfirmationXcd4435c3?>(element) },
        redirect = rawObject["redirect"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePortalFlowsFlowAfterCompletionRedirectX0a4a54c4?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PortalFlowsFlowAfterCompletion) {
...
public fun portalFlowsFlowAfterCompletion(block: PortalFlowsFlowAfterCompletion.Builder.() -> Unit): PortalFlowsFlowAfterCompletion = PortalFlowsFlowAfterCompletion.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("PortalFlowsFlowAfterCompletion is missing required property '" + name + "'")
