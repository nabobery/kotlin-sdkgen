...
@Serializable
public data class IssuingAuthorizationView internal constructor(
...
public class IssuingAuthorization(
   ...
   */
  public val fleet: InlineIssuingAuthorizationFleetXd66cb701? = null,
  fraudChallenges: List<IssuingAuthorizationFraudChallenge>? = null,
  /**
   ...
   */
  public val fraudChallenges: List<IssuingAuthorizationFraudChallenge>? =
      fraudChallenges?.let { collection0 -> collection0.toList() }
  ...
  public class Builder {
    ...
    public var balanceTransactions: List<BalanceTransaction>
      get() = requireNotNull(balanceTransactionsValue) { "balanceTransactions is required" }.toList()
      set(`value`) {
        balanceTransactionsValue = value.toList()
      }
    ...
    public var requestHistory: List<IssuingAuthorizationRequest>
      get() = requireNotNull(requestHistoryValue) { "requestHistory is required" }.toList()
      set(`value`) {
        requestHistoryValue = value.toList()
      }
    ...
    public var transactions: List<IssuingTransaction>
      get() = requireNotNull(transactionsValue) { "transactions is required" }.toList()
      set(`value`) {
        transactionsValue = value.toList()
      }
    ...
    private var fraudChallengesValue: List<IssuingAuthorizationFraudChallenge>? = null

    /**
     * Fraud challenges sent to the cardholder, if this authorization was declined for fraud risk reasons.
     */
    public var fraudChallenges: List<IssuingAuthorizationFraudChallenge>?
      get() = fraudChallengesValue?.let { collection0 -> collection0.toList() }
      set(`value`) {
        fraudChallengesValue = value?.let { collection0 -> collection0.toList() }
      }
  ...
  internal object Serializer : KSerializer<IssuingAuthorization> {
    ...
    override fun deserialize(decoder: Decoder): IssuingAuthorization {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("IssuingAuthorization must be a JSON object")
      val amount = json.decodeRequired<Int>(rawObject, "amount")
      val approved = json.decodeRequired<Boolean>(rawObject, "approved")
      val authorizationMethod = json.decodeRequired<InlineIssuingAuthorizationAuthorizationMethodXac0c6140>(rawObject, "authorization_method")
      ...
      val verificationData = json.decodeRequired<IssuingAuthorizationVerificationData>(rawObject, "verification_data")
      return IssuingAuthorization(
        amount = amount,
        approved = approved,
        authorizationMethod = authorizationMethod,
        ...
        transactions = transactions,
        verificationData = verificationData,
        amountDetails = rawObject["amount_details"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingAuthorizationAmountDetailsX5b709ff0?>(element) },
        cardPresence = rawObject["card_presence"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingAuthorizationCardPresenceX536d5448?>(element) },
        cardholder = rawObject["cardholder"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingAuthorizationCardholderXf1408f02?>(element) },
        fleet = rawObject["fleet"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingAuthorizationFleetXd66cb701?>(element) },
        fraudChallenges = rawObject["fraud_challenges"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<List<IssuingAuthorizationFraudChallenge>?>(element) },
        fuel = rawObject["fuel"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingAuthorizationFuelX28f8e146?>(element) },
        networkData = rawObject["network_data"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingAuthorizationNetworkDataX1c44162b?>(element) },
        pendingRequest = rawObject["pending_request"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingAuthorizationPendingRequestXbed94e8a?>(element) },
        token = rawObject["token"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingAuthorizationTokenXc4287492?>(element) },
        treasury = rawObject["treasury"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingAuthorizationTreasuryX86b379b3?>(element) },
        verifiedByFraudChallenge = rawObject["verified_by_fraud_challenge"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Boolean?>(element) },
        wallet = rawObject["wallet"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: IssuingAuthorization) {
      ...
      val raw = buildJsonObject {
        ...
        value.verifiedByFraudChallenge?.let { put("verified_by_fraud_challenge", json.encodeToJsonElement(it)) }
...
public fun issuingAuthorization(block: IssuingAuthorization.Builder.() -> Unit): IssuingAuthorization = IssuingAuthorization.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("IssuingAuthorization is missing required property '" + name + "'")
