...
public class CheckoutSepaDebitPaymentMethodOptions(
  ...
  public class Builder {
    ...
    public fun build(): CheckoutSepaDebitPaymentMethodOptions = CheckoutSepaDebitPaymentMethodOptions(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): CheckoutSepaDebitPaymentMethodOptions = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<CheckoutSepaDebitPaymentMethodOptions> {
    ...
    override fun deserialize(decoder: Decoder): CheckoutSepaDebitPaymentMethodOptions {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("CheckoutSepaDebitPaymentMethodOptions must be a JSON object")
      return CheckoutSepaDebitPaymentMethodOptions(
        mandateOptions = rawObject["mandate_options"]?.let { json.decodeFromJsonElement<CheckoutPaymentMethodOptionsMandateOptionsSepaDebit>(it) },
        setupFutureUsage = rawObject["setup_future_usage"]?.let { json.decodeFromJsonElement<InlineCheckoutSepaDebitPaymentMethodOptionsSetupFutureUsageXf139fe7a>(it) },
        targetDate = rawObject["target_date"]?.let { json.decodeFromJsonElement<String>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: CheckoutSepaDebitPaymentMethodOptions) {
...
public fun checkoutSepaDebitPaymentMethodOptions(block: CheckoutSepaDebitPaymentMethodOptions.Builder.() -> Unit): CheckoutSepaDebitPaymentMethodOptions = CheckoutSepaDebitPaymentMethodOptions.build(block)
