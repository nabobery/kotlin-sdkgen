...
public class ConnectCollectionTransfer(
  ...
  internal object Serializer : KSerializer<ConnectCollectionTransfer> {
    ...
    override fun deserialize(decoder: Decoder): ConnectCollectionTransfer {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("ConnectCollectionTransfer must be a JSON object")
      val amount = json.decodeRequired<Int>(rawObject, "amount")
      val currency = json.decodeRequired<String>(rawObject, "currency")
      val destination = json.decodeRequired<InlineConnectCollectionTransferDestinationX80ef6bc4>(rawObject, "destination")
      val id = json.decodeRequired<String>(rawObject, "id")
      val livemode = json.decodeRequired<Boolean>(rawObject, "livemode")
      val objectValue = json.decodeRequired<InlineConnectCollectionTransferObjectValueXbcc36ecd>(rawObject, "object")
      return ConnectCollectionTransfer(
        amount = amount,
        currency = currency,
        destination = destination,
    ...
    override fun serialize(encoder: Encoder, `value`: ConnectCollectionTransfer) {
...
public fun connectCollectionTransfer(block: ConnectCollectionTransfer.Builder.() -> Unit): ConnectCollectionTransfer = ConnectCollectionTransfer.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("ConnectCollectionTransfer is missing required property '" + name + "'")
