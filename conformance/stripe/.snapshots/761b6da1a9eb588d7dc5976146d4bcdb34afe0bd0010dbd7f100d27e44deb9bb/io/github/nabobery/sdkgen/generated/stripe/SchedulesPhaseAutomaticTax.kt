...
public class SchedulesPhaseAutomaticTax(
  ...
  internal object Serializer : KSerializer<SchedulesPhaseAutomaticTax> {
    ...
    override fun deserialize(decoder: Decoder): SchedulesPhaseAutomaticTax {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("SchedulesPhaseAutomaticTax must be a JSON object")
      val enabled = json.decodeRequired<Boolean>(rawObject, "enabled")
      return SchedulesPhaseAutomaticTax(
        enabled = enabled,
        disabledReason = rawObject["disabled_reason"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSchedulesPhaseAutomaticTaxDisabledReasonX2a6b0e67?>(element) },
        liability = rawObject["liability"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSchedulesPhaseAutomaticTaxLiabilityX662c7f8f?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: SchedulesPhaseAutomaticTax) {
...
public fun schedulesPhaseAutomaticTax(block: SchedulesPhaseAutomaticTax.Builder.() -> Unit): SchedulesPhaseAutomaticTax = SchedulesPhaseAutomaticTax.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("SchedulesPhaseAutomaticTax is missing required property '" + name + "'")
