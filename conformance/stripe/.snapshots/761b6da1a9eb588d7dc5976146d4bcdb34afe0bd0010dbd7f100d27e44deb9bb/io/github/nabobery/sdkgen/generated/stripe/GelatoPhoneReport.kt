...
public class GelatoPhoneReport(
  ...
  internal object Serializer : KSerializer<GelatoPhoneReport> {
    ...
    override fun deserialize(decoder: Decoder): GelatoPhoneReport {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("GelatoPhoneReport must be a JSON object")
      val status = json.decodeRequired<InlineGelatoPhoneReportStatusX57a0ae0c>(rawObject, "status")
      return GelatoPhoneReport(
        status = status,
        error = rawObject["error"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineGelatoPhoneReportErrorX675018cc?>(element) },
        phone = rawObject["phone"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: GelatoPhoneReport) {
...
public fun gelatoPhoneReport(block: GelatoPhoneReport.Builder.() -> Unit): GelatoPhoneReport = GelatoPhoneReport.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("GelatoPhoneReport is missing required property '" + name + "'")
