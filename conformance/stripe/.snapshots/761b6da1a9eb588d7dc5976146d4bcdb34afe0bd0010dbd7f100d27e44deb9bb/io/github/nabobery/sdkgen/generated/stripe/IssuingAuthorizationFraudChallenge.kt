...
public class IssuingAuthorizationFraudChallenge(
   ...
   */
  public val undeliverableReason:
  ...
  internal object Serializer : KSerializer<IssuingAuthorizationFraudChallenge> {
    ...
    override fun deserialize(decoder: Decoder): IssuingAuthorizationFraudChallenge {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("IssuingAuthorizationFraudChallenge must be a JSON object")
      val channel = json.decodeRequired<InlineIssuingAuthorizationFraudChallengeChannelX05ae2a0f>(rawObject, "channel")
      val status = json.decodeRequired<InlineIssuingAuthorizationFraudChallengeStatusX729193b4>(rawObject, "status")
      return IssuingAuthorizationFraudChallenge(
        channel = channel,
        status = status,
        undeliverableReason = rawObject["undeliverable_reason"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingAuthorizationFraudChallengeUndeliverableReasonX63afb3ec?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: IssuingAuthorizationFraudChallenge) {
...
public fun issuingAuthorizationFraudChallenge(block: IssuingAuthorizationFraudChallenge.Builder.() -> Unit): IssuingAuthorizationFraudChallenge = IssuingAuthorizationFraudChallenge.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("IssuingAuthorizationFraudChallenge is missing required property '" + name + "'")
