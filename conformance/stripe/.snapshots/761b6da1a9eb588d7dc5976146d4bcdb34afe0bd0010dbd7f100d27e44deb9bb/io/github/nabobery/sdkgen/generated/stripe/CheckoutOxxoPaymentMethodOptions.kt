...
public class CheckoutOxxoPaymentMethodOptions(
  ...
  public val setupFutureUsage:
      InlineCheckoutOxxoPaymentMethodOptionsSetupFutureUsageXe47b88a0? = null,
) {
  ...
  internal object Serializer : KSerializer<CheckoutOxxoPaymentMethodOptions> {
    ...
    override fun deserialize(decoder: Decoder): CheckoutOxxoPaymentMethodOptions {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("CheckoutOxxoPaymentMethodOptions must be a JSON object")
      val expiresAfterDays = json.decodeRequired<Int>(rawObject, "expires_after_days")
      return CheckoutOxxoPaymentMethodOptions(
        expiresAfterDays = expiresAfterDays,
        setupFutureUsage = rawObject["setup_future_usage"]?.let { json.decodeFromJsonElement<InlineCheckoutOxxoPaymentMethodOptionsSetupFutureUsageXe47b88a0>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: CheckoutOxxoPaymentMethodOptions) {
...
public fun checkoutOxxoPaymentMethodOptions(block: CheckoutOxxoPaymentMethodOptions.Builder.() -> Unit): CheckoutOxxoPaymentMethodOptions = CheckoutOxxoPaymentMethodOptions.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("CheckoutOxxoPaymentMethodOptions is missing required property '" + name + "'")
