...
public class AutomaticTax(
  ...
  internal object Serializer : KSerializer<AutomaticTax> {
    ...
    override fun deserialize(decoder: Decoder): AutomaticTax {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("AutomaticTax must be a JSON object")
      val enabled = json.decodeRequired<Boolean>(rawObject, "enabled")
      return AutomaticTax(
        enabled = enabled,
        disabledReason = rawObject["disabled_reason"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineAutomaticTaxDisabledReasonXdaa0977e?>(element) },
        liability = rawObject["liability"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineAutomaticTaxLiabilityX958f05b7?>(element) },
        provider = rawObject["provider"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        status = rawObject["status"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineAutomaticTaxStatusX9f5176f7?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: AutomaticTax) {
