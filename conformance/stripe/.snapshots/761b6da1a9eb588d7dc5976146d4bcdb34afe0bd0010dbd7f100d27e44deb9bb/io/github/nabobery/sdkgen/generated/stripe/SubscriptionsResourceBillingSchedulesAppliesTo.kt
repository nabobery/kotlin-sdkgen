...
public class SubscriptionsResourceBillingSchedulesAppliesTo(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): SubscriptionsResourceBillingSchedulesAppliesTo = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<SubscriptionsResourceBillingSchedulesAppliesTo> {
    ...
    override fun deserialize(decoder: Decoder): SubscriptionsResourceBillingSchedulesAppliesTo {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("SubscriptionsResourceBillingSchedulesAppliesTo must be a JSON object")
      val type = json.decodeRequired<InlineSubscriptionsResourceBillingSchedulesAppliesToTypeX9085e4fb>(rawObject, "type")
      return SubscriptionsResourceBillingSchedulesAppliesTo(
        type = type,
        price = rawObject["price"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSubscriptionsResourceBillingSchedulesAppliesToPriceXafa9c902?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: SubscriptionsResourceBillingSchedulesAppliesTo) {
...
public fun subscriptionsResourceBillingSchedulesAppliesTo(block: SubscriptionsResourceBillingSchedulesAppliesTo.Builder.() -> Unit): SubscriptionsResourceBillingSchedulesAppliesTo = SubscriptionsResourceBillingSchedulesAppliesTo.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("SubscriptionsResourceBillingSchedulesAppliesTo is missing required property '" + name + "'")
