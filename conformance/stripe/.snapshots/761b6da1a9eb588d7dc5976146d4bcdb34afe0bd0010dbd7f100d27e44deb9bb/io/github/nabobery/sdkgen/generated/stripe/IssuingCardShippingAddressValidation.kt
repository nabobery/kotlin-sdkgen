...
public class IssuingCardShippingAddressValidation(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): IssuingCardShippingAddressValidation = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<IssuingCardShippingAddressValidation> {
    ...
    override fun deserialize(decoder: Decoder): IssuingCardShippingAddressValidation {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("IssuingCardShippingAddressValidation must be a JSON object")
      val mode = json.decodeRequired<InlineIssuingCardShippingAddressValidationModeXe5f152f0>(rawObject, "mode")
      return IssuingCardShippingAddressValidation(
        mode = mode,
        normalizedAddress = rawObject["normalized_address"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingCardShippingAddressValidationNormalizedAddressX6599860a?>(element) },
        result = rawObject["result"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingCardShippingAddressValidationResultXfbdb4a26?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: IssuingCardShippingAddressValidation) {
...
public fun issuingCardShippingAddressValidation(block: IssuingCardShippingAddressValidation.Builder.() -> Unit): IssuingCardShippingAddressValidation = IssuingCardShippingAddressValidation.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("IssuingCardShippingAddressValidation is missing required property '" + name + "'")
