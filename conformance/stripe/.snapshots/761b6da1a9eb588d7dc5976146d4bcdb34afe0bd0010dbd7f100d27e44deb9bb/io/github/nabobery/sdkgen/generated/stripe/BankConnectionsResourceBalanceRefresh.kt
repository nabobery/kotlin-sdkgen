...
public class BankConnectionsResourceBalanceRefresh(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): BankConnectionsResourceBalanceRefresh = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<BankConnectionsResourceBalanceRefresh> {
    ...
    override fun deserialize(decoder: Decoder): BankConnectionsResourceBalanceRefresh {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("BankConnectionsResourceBalanceRefresh must be a JSON object")
      val lastAttemptedAt = json.decodeRequired<Int>(rawObject, "last_attempted_at")
      val status = json.decodeRequired<InlineBankConnectionsResourceBalanceRefreshStatusX70dc5746>(rawObject, "status")
      return BankConnectionsResourceBalanceRefresh(
        lastAttemptedAt = lastAttemptedAt,
        status = status,
    ...
    override fun serialize(encoder: Encoder, `value`: BankConnectionsResourceBalanceRefresh) {
...
public fun bankConnectionsResourceBalanceRefresh(block: BankConnectionsResourceBalanceRefresh.Builder.() -> Unit): BankConnectionsResourceBalanceRefresh = BankConnectionsResourceBalanceRefresh.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("BankConnectionsResourceBalanceRefresh is missing required property '" + name + "'")
