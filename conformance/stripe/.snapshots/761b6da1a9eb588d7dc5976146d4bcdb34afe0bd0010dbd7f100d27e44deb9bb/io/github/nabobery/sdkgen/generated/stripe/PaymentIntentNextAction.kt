...
public data class PaymentIntentNextActionView(
  ...
  public val displayBankTransferInstructions:
  ...
  public val swishHandleRedirectOrDisplayQrCode:
  ...
  public val wechatPayRedirectToAndroidApp:
      PaymentIntentNextActionWechatPayRedirectToAndroidApp? = null,
...
public class PaymentIntentNextAction(
  ...
  internal object Serializer : KSerializer<PaymentIntentNextAction> {
    ...
    override fun deserialize(decoder: Decoder): PaymentIntentNextAction {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentIntentNextAction must be a JSON object")
      val type = json.decodeRequired<String>(rawObject, "type")
      return PaymentIntentNextAction(
        type = type,
        alipayHandleRedirect = rawObject["alipay_handle_redirect"]?.let { json.decodeFromJsonElement<PaymentIntentNextActionAlipayHandleRedirect>(it) },
        blikAuthorize = rawObject["blik_authorize"]?.let { json.decodeFromJsonElement<JsonObject>(it) },
        boletoDisplayDetails = rawObject["boleto_display_details"]?.let { json.decodeFromJsonElement<PaymentIntentNextActionBoleto>(it) },
        cardAwaitNotification = rawObject["card_await_notification"]?.let { json.decodeFromJsonElement<PaymentIntentNextActionCardAwaitNotification>(it) },
        ...
        displayBankTransferInstructions = rawObject["display_bank_transfer_instructions"]?.let { json.decodeFromJsonElement<PaymentIntentNextActionDisplayBankTransferInstructions>(it) },
        klarnaDisplayQrCode = rawObject["klarna_display_qr_code"]?.let { json.decodeFromJsonElement<PaymentIntentNextActionKlarnaDisplayQrCode>(it) },
        konbiniDisplayDetails = rawObject["konbini_display_details"]?.let { json.decodeFromJsonElement<PaymentIntentNextActionKonbini>(it) },
        multibancoDisplayDetails = rawObject["multibanco_display_details"]?.let { json.decodeFromJsonElement<PaymentIntentNextActionDisplayMultibancoDetails>(it) },
        oxxoDisplayDetails = rawObject["oxxo_display_details"]?.let { json.decodeFromJsonElement<PaymentIntentNextActionDisplayOxxoDetails>(it) },
        paynowDisplayQrCode = rawObject["paynow_display_qr_code"]?.let { json.decodeFromJsonElement<PaymentIntentNextActionPaynowDisplayQrCode>(it) },
        pixDisplayQrCode = rawObject["pix_display_qr_code"]?.let { json.decodeFromJsonElement<PaymentIntentNextActionPixDisplayQrCode>(it) },
        promptpayDisplayQrCode = rawObject["promptpay_display_qr_code"]?.let { json.decodeFromJsonElement<PaymentIntentNextActionPromptpayDisplayQrCode>(it) },
        redirectToUrl = rawObject["redirect_to_url"]?.let { json.decodeFromJsonElement<PaymentIntentNextActionRedirectToUrl>(it) },
        swishHandleRedirectOrDisplayQrCode = rawObject["swish_handle_redirect_or_display_qr_code"]?.let { json.decodeFromJsonElement<PaymentIntentNextActionSwishHandleRedirectOrDisplayQrCode>(it) },
        ...
        verifyWithMicrodeposits = rawObject["verify_with_microdeposits"]?.let { json.decodeFromJsonElement<PaymentIntentNextActionVerifyWithMicrodeposits>(it) },
        wechatPayDisplayQrCode = rawObject["wechat_pay_display_qr_code"]?.let { json.decodeFromJsonElement<PaymentIntentNextActionWechatPayDisplayQrCode>(it) },
        wechatPayRedirectToAndroidApp = rawObject["wechat_pay_redirect_to_android_app"]?.let { json.decodeFromJsonElement<PaymentIntentNextActionWechatPayRedirectToAndroidApp>(it) },
        wechatPayRedirectToIosApp = rawObject["wechat_pay_redirect_to_ios_app"]?.let { json.decodeFromJsonElement<PaymentIntentNextActionWechatPayRedirectToIosApp>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PaymentIntentNextAction) {
      ...
      val raw = buildJsonObject {
        ...
        value.displayBankTransferInstructions?.let { put("display_bank_transfer_instructions", json.encodeToJsonElement(it)) }
        ...
        value.multibancoDisplayDetails?.let { put("multibanco_display_details", json.encodeToJsonElement(it)) }
        ...
        value.swishHandleRedirectOrDisplayQrCode?.let { put("swish_handle_redirect_or_display_qr_code", json.encodeToJsonElement(it)) }
        ...
        value.wechatPayDisplayQrCode?.let { put("wechat_pay_display_qr_code", json.encodeToJsonElement(it)) }
        value.wechatPayRedirectToAndroidApp?.let { put("wechat_pay_redirect_to_android_app", json.encodeToJsonElement(it)) }
        value.wechatPayRedirectToIosApp?.let { put("wechat_pay_redirect_to_ios_app", json.encodeToJsonElement(it)) }
      }
...
public fun paymentIntentNextAction(block: PaymentIntentNextAction.Builder.() -> Unit): PaymentIntentNextAction = PaymentIntentNextAction.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("PaymentIntentNextAction is missing required property '" + name + "'")
