...
public class PaymentIntentPaymentMethodOptionsUsBankAccount(
  ...
  public class Builder {
    ...
    public fun build(): PaymentIntentPaymentMethodOptionsUsBankAccount = PaymentIntentPaymentMethodOptionsUsBankAccount(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): PaymentIntentPaymentMethodOptionsUsBankAccount = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<PaymentIntentPaymentMethodOptionsUsBankAccount> {
    ...
    override fun deserialize(decoder: Decoder): PaymentIntentPaymentMethodOptionsUsBankAccount {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentIntentPaymentMethodOptionsUsBankAccount must be a JSON object")
      return PaymentIntentPaymentMethodOptionsUsBankAccount(
        ...
        setupFutureUsage = rawObject["setup_future_usage"]?.let { json.decodeFromJsonElement<InlinePaymentIntentPaymentMethodOptionsUsBankAccountSetupFutureUsageX0a863d58>(it) },
        targetDate = rawObject["target_date"]?.let { json.decodeFromJsonElement<String>(it) },
        transactionPurpose = rawObject["transaction_purpose"]?.let { json.decodeFromJsonElement<InlinePaymentIntentPaymentMethodOptionsUsBankAccountTransactionPurposeX0ccb14eb>(it) },
        verificationMethod = rawObject["verification_method"]?.let { json.decodeFromJsonElement<InlinePaymentIntentPaymentMethodOptionsUsBankAccountVerificationMethodX3195fcd7>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PaymentIntentPaymentMethodOptionsUsBankAccount) {
...
public fun paymentIntentPaymentMethodOptionsUsBankAccount(block: PaymentIntentPaymentMethodOptionsUsBankAccount.Builder.() -> Unit): PaymentIntentPaymentMethodOptionsUsBankAccount = PaymentIntentPaymentMethodOptionsUsBankAccount.build(block)
