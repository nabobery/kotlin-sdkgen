...
public class TreasuryFinancialAccountFeatures(
  ...
  public val financialAddresses:
      TreasuryFinancialAccountsResourceFinancialAddressesFeatures? = null,
  ...
  internal object Serializer : KSerializer<TreasuryFinancialAccountFeatures> {
    ...
    override fun deserialize(decoder: Decoder): TreasuryFinancialAccountFeatures {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("TreasuryFinancialAccountFeatures must be a JSON object")
      val objectValue = json.decodeRequired<InlineTreasuryFinancialAccountFeaturesObjectValueX72f91fa7>(rawObject, "object")
      return TreasuryFinancialAccountFeatures(
        objectValue = objectValue,
        cardIssuing = rawObject["card_issuing"]?.let { json.decodeFromJsonElement<TreasuryFinancialAccountsResourceToggleSettings>(it) },
        depositInsurance = rawObject["deposit_insurance"]?.let { json.decodeFromJsonElement<TreasuryFinancialAccountsResourceToggleSettings>(it) },
        financialAddresses = rawObject["financial_addresses"]?.let { json.decodeFromJsonElement<TreasuryFinancialAccountsResourceFinancialAddressesFeatures>(it) },
        inboundTransfers = rawObject["inbound_transfers"]?.let { json.decodeFromJsonElement<TreasuryFinancialAccountsResourceInboundTransfers>(it) },
        intraStripeFlows = rawObject["intra_stripe_flows"]?.let { json.decodeFromJsonElement<TreasuryFinancialAccountsResourceToggleSettings>(it) },
        outboundPayments = rawObject["outbound_payments"]?.let { json.decodeFromJsonElement<TreasuryFinancialAccountsResourceOutboundPayments>(it) },
        outboundTransfers = rawObject["outbound_transfers"]?.let { json.decodeFromJsonElement<TreasuryFinancialAccountsResourceOutboundTransfers>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: TreasuryFinancialAccountFeatures) {
...
public fun treasuryFinancialAccountFeatures(block: TreasuryFinancialAccountFeatures.Builder.() -> Unit): TreasuryFinancialAccountFeatures = TreasuryFinancialAccountFeatures.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("TreasuryFinancialAccountFeatures is missing required property '" + name + "'")
