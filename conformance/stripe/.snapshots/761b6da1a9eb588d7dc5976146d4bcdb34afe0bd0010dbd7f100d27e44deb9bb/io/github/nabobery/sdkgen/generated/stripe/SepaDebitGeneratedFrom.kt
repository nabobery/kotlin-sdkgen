...
public class SepaDebitGeneratedFrom(
  ...
  public class Builder {
    ...
    public fun build(): SepaDebitGeneratedFrom = SepaDebitGeneratedFrom(
      charge = charge,
  ...
  internal object Serializer : KSerializer<SepaDebitGeneratedFrom> {
    ...
    override fun deserialize(decoder: Decoder): SepaDebitGeneratedFrom {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("SepaDebitGeneratedFrom must be a JSON object")
      return SepaDebitGeneratedFrom(
        charge = rawObject["charge"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSepaDebitGeneratedFromChargeX65c261e2?>(element) },
        setupAttempt = rawObject["setup_attempt"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSepaDebitGeneratedFromSetupAttemptX62691f64?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: SepaDebitGeneratedFrom) {
...
public fun sepaDebitGeneratedFrom(block: SepaDebitGeneratedFrom.Builder.() -> Unit): SepaDebitGeneratedFrom = SepaDebitGeneratedFrom.build(block)
