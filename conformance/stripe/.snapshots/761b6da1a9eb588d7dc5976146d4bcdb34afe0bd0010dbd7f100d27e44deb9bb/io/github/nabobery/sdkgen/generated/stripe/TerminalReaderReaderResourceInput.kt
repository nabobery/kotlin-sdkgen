...
public class TerminalReaderReaderResourceInput(
  ...
  toggles: List<TerminalReaderReaderResourceToggle>? = null,
) {
  /**
   * List of toggles being collected. Values are present if collection is complete.
   */
  public val toggles: List<TerminalReaderReaderResourceToggle>? =
  ...
  public class Builder {
    ...
    private var togglesValue: List<TerminalReaderReaderResourceToggle>? = null

    /**
     * List of toggles being collected. Values are present if collection is complete.
     */
    public var toggles: List<TerminalReaderReaderResourceToggle>?
  ...
  internal object Serializer : KSerializer<TerminalReaderReaderResourceInput> {
    ...
    override fun deserialize(decoder: Decoder): TerminalReaderReaderResourceInput {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("TerminalReaderReaderResourceInput must be a JSON object")
      val type = json.decodeRequired<InlineTerminalReaderReaderResourceInputTypeXc23bd85e>(rawObject, "type")
      return TerminalReaderReaderResourceInput(
        type = type,
        customText = rawObject["custom_text"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineTerminalReaderReaderResourceInputCustomTextXcaef2151?>(element) },
        email = rawObject["email"]?.let { json.decodeFromJsonElement<TerminalReaderReaderResourceEmail>(it) },
        numeric = rawObject["numeric"]?.let { json.decodeFromJsonElement<TerminalReaderReaderResourceNumeric>(it) },
        phone = rawObject["phone"]?.let { json.decodeFromJsonElement<TerminalReaderReaderResourcePhone>(it) },
        required = rawObject["required"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Boolean?>(element) },
        selection = rawObject["selection"]?.let { json.decodeFromJsonElement<TerminalReaderReaderResourceSelection>(it) },
        signature = rawObject["signature"]?.let { json.decodeFromJsonElement<TerminalReaderReaderResourceSignature>(it) },
        ...
        toggles = rawObject["toggles"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<List<TerminalReaderReaderResourceToggle>?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: TerminalReaderReaderResourceInput) {
...
public fun terminalReaderReaderResourceInput(block: TerminalReaderReaderResourceInput.Builder.() -> Unit): TerminalReaderReaderResourceInput = TerminalReaderReaderResourceInput.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("TerminalReaderReaderResourceInput is missing required property '" + name + "'")
