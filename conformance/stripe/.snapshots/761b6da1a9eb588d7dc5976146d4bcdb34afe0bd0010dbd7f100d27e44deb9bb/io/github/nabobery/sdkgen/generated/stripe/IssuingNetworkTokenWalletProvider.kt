...
public class IssuingNetworkTokenWalletProvider(
   ...
   */
  public val cardNumberSource:
      InlineIssuingNetworkTokenWalletProviderCardNumberSourceX7ad5bf1c? = null,
  ...
  reasonCodes: List<InlineIssuingNetworkTokenWalletProviderReasonCodesItemXcb5712c5>? = null,
  /**
   * The recommendation on responding to the tokenization request.
   */
  public val suggestedDecision:
      InlineIssuingNetworkTokenWalletProviderSuggestedDecisionX00371348? = null,
  /**
   ...
   */
  public val reasonCodes: List<InlineIssuingNetworkTokenWalletProviderReasonCodesItemXcb5712c5>? =
      reasonCodes?.let { collection0 -> collection0.toList() }
  ...
  public class Builder {
    ...
    private var reasonCodesValue:
        List<InlineIssuingNetworkTokenWalletProviderReasonCodesItemXcb5712c5>? = null

    /**
     * The reasons for suggested tokenization given by the card network.
     */
    public var reasonCodes: List<InlineIssuingNetworkTokenWalletProviderReasonCodesItemXcb5712c5>?
      get() = reasonCodesValue?.let { collection0 -> collection0.toList() }
      set(`value`) {
        reasonCodesValue = value?.let { collection0 -> collection0.toList() }
      }
    ...
    public fun build(): IssuingNetworkTokenWalletProvider = IssuingNetworkTokenWalletProvider(
      accountId = accountId,
  ...
  internal object Serializer : KSerializer<IssuingNetworkTokenWalletProvider> {
    ...
    override fun deserialize(decoder: Decoder): IssuingNetworkTokenWalletProvider {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("IssuingNetworkTokenWalletProvider must be a JSON object")
      return IssuingNetworkTokenWalletProvider(
        ...
        cardNumberSource = rawObject["card_number_source"]?.let { json.decodeFromJsonElement<InlineIssuingNetworkTokenWalletProviderCardNumberSourceX7ad5bf1c>(it) },
        cardholderAddress = rawObject["cardholder_address"]?.let { json.decodeFromJsonElement<IssuingNetworkTokenAddress>(it) },
        ...
        hashedAccountEmailAddress = rawObject["hashed_account_email_address"]?.let { json.decodeFromJsonElement<String>(it) },
        reasonCodes = rawObject["reason_codes"]?.let { json.decodeFromJsonElement<List<InlineIssuingNetworkTokenWalletProviderReasonCodesItemXcb5712c5>>(it) },
        suggestedDecision = rawObject["suggested_decision"]?.let { json.decodeFromJsonElement<InlineIssuingNetworkTokenWalletProviderSuggestedDecisionX00371348>(it) },
        suggestedDecisionVersion = rawObject["suggested_decision_version"]?.let { json.decodeFromJsonElement<String>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: IssuingNetworkTokenWalletProvider) {
...
public fun issuingNetworkTokenWalletProvider(block: IssuingNetworkTokenWalletProvider.Builder.() -> Unit): IssuingNetworkTokenWalletProvider = IssuingNetworkTokenWalletProvider.build(block)
