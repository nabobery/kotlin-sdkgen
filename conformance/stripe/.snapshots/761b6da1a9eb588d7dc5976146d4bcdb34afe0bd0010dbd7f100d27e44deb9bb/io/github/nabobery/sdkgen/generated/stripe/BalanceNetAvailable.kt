...
public class BalanceNetAvailable(
  ...
  internal object Serializer : KSerializer<BalanceNetAvailable> {
    ...
    override fun deserialize(decoder: Decoder): BalanceNetAvailable {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("BalanceNetAvailable must be a JSON object")
    ...
    override fun serialize(encoder: Encoder, `value`: BalanceNetAvailable) {
...
public fun balanceNetAvailable(block: BalanceNetAvailable.Builder.() -> Unit): BalanceNetAvailable = BalanceNetAvailable.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("BalanceNetAvailable is missing required property '" + name + "'")
