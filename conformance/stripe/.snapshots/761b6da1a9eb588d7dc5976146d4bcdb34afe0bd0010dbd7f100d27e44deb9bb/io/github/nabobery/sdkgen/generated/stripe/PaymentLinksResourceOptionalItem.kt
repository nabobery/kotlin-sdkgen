...
public class PaymentLinksResourceOptionalItem(
  ...
  internal object Serializer : KSerializer<PaymentLinksResourceOptionalItem> {
    ...
    override fun deserialize(decoder: Decoder): PaymentLinksResourceOptionalItem {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentLinksResourceOptionalItem must be a JSON object")
      val price = json.decodeRequired<String>(rawObject, "price")
      val quantity = json.decodeRequired<Int>(rawObject, "quantity")
      return PaymentLinksResourceOptionalItem(
        price = price,
        quantity = quantity,
        adjustableQuantity = rawObject["adjustable_quantity"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentLinksResourceOptionalItemAdjustableQuantityXe9d6e1a3?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PaymentLinksResourceOptionalItem) {
...
public fun paymentLinksResourceOptionalItem(block: PaymentLinksResourceOptionalItem.Builder.() -> Unit): PaymentLinksResourceOptionalItem = PaymentLinksResourceOptionalItem.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("PaymentLinksResourceOptionalItem is missing required property '" + name + "'")
