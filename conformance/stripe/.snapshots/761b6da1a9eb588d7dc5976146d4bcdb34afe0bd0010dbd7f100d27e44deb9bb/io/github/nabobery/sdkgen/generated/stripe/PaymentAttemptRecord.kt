...
public class PaymentAttemptRecord(
  ...
  internal object Serializer : KSerializer<PaymentAttemptRecord> {
    ...
    override fun deserialize(decoder: Decoder): PaymentAttemptRecord {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentAttemptRecord must be a JSON object")
      ...
      val reportedBy = json.decodeRequired<InlinePaymentAttemptRecordReportedByXb8d2c634>(rawObject, "reported_by")
      return PaymentAttemptRecord(
        amount = amount,
        amountAuthorized = amountAuthorized,
        amountCanceled = amountCanceled,
        ...
        customerDetails = rawObject["customer_details"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentAttemptRecordCustomerDetailsXdfa20698?>(element) },
        customerPresence = rawObject["customer_presence"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentAttemptRecordCustomerPresenceX8fdd03e0?>(element) },
        description = rawObject["description"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        paymentMethodDetails = rawObject["payment_method_details"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentAttemptRecordPaymentMethodDetailsXcf3260e3?>(element) },
        paymentRecord = rawObject["payment_record"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        shippingDetails = rawObject["shipping_details"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentAttemptRecordShippingDetailsX2365fd11?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PaymentAttemptRecord) {
...
public fun paymentAttemptRecord(block: PaymentAttemptRecord.Builder.() -> Unit): PaymentAttemptRecord = PaymentAttemptRecord.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("PaymentAttemptRecord is missing required property '" + name + "'")
