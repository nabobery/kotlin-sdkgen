...
public class SetupIntentPaymentMethodOptionsAcssDebit(
  ...
  public class Builder {
    ...
    public fun build(): SetupIntentPaymentMethodOptionsAcssDebit = SetupIntentPaymentMethodOptionsAcssDebit(
      currency = currency,
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): SetupIntentPaymentMethodOptionsAcssDebit = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<SetupIntentPaymentMethodOptionsAcssDebit> {
    ...
    override fun deserialize(decoder: Decoder): SetupIntentPaymentMethodOptionsAcssDebit {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("SetupIntentPaymentMethodOptionsAcssDebit must be a JSON object")
      return SetupIntentPaymentMethodOptionsAcssDebit(
        currency = rawObject["currency"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSetupIntentPaymentMethodOptionsAcssDebitCurrencyX5103e97c?>(element) },
        mandateOptions = rawObject["mandate_options"]?.let { json.decodeFromJsonElement<SetupIntentPaymentMethodOptionsMandateOptionsAcssDebit>(it) },
        verificationMethod = rawObject["verification_method"]?.let { json.decodeFromJsonElement<InlineSetupIntentPaymentMethodOptionsAcssDebitVerificationMethodXfec0c5e7>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: SetupIntentPaymentMethodOptionsAcssDebit) {
...
public fun setupIntentPaymentMethodOptionsAcssDebit(block: SetupIntentPaymentMethodOptionsAcssDebit.Builder.() -> Unit): SetupIntentPaymentMethodOptionsAcssDebit = SetupIntentPaymentMethodOptionsAcssDebit.build(block)
