...
public class BillingCreditBalanceTransaction(
  ...
  internal object Serializer : KSerializer<BillingCreditBalanceTransaction> {
    ...
    override fun deserialize(decoder: Decoder): BillingCreditBalanceTransaction {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("BillingCreditBalanceTransaction must be a JSON object")
      val created = json.decodeRequired<Int>(rawObject, "created")
      val creditGrant = json.decodeRequired<InlineBillingCreditBalanceTransactionCreditGrantX6e0e6c5e>(rawObject, "credit_grant")
      val effectiveAt = json.decodeRequired<Int>(rawObject, "effective_at")
      val id = json.decodeRequired<String>(rawObject, "id")
      val livemode = json.decodeRequired<Boolean>(rawObject, "livemode")
      val objectValue = json.decodeRequired<InlineBillingCreditBalanceTransactionObjectValueX33e7fb04>(rawObject, "object")
      return BillingCreditBalanceTransaction(
        created = created,
        creditGrant = creditGrant,
        effectiveAt = effectiveAt,
        ...
        livemode = livemode,
        objectValue = objectValue,
        credit = rawObject["credit"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineBillingCreditBalanceTransactionCreditXc0e9748b?>(element) },
        debit = rawObject["debit"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineBillingCreditBalanceTransactionDebitX74175306?>(element) },
        testClock = rawObject["test_clock"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineBillingCreditBalanceTransactionTestClockX0ed6ef06?>(element) },
        type = rawObject["type"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineBillingCreditBalanceTransactionTypeX43796971?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: BillingCreditBalanceTransaction) {
...
public fun billingCreditBalanceTransaction(block: BillingCreditBalanceTransaction.Builder.() -> Unit): BillingCreditBalanceTransaction = BillingCreditBalanceTransaction.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("BillingCreditBalanceTransaction is missing required property '" + name + "'")
