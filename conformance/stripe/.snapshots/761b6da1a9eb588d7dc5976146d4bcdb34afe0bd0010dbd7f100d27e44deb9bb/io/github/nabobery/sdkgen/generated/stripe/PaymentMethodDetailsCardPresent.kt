...
public class PaymentMethodDetailsCardPresent(
  ...
  public class Builder {
    ...
    public var incrementalAuthorizationSupported: Boolean
      get() = requireNotNull(incrementalAuthorizationSupportedValue) { "incrementalAuthorizationSupported is required" }
      set(`value`) {
  ...
  internal object Serializer : KSerializer<PaymentMethodDetailsCardPresent> {
    ...
    override fun deserialize(decoder: Decoder): PaymentMethodDetailsCardPresent {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentMethodDetailsCardPresent must be a JSON object")
      val expMonth = json.decodeRequired<Int>(rawObject, "exp_month")
      val expYear = json.decodeRequired<Int>(rawObject, "exp_year")
      val incrementalAuthorizationSupported = json.decodeRequired<Boolean>(rawObject, "incremental_authorization_supported")
      ...
      return PaymentMethodDetailsCardPresent(
        ...
        offline = rawObject["offline"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentMethodDetailsCardPresentOfflineX2bc680b2?>(element) },
        ...
        readMethod = rawObject["read_method"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentMethodDetailsCardPresentReadMethodX713d2924?>(element) },
        reader = rawObject["reader"]?.let { json.decodeFromJsonElement<String>(it) },
        receipt = rawObject["receipt"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentMethodDetailsCardPresentReceiptXa62bb6ea?>(element) },
        wallet = rawObject["wallet"]?.let { json.decodeFromJsonElement<PaymentFlowsPrivatePaymentMethodsCardPresentCommonWallet>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PaymentMethodDetailsCardPresent) {
      ...
      val raw = buildJsonObject {
        ...
        put("incremental_authorization_supported", json.encodeToJsonElement(value.incrementalAuthorizationSupported))
...
public fun paymentMethodDetailsCardPresent(block: PaymentMethodDetailsCardPresent.Builder.() -> Unit): PaymentMethodDetailsCardPresent = PaymentMethodDetailsCardPresent.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("PaymentMethodDetailsCardPresent is missing required property '" + name + "'")
