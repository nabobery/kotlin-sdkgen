...
public class DisputeEvidence(
   ...
   */
  public val duplicateChargeDocumentation:
  ...
  internal object Serializer : KSerializer<DisputeEvidence> {
    ...
    override fun deserialize(decoder: Decoder): DisputeEvidence {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("DisputeEvidence must be a JSON object")
      val enhancedEvidence = json.decodeRequired<DisputeEnhancedEvidence>(rawObject, "enhanced_evidence")
      return DisputeEvidence(
        enhancedEvidence = enhancedEvidence,
        accessActivityLog = rawObject["access_activity_log"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        billingAddress = rawObject["billing_address"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        cancellationPolicy = rawObject["cancellation_policy"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineDisputeEvidenceCancellationPolicyXcaf1ed70?>(element) },
        cancellationPolicyDisclosure = rawObject["cancellation_policy_disclosure"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        cancellationRebuttal = rawObject["cancellation_rebuttal"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        customerCommunication = rawObject["customer_communication"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineDisputeEvidenceCustomerCommunicationXdea508c4?>(element) },
        ...
        customerSignature = rawObject["customer_signature"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineDisputeEvidenceCustomerSignatureX7f79c66a?>(element) },
        duplicateChargeDocumentation = rawObject["duplicate_charge_documentation"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineDisputeEvidenceDuplicateChargeDocumentationXa3255848?>(element) },
        duplicateChargeExplanation = rawObject["duplicate_charge_explanation"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        duplicateChargeId = rawObject["duplicate_charge_id"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        ...
        receipt = rawObject["receipt"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineDisputeEvidenceReceiptXefa00443?>(element) },
        refundPolicy = rawObject["refund_policy"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineDisputeEvidenceRefundPolicyX42bbd64a?>(element) },
        refundPolicyDisclosure = rawObject["refund_policy_disclosure"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        refundRefusalExplanation = rawObject["refund_refusal_explanation"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        serviceDate = rawObject["service_date"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        serviceDocumentation = rawObject["service_documentation"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineDisputeEvidenceServiceDocumentationXf2c81eae?>(element) },
        shippingAddress = rawObject["shipping_address"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        shippingCarrier = rawObject["shipping_carrier"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        shippingDate = rawObject["shipping_date"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        shippingDocumentation = rawObject["shipping_documentation"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineDisputeEvidenceShippingDocumentationX5192d247?>(element) },
        shippingTrackingNumber = rawObject["shipping_tracking_number"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        uncategorizedFile = rawObject["uncategorized_file"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineDisputeEvidenceUncategorizedFileXd3187fc3?>(element) },
        uncategorizedText = rawObject["uncategorized_text"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: DisputeEvidence) {
      ...
      val raw = buildJsonObject {
        ...
        value.duplicateChargeDocumentation?.let { put("duplicate_charge_documentation", json.encodeToJsonElement(it)) }
...
private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("DisputeEvidence is missing required property '" + name + "'")
