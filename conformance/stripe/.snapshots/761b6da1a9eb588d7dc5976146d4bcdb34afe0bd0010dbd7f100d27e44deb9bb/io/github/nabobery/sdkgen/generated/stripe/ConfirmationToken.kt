...
public class ConfirmationToken(
  ...
  internal object Serializer : KSerializer<ConfirmationToken> {
    ...
    override fun deserialize(decoder: Decoder): ConfirmationToken {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("ConfirmationToken must be a JSON object")
      ...
      return ConfirmationToken(
        ...
        mandateData = rawObject["mandate_data"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineConfirmationTokenMandateDataXfd4d495c?>(element) },
        paymentIntent = rawObject["payment_intent"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        paymentMethodOptions = rawObject["payment_method_options"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineConfirmationTokenPaymentMethodOptionsX0b4c12e3?>(element) },
        paymentMethodPreview = rawObject["payment_method_preview"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineConfirmationTokenPaymentMethodPreviewX06c42571?>(element) },
        returnUrl = rawObject["return_url"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        setupFutureUsage = rawObject["setup_future_usage"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineConfirmationTokenSetupFutureUsageX759e7354?>(element) },
        setupIntent = rawObject["setup_intent"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        shipping = rawObject["shipping"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineConfirmationTokenShippingX2977e808?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: ConfirmationToken) {
...
public fun confirmationToken(block: ConfirmationToken.Builder.() -> Unit): ConfirmationToken = ConfirmationToken.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("ConfirmationToken is missing required property '" + name + "'")
