...
public class PaymentPagesCheckoutSessionInvoiceSettings(
  accountTaxIds: List<InlinePaymentPagesCheckoutSessionInvoiceSettingsAccountTaxIdsItemXffd808e3>? = null,
  ...
  public val accountTaxIds:
      List<InlinePaymentPagesCheckoutSessionInvoiceSettingsAccountTaxIdsItemXffd808e3>? =
  ...
  public class Builder {
    ...
    public fun build(): PaymentPagesCheckoutSessionInvoiceSettings = PaymentPagesCheckoutSessionInvoiceSettings(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): PaymentPagesCheckoutSessionInvoiceSettings = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<PaymentPagesCheckoutSessionInvoiceSettings> {
    ...
    override fun deserialize(decoder: Decoder): PaymentPagesCheckoutSessionInvoiceSettings {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentPagesCheckoutSessionInvoiceSettings must be a JSON object")
      return PaymentPagesCheckoutSessionInvoiceSettings(
        accountTaxIds = rawObject["account_tax_ids"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<List<InlinePaymentPagesCheckoutSessionInvoiceSettingsAccountTaxIdsItemXffd808e3>?>(element) },
        ...
        issuer = rawObject["issuer"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentPagesCheckoutSessionInvoiceSettingsIssuerX84747291?>(element) },
        ...
        renderingOptions = rawObject["rendering_options"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentPagesCheckoutSessionInvoiceSettingsRenderingOptionsX9e0cf768?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PaymentPagesCheckoutSessionInvoiceSettings) {
...
public fun paymentPagesCheckoutSessionInvoiceSettings(block: PaymentPagesCheckoutSessionInvoiceSettings.Builder.() -> Unit): PaymentPagesCheckoutSessionInvoiceSettings = PaymentPagesCheckoutSessionInvoiceSettings.build(block)
