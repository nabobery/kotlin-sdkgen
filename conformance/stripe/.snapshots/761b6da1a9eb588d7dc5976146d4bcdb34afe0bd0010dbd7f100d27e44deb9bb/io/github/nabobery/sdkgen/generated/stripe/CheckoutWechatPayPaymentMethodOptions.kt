...
public class CheckoutWechatPayPaymentMethodOptions(
  ...
  public class Builder {
    ...
    public fun build(): CheckoutWechatPayPaymentMethodOptions = CheckoutWechatPayPaymentMethodOptions(
      appId = appId,
      client = client,
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): CheckoutWechatPayPaymentMethodOptions = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<CheckoutWechatPayPaymentMethodOptions> {
    ...
    override fun deserialize(decoder: Decoder): CheckoutWechatPayPaymentMethodOptions {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("CheckoutWechatPayPaymentMethodOptions must be a JSON object")
      return CheckoutWechatPayPaymentMethodOptions(
        appId = rawObject["app_id"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        client = rawObject["client"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineCheckoutWechatPayPaymentMethodOptionsClientXfa931533?>(element) },
        setupFutureUsage = rawObject["setup_future_usage"]?.let { json.decodeFromJsonElement<InlineCheckoutWechatPayPaymentMethodOptionsSetupFutureUsageX92769777>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: CheckoutWechatPayPaymentMethodOptions) {
...
public fun checkoutWechatPayPaymentMethodOptions(block: CheckoutWechatPayPaymentMethodOptions.Builder.() -> Unit): CheckoutWechatPayPaymentMethodOptions = CheckoutWechatPayPaymentMethodOptions.build(block)
