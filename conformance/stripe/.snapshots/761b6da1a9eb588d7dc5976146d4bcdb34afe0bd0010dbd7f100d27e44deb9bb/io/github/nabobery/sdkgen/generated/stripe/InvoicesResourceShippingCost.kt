...
@Serializable
public data class InvoicesResourceShippingCostView internal constructor(
...
public class InvoicesResourceShippingCost(
  ...
  internal object Serializer : KSerializer<InvoicesResourceShippingCost> {
    ...
    override fun deserialize(decoder: Decoder): InvoicesResourceShippingCost {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("InvoicesResourceShippingCost must be a JSON object")
      ...
      return InvoicesResourceShippingCost(
        ...
        amountTax = amountTax,
        amountTotal = amountTotal,
        shippingRate = rawObject["shipping_rate"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineInvoicesResourceShippingCostShippingRateX8cbdd80f?>(element) },
        taxes = rawObject["taxes"]?.let { json.decodeFromJsonElement<List<LineItemsTaxAmount>>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: InvoicesResourceShippingCost) {
...
public fun invoicesResourceShippingCost(block: InvoicesResourceShippingCost.Builder.() -> Unit): InvoicesResourceShippingCost = InvoicesResourceShippingCost.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("InvoicesResourceShippingCost is missing required property '" + name + "'")
