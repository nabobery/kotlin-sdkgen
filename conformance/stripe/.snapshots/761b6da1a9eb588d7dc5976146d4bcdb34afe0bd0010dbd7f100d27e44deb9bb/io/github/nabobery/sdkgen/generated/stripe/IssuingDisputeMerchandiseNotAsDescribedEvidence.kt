...
public class IssuingDisputeMerchandiseNotAsDescribedEvidence(
  ...
  public class Builder {
    ...
    public fun build(): IssuingDisputeMerchandiseNotAsDescribedEvidence = IssuingDisputeMerchandiseNotAsDescribedEvidence(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): IssuingDisputeMerchandiseNotAsDescribedEvidence = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<IssuingDisputeMerchandiseNotAsDescribedEvidence> {
    ...
    override fun deserialize(decoder: Decoder): IssuingDisputeMerchandiseNotAsDescribedEvidence {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("IssuingDisputeMerchandiseNotAsDescribedEvidence must be a JSON object")
      return IssuingDisputeMerchandiseNotAsDescribedEvidence(
        additionalDocumentation = rawObject["additional_documentation"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingDisputeMercha6387AdditionalDocumentationX3578dfd0?>(element) },
        ...
        returnDescription = rawObject["return_description"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        returnStatus = rawObject["return_status"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingDisputeMerchandiseNotAsDescribedEvidenceReturnStatusXec050985?>(element) },
        returnedAt = rawObject["returned_at"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: IssuingDisputeMerchandiseNotAsDescribedEvidence) {
...
public fun issuingDisputeMerchandiseNotAsDescribedEvidence(block: IssuingDisputeMerchandiseNotAsDescribedEvidence.Builder.() -> Unit): IssuingDisputeMerchandiseNotAsDescribedEvidence = IssuingDisputeMerchandiseNotAsDescribedEvidence.build(block)
