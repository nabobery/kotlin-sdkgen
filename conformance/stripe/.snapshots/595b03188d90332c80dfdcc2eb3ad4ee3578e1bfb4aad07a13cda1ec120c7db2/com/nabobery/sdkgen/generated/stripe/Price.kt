...
public class Price(
    ...
    public class Builder {
         ...
         * currency](https://stripe.com/docs/currencies).
         */
        public var currencyOptions: Map<String, CurrencyOption>? = null
         ...
         * the documentation for `billing_scheme`.
         */
        public var tiers: List<PriceTier>? = null
    ...
    public object Serializer : KSerializer<Price> {
        ...
        override fun deserialize(decoder: Decoder): Price {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("Price must be a JSON object")
            ...
            return Price(
                ...
                currencyOptions =
                    rawObject["currency_options"]?.let {
                        json.decodeFromJsonElement<Map<String, CurrencyOption>>(
                            it,
                        )
                    },
                customUnitAmount =
                    rawObject["custom_unit_amount"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlinePriceCustomUnitAmountXd58f53a7?>(element)
                        }
                    },
                lookupKey =
                ...
                recurring =
                    rawObject["recurring"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlinePriceRecurringX2e8687de?>(element)
                        }
                    },
                taxBehavior =
                    rawObject["tax_behavior"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlinePriceTaxBehaviorX0141c91e?>(element)
                        }
                    },
                tiers = rawObject["tiers"]?.let { json.decodeFromJsonElement<List<PriceTier>>(it) },
                tiersMode =
                    rawObject["tiers_mode"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlinePriceTiersModeX49856d16?>(element)
                        }
                    },
                transformQuantity =
                    rawObject["transform_quantity"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlinePriceTransformQuantityX05ef18f6?>(element)
                        }
                    },
                unitAmount =
