...
public class IssuingAuthorizationFuelData(
    ...
    public class Builder {
        ...
        public fun build(): IssuingAuthorizationFuelData =
    ...
    public object Serializer : KSerializer<IssuingAuthorizationFuelData> {
        ...
        override fun deserialize(decoder: Decoder): IssuingAuthorizationFuelData {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("IssuingAuthorizationFuelData must be a JSON object")
            return IssuingAuthorizationFuelData(
                industryProductCode =
                    rawObject["industry_product_code"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                type =
                    rawObject["type"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineIssuingAuthorizationFuelDataTypeX66f3603f?>(element)
                        }
                    },
                unit =
                    rawObject["unit"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineIssuingAuthorizationFuelDataUnitX94435e5d?>(element)
                        }
                    },
                unitCostDecimal =
                    rawObject["unit_cost_decimal"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: IssuingAuthorizationFuelData,
        ) {
...
public fun issuingAuthorizationFuelData(
    block: IssuingAuthorizationFuelData.Builder.() -> Unit,
): IssuingAuthorizationFuelData = IssuingAuthorizationFuelData.build(block)
