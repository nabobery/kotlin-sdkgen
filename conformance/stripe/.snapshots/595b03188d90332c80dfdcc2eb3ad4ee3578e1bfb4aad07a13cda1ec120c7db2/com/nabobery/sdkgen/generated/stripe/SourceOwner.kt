...
public class SourceOwner(
    ...
    public object Serializer : KSerializer<SourceOwner> {
        ...
        override fun deserialize(decoder: Decoder): SourceOwner {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("SourceOwner must be a JSON object")
            return SourceOwner(
                address =
                    rawObject["address"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineSourceOwnerAddressX25e060b2?>(element)
                        }
                    },
                email =
                ...
                verifiedAddress =
                    rawObject["verified_address"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineSourceOwnerVerifiedAddressXc1a5a209?>(element)
                        }
                    },
                verifiedEmail =
                    rawObject["verified_email"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                verifiedPhone =
                    rawObject["verified_phone"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
