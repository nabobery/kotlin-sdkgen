...
public class PaymentPagesCheckoutSessionTotalDetails(
    ...
    public companion object {
        public fun build(block: Builder.() -> Unit): PaymentPagesCheckoutSessionTotalDetails =
            Builder().apply(block).build()
    }

    public object Serializer : KSerializer<PaymentPagesCheckoutSessionTotalDetails> {
        ...
        override fun deserialize(decoder: Decoder): PaymentPagesCheckoutSessionTotalDetails {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("PaymentPagesCheckoutSessionTotalDetails must be a JSON object")
            ...
            return PaymentPagesCheckoutSessionTotalDetails(
                ...
                amountTax = amountTax,
                amountShipping =
                    rawObject["amount_shipping"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                breakdown =
                    rawObject["breakdown"]?.let {
                        json.decodeFromJsonElement<PaymentPagesCheckoutSessionTotalDetailsResourceBreakdown>(it)
                    },
            )
        }
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: PaymentPagesCheckoutSessionTotalDetails,
        ) {
...
public fun paymentPagesCheckoutSessionTotalDetails(
    block: PaymentPagesCheckoutSessionTotalDetails.Builder.() -> Unit,
): PaymentPagesCheckoutSessionTotalDetails = PaymentPagesCheckoutSessionTotalDetails.build(block)
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "PaymentPagesCheckoutSessionTotalDetails is missing required property '" + name + "'",
            )
