...
public class PaymentMethodOptionsCardPresent(
    ...
    public class Builder {
        ...
        public fun build(): PaymentMethodOptionsCardPresent =
    ...
    public object Serializer : KSerializer<PaymentMethodOptionsCardPresent> {
        ...
        override fun deserialize(decoder: Decoder): PaymentMethodOptionsCardPresent {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("PaymentMethodOptionsCardPresent must be a JSON object")
            return PaymentMethodOptionsCardPresent(
                captureMethod =
                    rawObject["capture_method"]?.let {
                        json.decodeFromJsonElement<InlinePaymentMethodOptionsCardPresentCaptureMethodXd1871575>(it)
                    },
                requestExtendedAuthorization =
                    rawObject["request_extended_authorization"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                requestIncrementalAuthorizationSupport =
                    rawObject["request_incremental_authorization_support"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: PaymentMethodOptionsCardPresent,
        ) {
...
public fun paymentMethodOptionsCardPresent(
    block: PaymentMethodOptionsCardPresent.Builder.() -> Unit,
): PaymentMethodOptionsCardPresent = PaymentMethodOptionsCardPresent.build(block)
