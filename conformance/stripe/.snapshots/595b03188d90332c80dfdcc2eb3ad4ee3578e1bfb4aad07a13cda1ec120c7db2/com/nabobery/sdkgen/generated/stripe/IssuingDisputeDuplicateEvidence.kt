...
public class IssuingDisputeDuplicateEvidence(
     ...
     * dispute.
     */
    public val additionalDocumentation: InlineIssuingDisputeDuplicateEvidenceAdditionalDocumentationXe7af1f67? = null,
    /**
    ...
    public class Builder {
        ...
        public fun build(): IssuingDisputeDuplicateEvidence =
    ...
    public object Serializer : KSerializer<IssuingDisputeDuplicateEvidence> {
        ...
        override fun deserialize(decoder: Decoder): IssuingDisputeDuplicateEvidence {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("IssuingDisputeDuplicateEvidence must be a JSON object")
            return IssuingDisputeDuplicateEvidence(
                additionalDocumentation =
                    rawObject["additional_documentation"]?.let { element ->
                        if (element ==
                            ...
                            json
                                .decodeFromJsonElement<InlineIssuingDisputeDuplicateEvidenceAdditionalDocumentationXe7af1f67?>(
                                    element,
                                )
                        }
                    },
                cardStatement =
                    rawObject["card_statement"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineIssuingDisputeDuplicateEvidenceCardStatementXbf9362eb?>(
                                element,
                            )
                        }
                    },
                cashReceipt =
                    rawObject["cash_receipt"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineIssuingDisputeDuplicateEvidenceCashReceiptX56eae54c?>(
                                element,
                            )
                        }
                    },
                checkImage =
                    rawObject["check_image"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineIssuingDisputeDuplicateEvidenceCheckImageX4d2bc900?>(
                                element,
                            )
                        }
                    },
                ...
                originalTransaction =
                    rawObject["original_transaction"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: IssuingDisputeDuplicateEvidence,
        ) {
...
public fun issuingDisputeDuplicateEvidence(
    block: IssuingDisputeDuplicateEvidence.Builder.() -> Unit,
): IssuingDisputeDuplicateEvidence = IssuingDisputeDuplicateEvidence.build(block)
