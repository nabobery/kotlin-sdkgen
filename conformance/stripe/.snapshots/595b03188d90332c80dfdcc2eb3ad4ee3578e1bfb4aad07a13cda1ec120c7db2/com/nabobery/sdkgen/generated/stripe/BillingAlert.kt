...
public class BillingAlert(
    ...
    public object Serializer : KSerializer<BillingAlert> {
        ...
        override fun deserialize(decoder: Decoder): BillingAlert {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("BillingAlert must be a JSON object")
                ...
                status =
                    rawObject["status"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineBillingAlertStatusX73f6a8ea?>(element)
                        }
                    },
                usageThreshold =
                    rawObject["usage_threshold"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineBillingAlertUsageThresholdXeb2ffc97?>(element)
                        }
                    },
            )
        }
