...
public class BankConnectionsResourceTransactionRefresh(
    ...
    public companion object {
        public fun build(block: Builder.() -> Unit): BankConnectionsResourceTransactionRefresh =
            Builder().apply(block).build()
    }

    public object Serializer : KSerializer<BankConnectionsResourceTransactionRefresh> {
        ...
        override fun deserialize(decoder: Decoder): BankConnectionsResourceTransactionRefresh {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("BankConnectionsResourceTransactionRefresh must be a JSON object")
            val id = json.decodeRequired<String>(rawObject, "id")
            val lastAttemptedAt = json.decodeRequired<Int>(rawObject, "last_attempted_at")
            val status =
                json.decodeRequired<InlineBankConnectionsResourceTransactionRefreshStatusX0b9d5432>(
                    rawObject,
                    "status",
                )
            return BankConnectionsResourceTransactionRefresh(
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: BankConnectionsResourceTransactionRefresh,
        ) {
...
public fun bankConnectionsResourceTransactionRefresh(
    block: BankConnectionsResourceTransactionRefresh.Builder.() -> Unit,
): BankConnectionsResourceTransactionRefresh = BankConnectionsResourceTransactionRefresh.build(block)
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "BankConnectionsResourceTransactionRefresh is missing required property '" + name + "'",
            )
