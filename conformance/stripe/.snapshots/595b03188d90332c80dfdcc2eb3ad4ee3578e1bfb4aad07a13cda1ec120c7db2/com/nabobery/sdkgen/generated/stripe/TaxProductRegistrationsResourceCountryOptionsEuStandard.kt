...
public class TaxProductRegistrationsResourceCountryOptionsEuStandard(
    ...
    public companion object {
        public fun build(block: Builder.() -> Unit): TaxProductRegistrationsResourceCountryOptionsEuStandard =
            Builder().apply(block).build()
    }

    public object Serializer : KSerializer<TaxProductRegistrationsResourceCountryOptionsEuStandard> {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException(
                        "TaxProductRegistrationsResourceCountryOptionsEuStandard must be a JSON object",
                    )
            val placeOfSupplyScheme =
                json.decodeRequired<InlineTaxProductRegistrati09bbPlaceOfSupplySchemeX7e674b46>(
                    rawObject,
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: TaxProductRegistrationsResourceCountryOptionsEuStandard,
        ) {
...
public fun taxProductRegistrationsResourceCountryOptionsEuStandard(
    block: TaxProductRegistrationsResourceCountryOptionsEuStandard.Builder.() -> Unit,
): TaxProductRegistrationsResourceCountryOptionsEuStandard =
    TaxProductRegistrationsResourceCountryOptionsEuStandard.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "TaxProductRegistrationsResourceCountryOptionsEuStandard is missing required property '" + name + "'",
            )
