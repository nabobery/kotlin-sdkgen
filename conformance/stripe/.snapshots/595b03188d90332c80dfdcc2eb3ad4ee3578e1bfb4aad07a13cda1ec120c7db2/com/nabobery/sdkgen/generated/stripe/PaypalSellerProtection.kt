...
@Serializable
public data class PaypalSellerProtectionView(
    @SerialName("dispute_categories")
    public val disputeCategories: List<InlinePaypalSellerProtectionDisputeCategoriesItemX3c52ff29>? = null,
...
public class PaypalSellerProtection(
    ...
    public object Serializer : KSerializer<PaypalSellerProtection> {
        ...
        override fun deserialize(decoder: Decoder): PaypalSellerProtection {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("PaypalSellerProtection must be a JSON object")
            val status = json.decodeRequired<InlinePaypalSellerProtectionStatusX8b3cff4a>(rawObject, "status")
            return PaypalSellerProtection(
                status = status,
                disputeCategories =
                    rawObject["dispute_categories"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                            ...
                            json
                                .decodeFromJsonElement<List<InlinePaypalSellerProtectionDisputeCategoriesItemX3c52ff29>?>(
                                    element,
                                )
                        }
                    },
...
public fun paypalSellerProtection(block: PaypalSellerProtection.Builder.() -> Unit): PaypalSellerProtection =
    PaypalSellerProtection.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name] ?: throw SerializationException("PaypalSellerProtection is missing required property '" + name + "'")
