...
public class TaxSettings(
    ...
    public object Serializer : KSerializer<TaxSettings> {
        ...
        override fun deserialize(decoder: Decoder): TaxSettings {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("TaxSettings must be a JSON object")
            ...
            val statusDetails =
                json.decodeRequired<TaxProductResourceTaxSettingsStatusDetails>(
                    rawObject,
                    "status_details",
                )
            return TaxSettings(
                ...
                statusDetails = statusDetails,
                headOffice =
                    rawObject["head_office"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineTaxSettingsHeadOfficeXdd03fd1f?>(element)
                        }
                    },
            )
        }
