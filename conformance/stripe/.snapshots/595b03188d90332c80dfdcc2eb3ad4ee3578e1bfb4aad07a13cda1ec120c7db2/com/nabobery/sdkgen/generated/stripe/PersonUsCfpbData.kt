...
public class PersonUsCfpbData(
    ...
    public class Builder {
        ...
        public fun build(): PersonUsCfpbData =
            PersonUsCfpbData(
    ...
    public object Serializer : KSerializer<PersonUsCfpbData> {
        ...
        override fun deserialize(decoder: Decoder): PersonUsCfpbData {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("PersonUsCfpbData must be a JSON object")
            return PersonUsCfpbData(
                ethnicityDetails =
                    rawObject["ethnicity_details"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlinePersonUsCfpbDataEthnicityDetailsX04c1a9f8?>(element)
                        }
                    },
                raceDetails =
                    rawObject["race_details"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlinePersonUsCfpbDataRaceDetailsX44c61382?>(element)
                        }
                    },
                selfIdentifiedGender =
                    rawObject["self_identified_gender"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
...
public fun personUsCfpbData(block: PersonUsCfpbData.Builder.() -> Unit): PersonUsCfpbData =
    PersonUsCfpbData.build(block)
