...
public class InlineIssuingPersonalizationDesignPhysicalBundleXdaa9c3d3 internal constructor(
    ...
    public val issuingPhysicalBundle: IssuingPhysicalBundle? by
        lazy(LazyThreadSafetyMode.NONE) {
            if (inspection.matchesIssuingPhysicalBundle) {
                json.decodeFromJsonElement<IssuingPhysicalBundle>(
                    raw,
                )
            } else {
                null
        ...
        get() =
            buildSet {
                if (inspection.matchesBranch1) {
                    add(
                        InlineIssuingPersonalizationDesignPhysicalBundleXdaa9c3d3Branch.Branch1,
                    )
                }
                if (inspection.matchesIssuingPhysicalBundle) {
                    add(
                        InlineIssuingPersonalizationDesignPhysicalBundleXdaa9c3d3Branch.IssuingPhysicalBundle,
                    )
                }
            }
    ...
    public companion object {
        ...
        public fun fromRaw(
            raw: JsonElement,
            json: Json = SdkJson,
        ): InlineIssuingPersonalizationDesignPhysicalBundleXdaa9c3d3 {
            val inspection = inspectInlineIssuingPersonalizationDesignPhysicalBundleXdaa9c3d3(raw)
            if (inspection.matchCount == 0) {
                throw InlineIssuingPersonalizationDesignPhysicalBundleXdaa9c3d3NoMatchException(
                    "InlineIssuingPersonalizationDesignPhysicalBundleXdaa9c3d3 matched 0 branches: " +
    ...
    public object Serializer : KSerializer<InlineIssuingPersonalizationDesignPhysicalBundleXdaa9c3d3> {
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: InlineIssuingPersonalizationDesignPhysicalBundleXdaa9c3d3,
        ) {
            encoder
                .requireJsonEncoder(
                    "InlineIssuingPersonalizationDesignPhysicalBundleXdaa9c3d3",
...
private fun inspectInlineIssuingPersonalizationDesignPhysicalBundleXdaa9c3d3(
    element: JsonElement,
): InlineIssuingPersonalizationDesignPhysicalBundleXdaa9c3d3Inspection {
    ...
    return InlineIssuingPersonalizationDesignPhysicalBundleXdaa9c3d3Inspection(
        ...
        failures =
            buildList {
                ...
                if (!matchesIssuingPhysicalBundle) {
                    add(
                        "IssuingPhysicalBundle: value does not match IssuingPhysicalBundle",
                    )
                }
            },
    )
