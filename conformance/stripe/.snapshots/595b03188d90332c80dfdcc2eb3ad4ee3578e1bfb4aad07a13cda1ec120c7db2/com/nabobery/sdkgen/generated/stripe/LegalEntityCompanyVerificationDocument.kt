...
public class LegalEntityCompanyVerificationDocument(
    ...
    public class Builder {
        ...
        public fun build(): LegalEntityCompanyVerificationDocument =
    ...
    public companion object {
        public fun build(block: Builder.() -> Unit): LegalEntityCompanyVerificationDocument =
            Builder().apply(block).build()
    }

    public object Serializer : KSerializer<LegalEntityCompanyVerificationDocument> {
        ...
        override fun deserialize(decoder: Decoder): LegalEntityCompanyVerificationDocument {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("LegalEntityCompanyVerificationDocument must be a JSON object")
            return LegalEntityCompanyVerificationDocument(
                back =
                    rawObject["back"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineLegalEntityCompanyVerificationDocumentBackX113d3925?>(
                                element,
                            )
                        }
                    },
                details =
                    rawObject["details"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                detailsCode =
                    rawObject["details_code"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                front =
                    rawObject["front"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineLegalEntityCompanyVerificationDocumentFrontXaf6516e7?>(
                                element,
                            )
                        }
                    },
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: LegalEntityCompanyVerificationDocument,
        ) {
...
public fun legalEntityCompanyVerificationDocument(
    block: LegalEntityCompanyVerificationDocument.Builder.() -> Unit,
): LegalEntityCompanyVerificationDocument = LegalEntityCompanyVerificationDocument.build(block)
