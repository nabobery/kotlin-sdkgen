...
public class PortalFlowsRetention(
    ...
    public object Serializer : KSerializer<PortalFlowsRetention> {
        ...
        override fun deserialize(decoder: Decoder): PortalFlowsRetention {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("PortalFlowsRetention must be a JSON object")
            val type = json.decodeRequired<InlinePortalFlowsRetentionTypeX7c3ffff6>(rawObject, "type")
            return PortalFlowsRetention(
                type = type,
                couponOffer =
                    rawObject["coupon_offer"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlinePortalFlowsRetentionCouponOfferX51f9e84e?>(element)
                        }
                    },
            )
        }
...
public fun portalFlowsRetention(block: PortalFlowsRetention.Builder.() -> Unit): PortalFlowsRetention =
    PortalFlowsRetention.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name] ?: throw SerializationException("PortalFlowsRetention is missing required property '" + name + "'")
