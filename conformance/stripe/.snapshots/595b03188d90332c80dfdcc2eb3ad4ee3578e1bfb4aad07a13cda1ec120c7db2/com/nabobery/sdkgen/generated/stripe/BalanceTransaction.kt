...
@Serializable
public data class BalanceTransactionView(
...
public class BalanceTransaction(
    ...
    public class Builder {
        ...
        public var feeDetails: List<Fee>
            get() = requireNotNull(feeDetailsValue) { "feeDetails is required" }
            set(`value`) {
                feeDetailsValue = value
            }
    ...
    public object Serializer : KSerializer<BalanceTransaction> {
        ...
        override fun deserialize(decoder: Decoder): BalanceTransaction {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("BalanceTransaction must be a JSON object")
            val amount = json.decodeRequired<Int>(rawObject, "amount")
            val availableOn = json.decodeRequired<Int>(rawObject, "available_on")
            val balanceType =
                json.decodeRequired<InlineBalanceTransactionBalanceTypeXc7269442>(
                    rawObject,
                    "balance_type",
                )
            ...
            return BalanceTransaction(
                ...
                exchangeRate =
                    rawObject["exchange_rate"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                source =
                    rawObject["source"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineBalanceTransactionSourceX72d51cf4?>(element)
                        }
                    },
            )
        }
...
public fun balanceTransaction(block: BalanceTransaction.Builder.() -> Unit): BalanceTransaction =
    BalanceTransaction.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name] ?: throw SerializationException("BalanceTransaction is missing required property '" + name + "'")
