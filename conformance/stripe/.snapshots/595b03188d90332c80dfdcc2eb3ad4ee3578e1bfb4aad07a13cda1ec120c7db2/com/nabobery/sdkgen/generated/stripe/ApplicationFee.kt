...
public class ApplicationFee(
    ...
    public object Serializer : KSerializer<ApplicationFee> {
        ...
        override fun deserialize(decoder: Decoder): ApplicationFee {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("ApplicationFee must be a JSON object")
                ...
                balanceTransaction =
                    rawObject["balance_transaction"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineApplicationFeeBalanceTransactionX1d204e15?>(element)
                        }
                    },
                feeSource =
                    rawObject["fee_source"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineApplicationFeeFeeSourceX78740799?>(element)
                        }
                    },
                originatingTransaction =
                    rawObject["originating_transaction"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineApplicationFeeOriginatingTransactionX9ab63b0d?>(element)
                        }
                    },
            )
        }
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name] ?: throw SerializationException("ApplicationFee is missing required property '" + name + "'")
