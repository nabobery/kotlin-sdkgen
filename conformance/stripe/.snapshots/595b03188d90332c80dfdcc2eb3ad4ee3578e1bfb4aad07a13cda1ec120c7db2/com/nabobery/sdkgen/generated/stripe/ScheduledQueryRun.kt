        ...
        override fun deserialize(decoder: Decoder): ScheduledQueryRun {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("ScheduledQueryRun must be a JSON object")
            ...
            return ScheduledQueryRun(
                ...
                file =
                    rawObject["file"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineScheduledQueryRunFileX630c80f5?>(element)
                        }
                    },
            )
        }
...
public fun scheduledQueryRun(block: ScheduledQueryRun.Builder.() -> Unit): ScheduledQueryRun =
    ScheduledQueryRun.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name] ?: throw SerializationException("ScheduledQueryRun is missing required property '" + name + "'")
