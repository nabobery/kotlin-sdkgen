...
public class Review(
    ...
    public object Serializer : KSerializer<Review> {
        ...
        override fun deserialize(decoder: Decoder): Review {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("Review must be a JSON object")
            ...
            return Review(
                ...
                reason = reason,
                billingZip =
                    rawObject["billing_zip"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                charge =
                    rawObject["charge"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineReviewChargeX13315133?>(element)
                        }
                    },
                closedReason =
                    rawObject["closed_reason"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineReviewClosedReasonXec1ea24e?>(element)
                        }
                    },
                ipAddress =
                ...
                ipAddressLocation =
                    rawObject["ip_address_location"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineReviewIpAddressLocationX09e4e999?>(element)
                        }
                    },
                paymentIntent =
                    rawObject["payment_intent"]?.let {
                        json
                            .decodeFromJsonElement<InlineReviewPaymentIntentXfa4a839e>(
                                it,
                            )
                    },
                session =
                    rawObject["session"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineReviewSessionXb92996b5?>(element)
                        }
                    },
            )
        }
