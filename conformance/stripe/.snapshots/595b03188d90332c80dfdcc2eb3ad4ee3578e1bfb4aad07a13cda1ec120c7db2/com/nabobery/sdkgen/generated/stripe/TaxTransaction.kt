        ...
        override fun deserialize(decoder: Decoder): TaxTransaction {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("TaxTransaction must be a JSON object")
            ...
            return TaxTransaction(
                ...
                lineItems =
                    rawObject["line_items"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineTaxTransactionLineItemsXbeaf3154?>(element)
                        }
                    },
                metadata =
                ...
                reversal =
                    rawObject["reversal"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineTaxTransactionReversalX8e426397?>(element)
                        }
                    },
                shipFromDetails =
                    rawObject["ship_from_details"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineTaxTransactionShipFromDetailsXb15ac2b5?>(element)
                        }
                    },
                shippingCost =
                    rawObject["shipping_cost"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineTaxTransactionShippingCostX19bb479f?>(element)
                        }
                    },
            )
        }
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name] ?: throw SerializationException("TaxTransaction is missing required property '" + name + "'")
