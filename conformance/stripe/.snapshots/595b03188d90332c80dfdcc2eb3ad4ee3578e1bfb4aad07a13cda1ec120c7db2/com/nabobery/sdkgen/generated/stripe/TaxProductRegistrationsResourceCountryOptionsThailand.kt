...
public class TaxProductRegistrationsResourceCountryOptionsThailand(
    ...
    public companion object {
        public fun build(block: Builder.() -> Unit): TaxProductRegistrationsResourceCountryOptionsThailand =
    ...
    public object Serializer : KSerializer<TaxProductRegistrationsResourceCountryOptionsThailand> {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException(
                        "TaxProductRegistrationsResourceCountryOptionsThailand must be a JSON object",
                    )
            val type =
                json.decodeRequired<InlineTaxProductRegistrationsResourceCountryOptionsThailandTypeX831d6740>(
                    rawObject,
                    "type",
                )
            return TaxProductRegistrationsResourceCountryOptionsThailand(
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: TaxProductRegistrationsResourceCountryOptionsThailand,
        ) {
...
public fun taxProductRegistrationsResourceCountryOptionsThailand(
    block: TaxProductRegistrationsResourceCountryOptionsThailand.Builder.() -> Unit,
): TaxProductRegistrationsResourceCountryOptionsThailand =
    TaxProductRegistrationsResourceCountryOptionsThailand.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "TaxProductRegistrationsResourceCountryOptionsThailand is missing required property '" + name + "'",
            )
