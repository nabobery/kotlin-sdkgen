...
public class SubscriptionsResourceBillingMode(
    ...
    public object Serializer : KSerializer<SubscriptionsResourceBillingMode> {
        ...
        override fun deserialize(decoder: Decoder): SubscriptionsResourceBillingMode {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("SubscriptionsResourceBillingMode must be a JSON object")
            val type = json.decodeRequired<InlineSubscriptionsResourceBillingModeTypeX424998b4>(rawObject, "type")
            return SubscriptionsResourceBillingMode(
                type = type,
                flexible =
                    rawObject["flexible"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineSubscriptionsResourceBillingModeFlexibleXcfa0fc39?>(
                                element,
                            )
                        }
                    },
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: SubscriptionsResourceBillingMode,
        ) {
...
public fun subscriptionsResourceBillingMode(
    block: SubscriptionsResourceBillingMode.Builder.() -> Unit,
): SubscriptionsResourceBillingMode = SubscriptionsResourceBillingMode.build(block)
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "SubscriptionsResourceBillingMode is missing required property '" + name + "'",
            )
