...
public class IdentityVerificationSession(
     ...
     */
    public val lastVerificationReport: InlineIdentityVerificationSessionLastVerificationReportX5959b875? = null,
    /**
    ...
    public object Serializer : KSerializer<IdentityVerificationSession> {
        ...
        override fun deserialize(decoder: Decoder): IdentityVerificationSession {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("IdentityVerificationSession must be a JSON object")
            val created = json.decodeRequired<Int>(rawObject, "created")
            val id = json.decodeRequired<String>(rawObject, "id")
            val livemode = json.decodeRequired<Boolean>(rawObject, "livemode")
            val metadata = json.decodeRequired<Map<String, String>>(rawObject, "metadata")
            val objectValue =
                json.decodeRequired<InlineIdentityVerificationSessionObjectValueX913fcef9>(
                    rawObject,
                    "object",
                )
            ...
            return IdentityVerificationSession(
                ...
                lastError =
                    rawObject["last_error"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineIdentityVerificationSessionLastErrorXbe973247?>(element)
                        }
                    },
                lastVerificationReport =
                    rawObject["last_verification_report"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                            ...
                            json
                                .decodeFromJsonElement<InlineIdentityVerificationSessionLastVerificationReportX5959b875?>(
                                    element,
                                )
                        }
                    },
                options =
                    rawObject["options"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineIdentityVerificationSessionOptionsX8b054091?>(element)
                        }
                    },
                providedDetails =
                    rawObject["provided_details"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineIdentityVerificationSessionProvidedDetailsXa010c3d8?>(
                                element,
                            )
                        }
                    },
                redaction =
                    rawObject["redaction"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineIdentityVerificationSessionRedactionXf0d957f7?>(element)
                        }
                    },
                relatedCustomer =
                    rawObject["related_customer"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                relatedCustomerAccount =
                    rawObject["related_customer_account"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                relatedPerson =
                    rawObject["related_person"]?.let {
                        json
                            .decodeFromJsonElement<GelatoRelatedPerson>(
                                it,
                            )
                    },
                url =
                ...
                verifiedOutputs =
                    rawObject["verified_outputs"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineIdentityVerificationSessionVerifiedOutputsXd58d8d01?>(
                                element,
                            )
                        }
                    },
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: IdentityVerificationSession,
        ) {
...
public fun identityVerificationSession(
    block: IdentityVerificationSession.Builder.() -> Unit,
): IdentityVerificationSession = IdentityVerificationSession.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name]
            ?: throw SerializationException("IdentityVerificationSession is missing required property '" + name + "'")
