...
public class PaymentMethodOptionsBoleto(
    ...
    public object Serializer : KSerializer<PaymentMethodOptionsBoleto> {
        ...
        override fun deserialize(decoder: Decoder): PaymentMethodOptionsBoleto {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("PaymentMethodOptionsBoleto must be a JSON object")
            val expiresAfterDays = json.decodeRequired<Int>(rawObject, "expires_after_days")
            return PaymentMethodOptionsBoleto(
                expiresAfterDays = expiresAfterDays,
                setupFutureUsage =
                    rawObject["setup_future_usage"]?.let {
                        json.decodeFromJsonElement<InlinePaymentMethodOptionsBoletoSetupFutureUsageX3117ae15>(it)
                    },
            )
        }
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: PaymentMethodOptionsBoleto,
        ) {
...
public fun paymentMethodOptionsBoleto(
    block: PaymentMethodOptionsBoleto.Builder.() -> Unit,
): PaymentMethodOptionsBoleto = PaymentMethodOptionsBoleto.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name]
            ?: throw SerializationException("PaymentMethodOptionsBoleto is missing required property '" + name + "'")
