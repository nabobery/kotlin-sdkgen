...
public class TreasuryFinancialAccountsResourceToggleSettings(
    ...
    public companion object {
        public fun build(block: Builder.() -> Unit): TreasuryFinancialAccountsResourceToggleSettings =
            Builder().apply(block).build()
    }

    public object Serializer : KSerializer<TreasuryFinancialAccountsResourceToggleSettings> {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException(
                        "TreasuryFinancialAccountsResourceToggleSettings must be a JSON object",
                    )
            val requested = json.decodeRequired<Boolean>(rawObject, "requested")
            val status =
                json.decodeRequired<InlineTreasuryFinancialAccountsResourceToggleSettingsStatusX34733635>(
                    rawObject,
                    "status",
                )
            val statusDetails =
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: TreasuryFinancialAccountsResourceToggleSettings,
        ) {
...
public fun treasuryFinancialAccountsResourceToggleSettings(
    block: TreasuryFinancialAccountsResourceToggleSettings.Builder.() -> Unit,
): TreasuryFinancialAccountsResourceToggleSettings = TreasuryFinancialAccountsResourceToggleSettings.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "TreasuryFinancialAccountsResourceToggleSettings is missing required property '" + name + "'",
            )
