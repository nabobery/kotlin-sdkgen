...
public class LegalEntityPersonVerification(
     ...
     */
    public val additionalDocument: InlineLegalEntityPersonVerificationAdditionalDocumentXcccff13d? = null,
    /**
    ...
    public object Serializer : KSerializer<LegalEntityPersonVerification> {
        ...
        override fun deserialize(decoder: Decoder): LegalEntityPersonVerification {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("LegalEntityPersonVerification must be a JSON object")
            val status = json.decodeRequired<String>(rawObject, "status")
            return LegalEntityPersonVerification(
                status = status,
                additionalDocument =
                    rawObject["additional_document"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineLegalEntityPersonVerificationAdditionalDocumentXcccff13d?>(
                                element,
                            )
                        }
                    },
                ...
                document =
                    rawObject["document"]?.let {
                        json
                            .decodeFromJsonElement<LegalEntityPersonVerificationDocument>(
                                it,
                            )
                    },
            )
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: LegalEntityPersonVerification,
        ) {
...
public fun legalEntityPersonVerification(
    block: LegalEntityPersonVerification.Builder.() -> Unit,
): LegalEntityPersonVerification = LegalEntityPersonVerification.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name]
            ?: throw SerializationException("LegalEntityPersonVerification is missing required property '" + name + "'")
