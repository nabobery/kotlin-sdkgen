...
public class PortalSubscriptionUpdate(
     ...
     */
    public val products: List<PortalSubscriptionUpdateProduct>? = null,
) {
    /**
    ...
    public class Builder {
        ...
        public var defaultAllowedUpdates:
            List<InlinePortalSubscriptionUpdateDefaultAllowedUpdatesItemXa88ddcc9>
            get() = requireNotNull(defaultAllowedUpdatesValue) { "defaultAllowedUpdates is required" }
            set(`value`) {
                defaultAllowedUpdatesValue = value
            }
         ...
         * The list of up to 10 products that support subscription updates.
         */
        public var products: List<PortalSubscriptionUpdateProduct>? = null
    ...
    public object Serializer : KSerializer<PortalSubscriptionUpdate> {
        ...
        override fun deserialize(decoder: Decoder): PortalSubscriptionUpdate {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("PortalSubscriptionUpdate must be a JSON object")
            val defaultAllowedUpdates =
                json
                    .decodeRequired<List<InlinePortalSubscriptionUpdateDefaultAllowedUpdatesItemXa88ddcc9>>(
                        rawObject,
                        "default_allowed_updates",
                    )
            val enabled = json.decodeRequired<Boolean>(rawObject, "enabled")
            val prorationBehavior =
                json.decodeRequired<InlinePortalSubscriptionUpdateProrationBehaviorXbd6e9809>(
                    rawObject,
                    "proration_behavior",
                )
            val scheduleAtPeriodEnd =
                json.decodeRequired<PortalResourceScheduleUpdateAtPeriodEnd>(
                    rawObject,
                    "schedule_at_period_end",
                )
            val trialUpdateBehavior =
                json.decodeRequired<InlinePortalSubscriptionUpdateTrialUpdateBehaviorX4154e1cf>(
                    rawObject,
                ...
                billingCycleAnchor =
                    rawObject["billing_cycle_anchor"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlinePortalSubscriptionUpdateBillingCycleAnchorXb8f51ecd?>(
                                element,
                            )
                        }
                    },
                products =
                    rawObject["products"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<List<PortalSubscriptionUpdateProduct>?>(element)
                        }
                    },
            )
        }
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: PortalSubscriptionUpdate,
        ) {
...
public fun portalSubscriptionUpdate(block: PortalSubscriptionUpdate.Builder.() -> Unit): PortalSubscriptionUpdate =
    PortalSubscriptionUpdate.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name]
            ?: throw SerializationException("PortalSubscriptionUpdate is missing required property '" + name + "'")
