...
@Serializable
public data class ThresholdsResourceUsageThresholdConfigView(
...
public class ThresholdsResourceUsageThresholdConfig(
    ...
    public class Builder {
         ...
         * The filters allow limiting the scope of this usage alert. You can only specify up to one filter at this time.
         */
        public var filters: List<ThresholdsResourceUsageAlertFilter>? = null
    ...
    public companion object {
        public fun build(block: Builder.() -> Unit): ThresholdsResourceUsageThresholdConfig =
            Builder().apply(block).build()
    }

    public object Serializer : KSerializer<ThresholdsResourceUsageThresholdConfig> {
        ...
        override fun deserialize(decoder: Decoder): ThresholdsResourceUsageThresholdConfig {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("ThresholdsResourceUsageThresholdConfig must be a JSON object")
            val gte = json.decodeRequired<Int>(rawObject, "gte")
            val meter =
                json.decodeRequired<InlineThresholdsResourceUsageThresholdConfigMeterXe7f01422>(
                    rawObject,
                    "meter",
                )
            val recurrence =
                json.decodeRequired<InlineThresholdsResourceUsageThresholdConfigRecurrenceXbf90c41d>(
                    rawObject,
                    "recurrence",
                )
            return ThresholdsResourceUsageThresholdConfig(
                ...
                recurrence = recurrence,
                filters =
                    rawObject["filters"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<List<ThresholdsResourceUsageAlertFilter>?>(element)
                        }
                    },
            )
        }
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: ThresholdsResourceUsageThresholdConfig,
        ) {
...
public fun thresholdsResourceUsageThresholdConfig(
    block: ThresholdsResourceUsageThresholdConfig.Builder.() -> Unit,
): ThresholdsResourceUsageThresholdConfig = ThresholdsResourceUsageThresholdConfig.build(block)
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "ThresholdsResourceUsageThresholdConfig is missing required property '" + name + "'",
            )
