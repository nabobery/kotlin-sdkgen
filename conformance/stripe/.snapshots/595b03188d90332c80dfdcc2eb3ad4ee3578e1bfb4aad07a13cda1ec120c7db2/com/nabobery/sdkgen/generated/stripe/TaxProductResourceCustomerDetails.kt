...
public class TaxProductResourceCustomerDetails(
    ...
    public class Builder {
        ...
        public var taxIds: List<TaxProductResourceCustomerDetailsResourceTaxId>
            get() = requireNotNull(taxIdsValue) { "taxIds is required" }
            set(`value`) {
                taxIdsValue = value
            }
    ...
    public object Serializer : KSerializer<TaxProductResourceCustomerDetails> {
        ...
        override fun deserialize(decoder: Decoder): TaxProductResourceCustomerDetails {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("TaxProductResourceCustomerDetails must be a JSON object")
            val taxIds = json.decodeRequired<List<TaxProductResourceCustomerDetailsResourceTaxId>>(rawObject, "tax_ids")
            val taxabilityOverride =
                json.decodeRequired<InlineTaxProductResourceCustomerDetailsTaxabilityOverrideXae37b37b>(
                    rawObject,
                    "taxability_override",
                )
            return TaxProductResourceCustomerDetails(
                ...
                address =
                    rawObject["address"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineTaxProductResourceCustomerDetailsAddressXdece9341?>(
                                element,
                            )
                        }
                    },
                addressSource =
                    rawObject["address_source"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineTaxProductResourceCustomerDetailsAddressSourceX64204034?>(
                                element,
                            )
                        }
                    },
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: TaxProductResourceCustomerDetails,
        ) {
...
public fun taxProductResourceCustomerDetails(
    block: TaxProductResourceCustomerDetails.Builder.() -> Unit,
): TaxProductResourceCustomerDetails = TaxProductResourceCustomerDetails.build(block)
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "TaxProductResourceCustomerDetails is missing required property '" + name + "'",
            )
