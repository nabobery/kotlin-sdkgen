...
public data class PortalFlowsFlowView(
    ...
    public val subscriptionUpdateConfirm: InlinePortalFlowsFlowSubscriptionUpdateConfirmX6ccda963? = null,
...
public class PortalFlowsFlow(
    ...
    public object Serializer : KSerializer<PortalFlowsFlow> {
        ...
        override fun deserialize(decoder: Decoder): PortalFlowsFlow {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("PortalFlowsFlow must be a JSON object")
            ...
            return PortalFlowsFlow(
                ...
                type = type,
                subscriptionCancel =
                    rawObject["subscription_cancel"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlinePortalFlowsFlowSubscriptionCancelX1d1da8b1?>(element)
                        }
                    },
                subscriptionUpdate =
                    rawObject["subscription_update"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlinePortalFlowsFlowSubscriptionUpdateX2b2e419e?>(element)
                        }
                    },
                subscriptionUpdateConfirm =
                    rawObject["subscription_update_confirm"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlinePortalFlowsFlowSubscriptionUpdateConfirmX6ccda963?>(
                                element,
                            )
                        }
                    },
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name] ?: throw SerializationException("PortalFlowsFlow is missing required property '" + name + "'")
