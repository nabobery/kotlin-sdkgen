        ...
        override fun deserialize(decoder: Decoder): ChargeOutcome {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("ChargeOutcome must be a JSON object")
            val type = json.decodeRequired<String>(rawObject, "type")
            return ChargeOutcome(
                type = type,
                adviceCode =
                    rawObject["advice_code"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineChargeOutcomeAdviceCodeX46e228bf?>(element)
                        }
                    },
                networkAdviceCode =
                ...
                networkStatus =
                    rawObject["network_status"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                sellerMessage =
                    rawObject["seller_message"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
