...
public class LineItemsTaxAmount(
    ...
    public object Serializer : KSerializer<LineItemsTaxAmount> {
        ...
        override fun deserialize(decoder: Decoder): LineItemsTaxAmount {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("LineItemsTaxAmount must be a JSON object")
            ...
            return LineItemsTaxAmount(
                ...
                rate = rate,
                taxabilityReason =
                    rawObject["taxability_reason"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineLineItemsTaxAmountTaxabilityReasonX796c6e30?>(element)
                        }
                    },
                taxableAmount =
...
public fun lineItemsTaxAmount(block: LineItemsTaxAmount.Builder.() -> Unit): LineItemsTaxAmount =
    LineItemsTaxAmount.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name] ?: throw SerializationException("LineItemsTaxAmount is missing required property '" + name + "'")
