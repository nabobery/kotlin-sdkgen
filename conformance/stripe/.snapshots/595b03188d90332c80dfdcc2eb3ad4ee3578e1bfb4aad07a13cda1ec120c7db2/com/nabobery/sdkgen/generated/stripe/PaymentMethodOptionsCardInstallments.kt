...
@Serializable
public data class PaymentMethodOptionsCardInstallmentsView(
...
public class PaymentMethodOptionsCardInstallments(
    ...
    public class Builder {
         ...
         * Installment plans that may be selected for this PaymentIntent.
         */
        public var availablePlans: List<PaymentMethodDetailsCardInstallmentsPlan>? = null
    ...
    public companion object {
        public fun build(block: Builder.() -> Unit): PaymentMethodOptionsCardInstallments =
            Builder().apply(block).build()
    }

    public object Serializer : KSerializer<PaymentMethodOptionsCardInstallments> {
        ...
        override fun deserialize(decoder: Decoder): PaymentMethodOptionsCardInstallments {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("PaymentMethodOptionsCardInstallments must be a JSON object")
            val enabled = json.decodeRequired<Boolean>(rawObject, "enabled")
            return PaymentMethodOptionsCardInstallments(
                enabled = enabled,
                availablePlans =
                    rawObject["available_plans"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<List<PaymentMethodDetailsCardInstallmentsPlan>?>(element)
                        }
                    },
                plan =
                    rawObject["plan"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlinePaymentMethodOptionsCardInstallmentsPlanXd122284b?>(
                                element,
                            )
                        }
                    },
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: PaymentMethodOptionsCardInstallments,
        ) {
...
public fun paymentMethodOptionsCardInstallments(
    block: PaymentMethodOptionsCardInstallments.Builder.() -> Unit,
): PaymentMethodOptionsCardInstallments = PaymentMethodOptionsCardInstallments.build(block)
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "PaymentMethodOptionsCardInstallments is missing required property '" + name + "'",
            )
