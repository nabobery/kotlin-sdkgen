        ...
        override fun deserialize(decoder: Decoder): IssuingCardShipping {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("IssuingCardShipping must be a JSON object")
            ...
            return IssuingCardShipping(
                ...
                type = type,
                addressValidation =
                    rawObject["address_validation"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineIssuingCardShippingAddressValidationX88177330?>(element)
                        }
                    },
                carrier =
                    rawObject["carrier"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineIssuingCardShippingCarrierXefe72726?>(element)
                        }
                    },
                customs =
                    rawObject["customs"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineIssuingCardShippingCustomsXc4db715f?>(element)
                        }
                    },
                eta =
                    rawObject["eta"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                requireSignature =
                    rawObject["require_signature"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                status =
                    rawObject["status"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineIssuingCardShippingStatusX83a95b24?>(element)
                        }
                    },
                trackingNumber =
                    rawObject["tracking_number"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                trackingUrl =
                    rawObject["tracking_url"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
...
public fun issuingCardShipping(block: IssuingCardShipping.Builder.() -> Unit): IssuingCardShipping =
    IssuingCardShipping.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name] ?: throw SerializationException("IssuingCardShipping is missing required property '" + name + "'")
