...
public class SubscriptionTransferData(
    ...
    public object Serializer : KSerializer<SubscriptionTransferData> {
        ...
        override fun deserialize(decoder: Decoder): SubscriptionTransferData {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("SubscriptionTransferData must be a JSON object")
            val destination =
                json.decodeRequired<InlineSubscriptionTransferDataDestinationXf63f04cd>(
                    rawObject,
                    "destination",
                )
            return SubscriptionTransferData(
                destination = destination,
                amountPercent =
                    rawObject["amount_percent"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: SubscriptionTransferData,
        ) {
...
public fun subscriptionTransferData(block: SubscriptionTransferData.Builder.() -> Unit): SubscriptionTransferData =
    SubscriptionTransferData.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name]
            ?: throw SerializationException("SubscriptionTransferData is missing required property '" + name + "'")
