...
public class FinancialConnectionsAccountOwner(
    ...
    public object Serializer : KSerializer<FinancialConnectionsAccountOwner> {
        ...
        override fun deserialize(decoder: Decoder): FinancialConnectionsAccountOwner {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("FinancialConnectionsAccountOwner must be a JSON object")
            val id = json.decodeRequired<String>(rawObject, "id")
            val name = json.decodeRequired<String>(rawObject, "name")
            val objectValue =
                json.decodeRequired<InlineFinancialConnectionsAccountOwnerObjectValueX05d0828b>(
                    rawObject,
                    "object",
                )
            ...
            return FinancialConnectionsAccountOwner(
                ...
                rawAddress =
                    rawObject["raw_address"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                refreshedAt =
                    rawObject["refreshed_at"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: FinancialConnectionsAccountOwner,
        ) {
...
public fun financialConnectionsAccountOwner(
    block: FinancialConnectionsAccountOwner.Builder.() -> Unit,
): FinancialConnectionsAccountOwner = FinancialConnectionsAccountOwner.build(block)
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "FinancialConnectionsAccountOwner is missing required property '" + name + "'",
            )
