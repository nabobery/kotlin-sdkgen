...
@Serializable
public data class IssuingCardholderView(
...
public class IssuingCardholder(
    ...
    public class Builder {
         ...
         */
        public var preferredLocales: List<InlineIssuingCardholderPreferredLocalesItemXdb230b01>? = null
    ...
    public object Serializer : KSerializer<IssuingCardholder> {
        ...
        override fun deserialize(decoder: Decoder): IssuingCardholder {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("IssuingCardholder must be a JSON object")
            ...
            return IssuingCardholder(
                ...
                type = type,
                company =
                    rawObject["company"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineIssuingCardholderCompanyX4ad567e6?>(element)
                        }
                    },
                email =
                ...
                individual =
                    rawObject["individual"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineIssuingCardholderIndividualXb90844ef?>(element)
                        }
                    },
                phoneNumber =
                    rawObject["phone_number"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                preferredLocales =
                    rawObject["preferred_locales"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<List<InlineIssuingCardholderPreferredLocalesItemXdb230b01>?>(
                                element,
                            )
                        }
                    },
                spendingControls =
                    rawObject["spending_controls"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineIssuingCardholderSpendingControlsX1c119ff6?>(element)
                        }
                    },
            )
        }
...
public fun issuingCardholder(block: IssuingCardholder.Builder.() -> Unit): IssuingCardholder =
    IssuingCardholder.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name] ?: throw SerializationException("IssuingCardholder is missing required property '" + name + "'")
