...
public data class InvoicePaymentMethodOptionsAcssDebitView(
    ...
    public val verificationMethod: InlineInvoicePaymentMethodOptionsAcssDebitVerificationMethodX087d0e38? = null,
)
...
public class InvoicePaymentMethodOptionsAcssDebit(
    ...
    public class Builder {
        ...
        public fun build(): InvoicePaymentMethodOptionsAcssDebit =
    ...
    public companion object {
        public fun build(block: Builder.() -> Unit): InvoicePaymentMethodOptionsAcssDebit =
            Builder().apply(block).build()
    }

    public object Serializer : KSerializer<InvoicePaymentMethodOptionsAcssDebit> {
        ...
        override fun deserialize(decoder: Decoder): InvoicePaymentMethodOptionsAcssDebit {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("InvoicePaymentMethodOptionsAcssDebit must be a JSON object")
            return InvoicePaymentMethodOptionsAcssDebit(
                mandateOptions =
                    rawObject["mandate_options"]?.let {
                        json.decodeFromJsonElement<InvoicePaymentMethodOptionsAcssDebitMandateOptions>(it)
                    },
                verificationMethod =
                    rawObject["verification_method"]?.let {
                        json
                            .decodeFromJsonElement<InlineInvoicePaymentMethodOptionsAcssDebitVerificationMethodX087d0e38>(
                                it,
                            )
                    },
            )
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: InvoicePaymentMethodOptionsAcssDebit,
        ) {
...
public fun invoicePaymentMethodOptionsAcssDebit(
    block: InvoicePaymentMethodOptionsAcssDebit.Builder.() -> Unit,
): InvoicePaymentMethodOptionsAcssDebit = InvoicePaymentMethodOptionsAcssDebit.build(block)
