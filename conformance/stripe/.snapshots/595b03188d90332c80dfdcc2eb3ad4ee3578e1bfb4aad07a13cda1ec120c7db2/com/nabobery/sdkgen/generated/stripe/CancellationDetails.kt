...
public class CancellationDetails(
    ...
    public class Builder {
        ...
        public fun build(): CancellationDetails =
            CancellationDetails(
                comment = comment,
                feedback = feedback,
                reason = reason,
        ...
        override fun deserialize(decoder: Decoder): CancellationDetails {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("CancellationDetails must be a JSON object")
            return CancellationDetails(
                comment =
                    rawObject["comment"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                feedback =
                    rawObject["feedback"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineCancellationDetailsFeedbackX1582a4cc?>(element)
                        }
                    },
                reason =
                    rawObject["reason"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineCancellationDetailsReasonX61ab0391?>(element)
                        }
                    },
            )
        }
...
public fun cancellationDetails(block: CancellationDetails.Builder.() -> Unit): CancellationDetails =
    CancellationDetails.build(block)
