...
public class IssuingDisputeEvidence(
    ...
    public object Serializer : KSerializer<IssuingDisputeEvidence> {
        ...
        override fun deserialize(decoder: Decoder): IssuingDisputeEvidence {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("IssuingDisputeEvidence must be a JSON object")
            val reason = json.decodeRequired<InlineIssuingDisputeEvidenceReasonXa0313758>(rawObject, "reason")
            return IssuingDisputeEvidence(
                reason = reason,
                canceled =
                    rawObject["canceled"]?.let {
                        json.decodeFromJsonElement<IssuingDisputeCanceledEvidence>(
                            it,
                        )
                    },
                duplicate =
                    rawObject["duplicate"]?.let {
                        json.decodeFromJsonElement<IssuingDisputeDuplicateEvidence>(
                            it,
                        )
                    },
                fraudulent =
                    rawObject["fraudulent"]?.let {
                        json
                            .decodeFromJsonElement<IssuingDisputeFraudulentEvidence>(
                                it,
                            )
                    },
                merchandiseNotAsDescribed =
                    rawObject["merchandise_not_as_described"]?.let {
                        json.decodeFromJsonElement<IssuingDisputeMerchandiseNotAsDescribedEvidence>(it)
                    },
                noValidAuthorization =
                    rawObject["no_valid_authorization"]?.let {
                        json.decodeFromJsonElement<IssuingDisputeNoValidAuthorizationEvidence>(it)
                    },
                notReceived =
                    rawObject["not_received"]?.let {
                        json.decodeFromJsonElement<IssuingDisputeNotReceivedEvidence>(
                            it,
                        )
                    },
                other =
                    rawObject["other"]?.let {
                        json.decodeFromJsonElement<IssuingDisputeOtherEvidence>(
                            it,
                        )
                    },
                serviceNotAsDescribed =
                    rawObject["service_not_as_described"]?.let {
                        json.decodeFromJsonElement<IssuingDisputeServiceNotAsDescribedEvidence>(it)
                    },
            )
        }
...
public fun issuingDisputeEvidence(block: IssuingDisputeEvidence.Builder.() -> Unit): IssuingDisputeEvidence =
    IssuingDisputeEvidence.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name] ?: throw SerializationException("IssuingDisputeEvidence is missing required property '" + name + "'")
