...
public class ReceivedPaymentMethodDetailsFinancialAccount(
    ...
    public companion object {
        public fun build(block: Builder.() -> Unit): ReceivedPaymentMethodDetailsFinancialAccount =
            Builder().apply(block).build()
    }

    public object Serializer : KSerializer<ReceivedPaymentMethodDetailsFinancialAccount> {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException(
                        "ReceivedPaymentMethodDetailsFinancialAccount must be a JSON object",
                    )
            val id = json.decodeRequired<String>(rawObject, "id")
            val network =
                json.decodeRequired<InlineReceivedPaymentMethodDetailsFinancialAccountNetworkX4044139f>(
                    rawObject,
                    "network",
                )
            return ReceivedPaymentMethodDetailsFinancialAccount(
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: ReceivedPaymentMethodDetailsFinancialAccount,
        ) {
...
public fun receivedPaymentMethodDetailsFinancialAccount(
    block: ReceivedPaymentMethodDetailsFinancialAccount.Builder.() -> Unit,
): ReceivedPaymentMethodDetailsFinancialAccount = ReceivedPaymentMethodDetailsFinancialAccount.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "ReceivedPaymentMethodDetailsFinancialAccount is missing required property '" + name + "'",
            )
