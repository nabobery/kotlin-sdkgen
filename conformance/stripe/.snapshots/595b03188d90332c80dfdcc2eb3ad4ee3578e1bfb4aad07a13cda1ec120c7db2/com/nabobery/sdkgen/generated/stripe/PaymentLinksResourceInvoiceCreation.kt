...
public class PaymentLinksResourceInvoiceCreation(
    ...
    public companion object {
        public fun build(block: Builder.() -> Unit): PaymentLinksResourceInvoiceCreation =
    ...
    public object Serializer : KSerializer<PaymentLinksResourceInvoiceCreation> {
        ...
        override fun deserialize(decoder: Decoder): PaymentLinksResourceInvoiceCreation {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("PaymentLinksResourceInvoiceCreation must be a JSON object")
            val enabled = json.decodeRequired<Boolean>(rawObject, "enabled")
            return PaymentLinksResourceInvoiceCreation(
                enabled = enabled,
                invoiceData =
                    rawObject["invoice_data"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlinePaymentLinksResourceInvoiceCreationInvoiceDataX2811de7a?>(
                                element,
                            )
                        }
                    },
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: PaymentLinksResourceInvoiceCreation,
        ) {
...
public fun paymentLinksResourceInvoiceCreation(
    block: PaymentLinksResourceInvoiceCreation.Builder.() -> Unit,
): PaymentLinksResourceInvoiceCreation = PaymentLinksResourceInvoiceCreation.build(block)
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "PaymentLinksResourceInvoiceCreation is missing required property '" + name + "'",
            )
