...
public class SubscriptionsResourcePauseCollection(
    ...
    public companion object {
        public fun build(block: Builder.() -> Unit): SubscriptionsResourcePauseCollection =
    ...
    public object Serializer : KSerializer<SubscriptionsResourcePauseCollection> {
        ...
        override fun deserialize(decoder: Decoder): SubscriptionsResourcePauseCollection {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("SubscriptionsResourcePauseCollection must be a JSON object")
            val behavior =
                json.decodeRequired<InlineSubscriptionsResourcePauseCollectionBehaviorX60bc805d>(
                    rawObject,
                    "behavior",
                )
            return SubscriptionsResourcePauseCollection(
                behavior = behavior,
                resumesAt =
                    rawObject["resumes_at"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: SubscriptionsResourcePauseCollection,
        ) {
...
public fun subscriptionsResourcePauseCollection(
    block: SubscriptionsResourcePauseCollection.Builder.() -> Unit,
): SubscriptionsResourcePauseCollection = SubscriptionsResourcePauseCollection.build(block)
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "SubscriptionsResourcePauseCollection is missing required property '" + name + "'",
            )
