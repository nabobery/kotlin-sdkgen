...
public class BalanceSettingsResourcePayments(
    ...
    public object Serializer : KSerializer<BalanceSettingsResourcePayments> {
        ...
        override fun deserialize(decoder: Decoder): BalanceSettingsResourcePayments {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("BalanceSettingsResourcePayments must be a JSON object")
            val settlementTiming =
                json.decodeRequired<BalanceSettingsResourceSettlementTiming>(
                    rawObject,
                    "settlement_timing",
                )
            return BalanceSettingsResourcePayments(
                ...
                debitNegativeBalances =
                    rawObject["debit_negative_balances"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                payouts =
                    rawObject["payouts"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineBalanceSettingsResourcePaymentsPayoutsX589c9ccb?>(element)
                        }
                    },
            )
        }
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: BalanceSettingsResourcePayments,
        ) {
...
public fun balanceSettingsResourcePayments(
    block: BalanceSettingsResourcePayments.Builder.() -> Unit,
): BalanceSettingsResourcePayments = BalanceSettingsResourcePayments.build(block)
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "BalanceSettingsResourcePayments is missing required property '" + name + "'",
            )
