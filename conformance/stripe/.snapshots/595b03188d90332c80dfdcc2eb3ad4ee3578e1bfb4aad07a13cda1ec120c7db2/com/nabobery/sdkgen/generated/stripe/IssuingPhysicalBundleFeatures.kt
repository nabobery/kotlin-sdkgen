...
public class IssuingPhysicalBundleFeatures(
    ...
    public object Serializer : KSerializer<IssuingPhysicalBundleFeatures> {
        ...
        override fun deserialize(decoder: Decoder): IssuingPhysicalBundleFeatures {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("IssuingPhysicalBundleFeatures must be a JSON object")
            val cardLogo =
                json.decodeRequired<InlineIssuingPhysicalBundleFeaturesCardLogoX39f3dc2e>(
                    rawObject,
                    "card_logo",
                )
            val carrierText =
                json.decodeRequired<InlineIssuingPhysicalBundleFeaturesCarrierTextXdf9185c4>(
                    rawObject,
                    "carrier_text",
                )
            val secondLine =
                json.decodeRequired<InlineIssuingPhysicalBundleFeaturesSecondLineX6cb11ac2>(
                    rawObject,
                    "second_line",
                )
            return IssuingPhysicalBundleFeatures(
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: IssuingPhysicalBundleFeatures,
        ) {
...
public fun issuingPhysicalBundleFeatures(
    block: IssuingPhysicalBundleFeatures.Builder.() -> Unit,
): IssuingPhysicalBundleFeatures = IssuingPhysicalBundleFeatures.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name]
            ?: throw SerializationException("IssuingPhysicalBundleFeatures is missing required property '" + name + "'")
