...
public class PaymentMethodOptionsAffirm(
    ...
    public class Builder {
        ...
        public fun build(): PaymentMethodOptionsAffirm =
    ...
    public object Serializer : KSerializer<PaymentMethodOptionsAffirm> {
        ...
        override fun deserialize(decoder: Decoder): PaymentMethodOptionsAffirm {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("PaymentMethodOptionsAffirm must be a JSON object")
            return PaymentMethodOptionsAffirm(
                captureMethod =
                    rawObject["capture_method"]?.let {
                        json.decodeFromJsonElement<InlinePaymentMethodOptionsAffirmCaptureMethodXbd4539f3>(it)
                    },
                preferredLocale = rawObject["preferred_locale"]?.let { json.decodeFromJsonElement<String>(it) },
                setupFutureUsage =
                    rawObject["setup_future_usage"]?.let {
                        json.decodeFromJsonElement<InlinePaymentMethodOptionsAffirmSetupFutureUsageX472c5fde>(it)
                    },
            )
        }
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: PaymentMethodOptionsAffirm,
        ) {
...
public fun paymentMethodOptionsAffirm(
    block: PaymentMethodOptionsAffirm.Builder.() -> Unit,
): PaymentMethodOptionsAffirm = PaymentMethodOptionsAffirm.build(block)
