...
public class PaymentFlowsAmountDetails(
    ...
    public class Builder {
        ...
        public fun build(): PaymentFlowsAmountDetails =
    ...
    public object Serializer : KSerializer<PaymentFlowsAmountDetails> {
        ...
        override fun deserialize(decoder: Decoder): PaymentFlowsAmountDetails {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("PaymentFlowsAmountDetails must be a JSON object")
            return PaymentFlowsAmountDetails(
                discountAmount = rawObject["discount_amount"]?.let { json.decodeFromJsonElement<Int>(it) },
                error =
                    rawObject["error"]?.let {
                        json.decodeFromJsonElement<PaymentFlowsAmountDetailsResourceError>(
                            it,
                        )
                    },
                lineItems =
                    rawObject["line_items"]?.let {
                        json.decodeFromJsonElement<InlinePaymentFlowsAmountDetailsLineItemsXce22638c>(it)
                    },
                shipping =
                    rawObject["shipping"]?.let {
                        json.decodeFromJsonElement<PaymentFlowsAmountDetailsResourceShipping>(
                            it,
                        )
                    },
                tax =
                    rawObject["tax"]?.let {
                        json.decodeFromJsonElement<PaymentFlowsAmountDetailsResourceTax>(
                            it,
                        )
                    },
                tip =
                    rawObject["tip"]?.let {
                        json.decodeFromJsonElement<PaymentFlowsAmountDetailsClientResourceTip>(
                            it,
                        )
                    },
            )
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: PaymentFlowsAmountDetails,
        ) {
...
public fun paymentFlowsAmountDetails(block: PaymentFlowsAmountDetails.Builder.() -> Unit): PaymentFlowsAmountDetails =
    PaymentFlowsAmountDetails.build(block)
