...
public class SubscriptionsResourceBillingCycleAnchorConfig(
    ...
    public companion object {
        public fun build(block: Builder.() -> Unit): SubscriptionsResourceBillingCycleAnchorConfig =
            Builder().apply(block).build()
    }

    public object Serializer : KSerializer<SubscriptionsResourceBillingCycleAnchorConfig> {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException(
                        "SubscriptionsResourceBillingCycleAnchorConfig must be a JSON object",
                    )
            val dayOfMonth = json.decodeRequired<Int>(rawObject, "day_of_month")
            return SubscriptionsResourceBillingCycleAnchorConfig(
                dayOfMonth = dayOfMonth,
                hour =
                    rawObject["hour"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: SubscriptionsResourceBillingCycleAnchorConfig,
        ) {
...
public fun subscriptionsResourceBillingCycleAnchorConfig(
    block: SubscriptionsResourceBillingCycleAnchorConfig.Builder.() -> Unit,
): SubscriptionsResourceBillingCycleAnchorConfig = SubscriptionsResourceBillingCycleAnchorConfig.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "SubscriptionsResourceBillingCycleAnchorConfig is missing required property '" + name + "'",
            )
