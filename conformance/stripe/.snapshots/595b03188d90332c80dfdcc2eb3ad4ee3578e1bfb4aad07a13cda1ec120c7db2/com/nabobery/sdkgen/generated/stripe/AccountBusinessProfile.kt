...
@Serializable
public data class AccountBusinessProfileView(
...
public class AccountBusinessProfile(
    ...
    public class Builder {
        ...
        public fun build(): AccountBusinessProfile =
    ...
    public object Serializer : KSerializer<AccountBusinessProfile> {
        ...
        override fun deserialize(decoder: Decoder): AccountBusinessProfile {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("AccountBusinessProfile must be a JSON object")
            return AccountBusinessProfile(
                annualRevenue =
                    rawObject["annual_revenue"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineAccountBusinessProfileAnnualRevenueX7589bbd6?>(element)
                        }
                    },
                estimatedWorkerCount =
                    rawObject["estimated_worker_count"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                minorityOwnedBusinessDesignation =
                    rawObject["minority_owned_business_designation"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                            ...
                            json
                                .decodeFromJsonElement<List<InlineAccountBusinessProfileMinorityOwnedBusinessDesignationItemX7ae94485>?>(
                                    element,
                                )
                        }
                    },
                monthlyEstimatedRevenue =
                    rawObject["monthly_estimated_revenue"]?.let {
                        json.decodeFromJsonElement<AccountMonthlyEstimatedRevenue>(it)
                    },
                name =
                ...
                supportAddress =
                    rawObject["support_address"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineAccountBusinessProfileSupportAddressX4693a646?>(element)
                        }
                    },
                supportEmail =
                    rawObject["support_email"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                supportPhone =
                    rawObject["support_phone"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                supportUrl =
                    rawObject["support_url"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
...
public fun accountBusinessProfile(block: AccountBusinessProfile.Builder.() -> Unit): AccountBusinessProfile =
    AccountBusinessProfile.build(block)
