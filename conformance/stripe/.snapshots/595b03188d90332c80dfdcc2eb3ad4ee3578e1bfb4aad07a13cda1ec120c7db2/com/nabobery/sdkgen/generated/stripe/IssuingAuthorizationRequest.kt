...
public class IssuingAuthorizationRequest(
    ...
    public object Serializer : KSerializer<IssuingAuthorizationRequest> {
        ...
        override fun deserialize(decoder: Decoder): IssuingAuthorizationRequest {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("IssuingAuthorizationRequest must be a JSON object")
            ...
            return IssuingAuthorizationRequest(
                ...
                amountDetails =
                    rawObject["amount_details"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineIssuingAuthorizationRequestAmountDetailsX5f1eb6cd?>(
                                element,
                            )
                        }
                    },
                ...
                networkRiskScore =
                    rawObject["network_risk_score"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                reasonMessage =
                    rawObject["reason_message"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                requestedAt =
                    rawObject["requested_at"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: IssuingAuthorizationRequest,
        ) {
...
public fun issuingAuthorizationRequest(
    block: IssuingAuthorizationRequest.Builder.() -> Unit,
): IssuingAuthorizationRequest = IssuingAuthorizationRequest.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name]
            ?: throw SerializationException("IssuingAuthorizationRequest is missing required property '" + name + "'")
