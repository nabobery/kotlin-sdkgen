...
@Serializable
public data class UsBankAccountNetworksView(
...
public class UsBankAccountNetworks(
    ...
    public object Serializer : KSerializer<UsBankAccountNetworks> {
        ...
        override fun deserialize(decoder: Decoder): UsBankAccountNetworks {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("UsBankAccountNetworks must be a JSON object")
            val supported =
                json.decodeRequired<List<InlineUsBankAccountNetworksSupportedItemXc9194e8b>>(
                    rawObject,
                    "supported",
                )
            return UsBankAccountNetworks(
                supported = supported,
                preferred =
                    rawObject["preferred"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
...
public fun usBankAccountNetworks(block: UsBankAccountNetworks.Builder.() -> Unit): UsBankAccountNetworks =
    UsBankAccountNetworks.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name] ?: throw SerializationException("UsBankAccountNetworks is missing required property '" + name + "'")
