...
public class CheckoutBoletoPaymentMethodOptions(
     ...
     */
    public val setupFutureUsage: InlineCheckoutBoletoPaymentMethodOptionsSetupFutureUsageXb117922a? = null,
) {
    ...
    public object Serializer : KSerializer<CheckoutBoletoPaymentMethodOptions> {
        ...
        override fun deserialize(decoder: Decoder): CheckoutBoletoPaymentMethodOptions {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("CheckoutBoletoPaymentMethodOptions must be a JSON object")
            val expiresAfterDays = json.decodeRequired<Int>(rawObject, "expires_after_days")
            return CheckoutBoletoPaymentMethodOptions(
                expiresAfterDays = expiresAfterDays,
                setupFutureUsage =
                    rawObject["setup_future_usage"]?.let {
                        json.decodeFromJsonElement<InlineCheckoutBoletoPaymentMethodOptionsSetupFutureUsageXb117922a>(
                            it,
                        )
                    },
            )
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: CheckoutBoletoPaymentMethodOptions,
        ) {
...
public fun checkoutBoletoPaymentMethodOptions(
    block: CheckoutBoletoPaymentMethodOptions.Builder.() -> Unit,
): CheckoutBoletoPaymentMethodOptions = CheckoutBoletoPaymentMethodOptions.build(block)
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "CheckoutBoletoPaymentMethodOptions is missing required property '" + name + "'",
            )
