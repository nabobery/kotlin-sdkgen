...
public class TaxProductRegistrationsResourceCountryOptionsEurope(
    ...
    public companion object {
        public fun build(block: Builder.() -> Unit): TaxProductRegistrationsResourceCountryOptionsEurope =
            Builder().apply(block).build()
    }

    public object Serializer : KSerializer<TaxProductRegistrationsResourceCountryOptionsEurope> {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException(
                        "TaxProductRegistrationsResourceCountryOptionsEurope must be a JSON object",
                    )
            val type =
                json.decodeRequired<InlineTaxProductRegistrationsResourceCountryOptionsEuropeTypeXdf9315c6>(
                    rawObject,
                    "type",
                )
            return TaxProductRegistrationsResourceCountryOptionsEurope(
                ...
                standard =
                    rawObject["standard"]?.let {
                        json.decodeFromJsonElement<TaxProductRegistrationsResourceCountryOptionsEuStandard>(it)
                    },
            )
        }
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: TaxProductRegistrationsResourceCountryOptionsEurope,
        ) {
...
public fun taxProductRegistrationsResourceCountryOptionsEurope(
    block: TaxProductRegistrationsResourceCountryOptionsEurope.Builder.() -> Unit,
): TaxProductRegistrationsResourceCountryOptionsEurope =
    TaxProductRegistrationsResourceCountryOptionsEurope.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "TaxProductRegistrationsResourceCountryOptionsEurope is missing required property '" + name + "'",
            )
