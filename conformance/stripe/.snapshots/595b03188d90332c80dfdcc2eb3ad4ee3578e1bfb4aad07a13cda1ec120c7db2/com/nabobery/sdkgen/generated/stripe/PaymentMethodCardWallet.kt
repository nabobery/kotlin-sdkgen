        ...
        override fun deserialize(decoder: Decoder): PaymentMethodCardWallet {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("PaymentMethodCardWallet must be a JSON object")
            val type = json.decodeRequired<InlinePaymentMethodCardWalletTypeXa8edbe9b>(rawObject, "type")
            return PaymentMethodCardWallet(
                type = type,
                amexExpressCheckout =
                    rawObject["amex_express_checkout"]?.let {
                ...
                dynamicLast4 =
                    rawObject["dynamic_last4"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                masterpass =
                    rawObject["masterpass"]?.let {
                        json
                            .decodeFromJsonElement<PaymentMethodCardWalletMasterpass>(
                                it,
                            )
                    },
                samsungPay = rawObject["samsung_pay"]?.let { json.decodeFromJsonElement<JsonObject>(it) },
                visaCheckout =
                    rawObject["visa_checkout"]?.let {
                        json
                            .decodeFromJsonElement<PaymentMethodCardWalletVisaCheckout>(
                                it,
                            )
                    },
            )
...
public fun paymentMethodCardWallet(block: PaymentMethodCardWallet.Builder.() -> Unit): PaymentMethodCardWallet =
    PaymentMethodCardWallet.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name] ?: throw SerializationException("PaymentMethodCardWallet is missing required property '" + name + "'")
