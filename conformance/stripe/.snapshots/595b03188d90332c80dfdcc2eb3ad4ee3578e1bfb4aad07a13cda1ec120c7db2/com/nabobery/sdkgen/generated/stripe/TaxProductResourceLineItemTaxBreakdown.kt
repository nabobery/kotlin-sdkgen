...
public class TaxProductResourceLineItemTaxBreakdown(
     ...
     */
    public val taxabilityReason: InlineTaxProductResourceLineItemTaxBreakdownTaxabilityReasonX2de45fc8,
    /**
     ...
     */
    public val taxRateDetails: InlineTaxProductResourceLineItemTaxBreakdownTaxRateDetailsXff548130? = null,
) {
    ...
    public companion object {
        public fun build(block: Builder.() -> Unit): TaxProductResourceLineItemTaxBreakdown =
            Builder().apply(block).build()
    }

    public object Serializer : KSerializer<TaxProductResourceLineItemTaxBreakdown> {
        ...
        override fun deserialize(decoder: Decoder): TaxProductResourceLineItemTaxBreakdown {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("TaxProductResourceLineItemTaxBreakdown must be a JSON object")
            val amount = json.decodeRequired<Int>(rawObject, "amount")
            val jurisdiction = json.decodeRequired<TaxProductResourceJurisdiction>(rawObject, "jurisdiction")
            val sourcing =
                json.decodeRequired<InlineTaxProductResourceLineItemTaxBreakdownSourcingX2d203456>(
                    rawObject,
                    "sourcing",
                )
            val taxabilityReason =
                json.decodeRequired<InlineTaxProductResourceLineItemTaxBreakdownTaxabilityReasonX2de45fc8>(
                    rawObject,
                    "taxability_reason",
                )
                ...
                taxRateDetails =
                    rawObject["tax_rate_details"]?.let { element ->
                        if (element ==
                            ...
                            json
                                .decodeFromJsonElement<InlineTaxProductResourceLineItemTaxBreakdownTaxRateDetailsXff548130?>(
                                    element,
                                )
                        }
                    },
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: TaxProductResourceLineItemTaxBreakdown,
        ) {
...
public fun taxProductResourceLineItemTaxBreakdown(
    block: TaxProductResourceLineItemTaxBreakdown.Builder.() -> Unit,
): TaxProductResourceLineItemTaxBreakdown = TaxProductResourceLineItemTaxBreakdown.build(block)
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "TaxProductResourceLineItemTaxBreakdown is missing required property '" + name + "'",
            )
