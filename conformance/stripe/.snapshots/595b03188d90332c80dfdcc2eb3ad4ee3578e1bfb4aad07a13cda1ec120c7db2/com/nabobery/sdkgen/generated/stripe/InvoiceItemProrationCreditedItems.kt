...
public class InvoiceItemProrationCreditedItems(
    ...
    public object Serializer : KSerializer<InvoiceItemProrationCreditedItems> {
        ...
        override fun deserialize(decoder: Decoder): InvoiceItemProrationCreditedItems {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("InvoiceItemProrationCreditedItems must be a JSON object")
            ...
            return InvoiceItemProrationCreditedItems(
                ...
                invoiceLineItemDetails =
                    rawObject["invoice_line_item_details"]?.let {
                        json.decodeFromJsonElement<CreditedItemsInvoiceLineItems>(it)
                    },
            )
        }
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: InvoiceItemProrationCreditedItems,
        ) {
...
public fun invoiceItemProrationCreditedItems(
    block: InvoiceItemProrationCreditedItems.Builder.() -> Unit,
): InvoiceItemProrationCreditedItems = InvoiceItemProrationCreditedItems.build(block)
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "InvoiceItemProrationCreditedItems is missing required property '" + name + "'",
            )
