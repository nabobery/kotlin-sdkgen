        ...
        override fun deserialize(decoder: Decoder): TaxIDsOwner {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("TaxIDsOwner must be a JSON object")
            val type = json.decodeRequired<InlineTaxIDsOwnerTypeXb1bf3e21>(rawObject, "type")
            return TaxIDsOwner(
                type = type,
                account =
                    rawObject["account"]?.let {
                        json.decodeFromJsonElement<InlineTaxIDsOwnerAccountX1491eced>(
                            it,
                        )
                    },
                application =
                    rawObject["application"]?.let {
                        json.decodeFromJsonElement<InlineTaxIDsOwnerApplicationXe5bdf86c>(
                            it,
                        )
                    },
                customer =
                    rawObject["customer"]?.let {
                        json
                            .decodeFromJsonElement<InlineTaxIDsOwnerCustomerXa297aa4e>(
                                it,
                            )
                    },
                customerAccount =
