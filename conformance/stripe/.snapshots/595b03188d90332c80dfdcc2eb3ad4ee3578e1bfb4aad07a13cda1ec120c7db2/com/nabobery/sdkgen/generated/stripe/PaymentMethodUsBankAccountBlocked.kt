...
public class PaymentMethodUsBankAccountBlocked(
    ...
    public class Builder {
        ...
        public fun build(): PaymentMethodUsBankAccountBlocked =
    ...
    public object Serializer : KSerializer<PaymentMethodUsBankAccountBlocked> {
        ...
        override fun deserialize(decoder: Decoder): PaymentMethodUsBankAccountBlocked {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("PaymentMethodUsBankAccountBlocked must be a JSON object")
            return PaymentMethodUsBankAccountBlocked(
                networkCode =
                    rawObject["network_code"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlinePaymentMethodUsBankAccountBlockedNetworkCodeX0a662ce1?>(
                                element,
                            )
                        }
                    },
                reason =
                    rawObject["reason"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlinePaymentMethodUsBankAccountBlockedReasonX9256636c?>(element)
                        }
                    },
            )
        }
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: PaymentMethodUsBankAccountBlocked,
        ) {
...
public fun paymentMethodUsBankAccountBlocked(
    block: PaymentMethodUsBankAccountBlocked.Builder.() -> Unit,
): PaymentMethodUsBankAccountBlocked = PaymentMethodUsBankAccountBlocked.build(block)
