...
public class PortalFlowsFlowSubscriptionCancel(
    ...
    public object Serializer : KSerializer<PortalFlowsFlowSubscriptionCancel> {
        ...
        override fun deserialize(decoder: Decoder): PortalFlowsFlowSubscriptionCancel {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("PortalFlowsFlowSubscriptionCancel must be a JSON object")
            val subscription = json.decodeRequired<String>(rawObject, "subscription")
            return PortalFlowsFlowSubscriptionCancel(
                subscription = subscription,
                retention =
                    rawObject["retention"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlinePortalFlowsFlowSubscriptionCancelRetentionX088b5b60?>(
                                element,
                            )
                        }
                    },
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: PortalFlowsFlowSubscriptionCancel,
        ) {
...
public fun portalFlowsFlowSubscriptionCancel(
    block: PortalFlowsFlowSubscriptionCancel.Builder.() -> Unit,
): PortalFlowsFlowSubscriptionCancel = PortalFlowsFlowSubscriptionCancel.build(block)
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "PortalFlowsFlowSubscriptionCancel is missing required property '" + name + "'",
            )
