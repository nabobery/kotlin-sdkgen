...
public data class TreasuryTransactionView(
    ...
    public val statusTransitions: TreasuryTransactionsResourceAbstractTransactionResourceStatusTransitions,
)
        ...
        override fun deserialize(decoder: Decoder): TreasuryTransaction {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("TreasuryTransaction must be a JSON object")
            val amount = json.decodeRequired<Int>(rawObject, "amount")
            val balanceImpact =
                json.decodeRequired<TreasuryTransactionsResourceBalanceImpact>(
                    rawObject,
                    "balance_impact",
                )
            ...
            val statusTransitions =
                json
                    .decodeRequired<TreasuryTransactionsResourceAbstractTransactionResourceStatusTransitions>(
                        rawObject,
                        "status_transitions",
                    )
            return TreasuryTransaction(
                ...
                statusTransitions = statusTransitions,
                entries =
                    rawObject["entries"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineTreasuryTransactionEntriesX6cf91389?>(element)
                        }
                    },
                flow =
                ...
                flowDetails =
                    rawObject["flow_details"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineTreasuryTransactionFlowDetailsX79bb758a?>(element)
                        }
                    },
            )
        }
...
public fun treasuryTransaction(block: TreasuryTransaction.Builder.() -> Unit): TreasuryTransaction =
    TreasuryTransaction.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name] ?: throw SerializationException("TreasuryTransaction is missing required property '" + name + "'")
