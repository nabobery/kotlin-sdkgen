...
public class SourceTransaction(
    ...
    public object Serializer : KSerializer<SourceTransaction> {
        ...
        override fun deserialize(decoder: Decoder): SourceTransaction {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("SourceTransaction must be a JSON object")
            ...
            return SourceTransaction(
                ...
                achCreditTransfer =
                    rawObject["ach_credit_transfer"]?.let {
                        json.decodeFromJsonElement<SourceTransactionAchCreditTransferData>(it)
                    },
                chfCreditTransfer =
                    rawObject["chf_credit_transfer"]?.let {
                        json.decodeFromJsonElement<SourceTransactionChfCreditTransferData>(it)
                    },
                gbpCreditTransfer =
                    rawObject["gbp_credit_transfer"]?.let {
                        json.decodeFromJsonElement<SourceTransactionGbpCreditTransferData>(it)
                    },
                paperCheck =
                    rawObject["paper_check"]?.let {
                        json
                            .decodeFromJsonElement<SourceTransactionPaperCheckData>(
                                it,
                            )
                    },
                sepaCreditTransfer =
                    rawObject["sepa_credit_transfer"]?.let {
                        json.decodeFromJsonElement<SourceTransactionSepaCreditTransferData>(it)
                    },
            )
        }
...
public fun sourceTransaction(block: SourceTransaction.Builder.() -> Unit): SourceTransaction =
    SourceTransaction.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name] ?: throw SerializationException("SourceTransaction is missing required property '" + name + "'")
