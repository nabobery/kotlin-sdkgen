        ...
        override fun deserialize(decoder: Decoder): GelatoDocumentReport {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("GelatoDocumentReport must be a JSON object")
            ...
            return GelatoDocumentReport(
                ...
                address =
                    rawObject["address"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineGelatoDocumentReportAddressXf1c23419?>(element)
                        }
                    },
                dob =
                    rawObject["dob"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineGelatoDocumentReportDobX15024453?>(element)
                        }
                    },
                error =
                    rawObject["error"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineGelatoDocumentReportErrorX9e751596?>(element)
                        }
                    },
                expirationDate =
                    rawObject["expiration_date"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineGelatoDocumentReportExpirationDateX3a0a05a9?>(element)
                        }
                    },
                files =
                    rawObject["files"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                issuedDate =
                    rawObject["issued_date"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineGelatoDocumentReportIssuedDateX5d222158?>(element)
                        }
                    },
                issuingCountry =
                    rawObject["issuing_country"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                sex =
                    rawObject["sex"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineGelatoDocumentReportSexX65e79c91?>(element)
                        }
                    },
                type =
                    rawObject["type"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineGelatoDocumentReportTypeXef41d7e5?>(element)
                        }
                    },
                unparsedPlaceOfBirth =
                    rawObject["unparsed_place_of_birth"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                unparsedSex =
                    rawObject["unparsed_sex"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
...
public fun gelatoDocumentReport(block: GelatoDocumentReport.Builder.() -> Unit): GelatoDocumentReport =
    GelatoDocumentReport.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name] ?: throw SerializationException("GelatoDocumentReport is missing required property '" + name + "'")
