...
@Serializable
public data class TransferView(
        ...
        override fun deserialize(decoder: Decoder): Transfer {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("Transfer must be a JSON object")
                ...
                balanceTransaction =
                    rawObject["balance_transaction"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineTransferBalanceTransactionX0d42b38e?>(element)
                        }
                    },
                description =
                ...
                destination =
                    rawObject["destination"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineTransferDestinationX606a81c1?>(element)
                        }
                    },
                destinationPayment =
                    rawObject["destination_payment"]?.let {
                        json.decodeFromJsonElement<InlineTransferDestinationPaymentX1562c10c>(it)
                    },
                sourceTransaction =
                    rawObject["source_transaction"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineTransferSourceTransactionX7175398c?>(element)
                        }
                    },
