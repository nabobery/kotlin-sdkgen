...
public class AccountInvoicesSettings(
     ...
     * is finalized.
     */
    public val defaultAccountTaxIds: List<InlineAccountInvoicesSettingsDefaultAccountTaxIdsItemX7a5634e4>? = null,
    /**
     ...
     */
    public val hostedPaymentMethodSave: InlineAccountInvoicesSettingsHostedPaymentMethodSaveXdc5ae523? = null,
) {
    public class Builder {
        ...
        public fun build(): AccountInvoicesSettings =
    ...
    public object Serializer : KSerializer<AccountInvoicesSettings> {
        ...
        override fun deserialize(decoder: Decoder): AccountInvoicesSettings {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("AccountInvoicesSettings must be a JSON object")
            return AccountInvoicesSettings(
                defaultAccountTaxIds =
                    rawObject["default_account_tax_ids"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                            ...
                            json
                                .decodeFromJsonElement<List<InlineAccountInvoicesSettingsDefaultAccountTaxIdsItemX7a5634e4>?>(
                                    element,
                                )
                        }
                    },
                hostedPaymentMethodSave =
                    rawObject["hosted_payment_method_save"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineAccountInvoicesSettingsHostedPaymentMethodSaveXdc5ae523?>(
                                element,
                            )
                        }
                    },
...
public fun accountInvoicesSettings(block: AccountInvoicesSettings.Builder.() -> Unit): AccountInvoicesSettings =
    AccountInvoicesSettings.build(block)
