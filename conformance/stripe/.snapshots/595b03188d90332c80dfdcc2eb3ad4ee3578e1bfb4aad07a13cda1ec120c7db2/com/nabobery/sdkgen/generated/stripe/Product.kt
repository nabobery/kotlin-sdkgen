...
public class Product(
    ...
    public class Builder {
        ...
        public var marketingFeatures: List<ProductMarketingFeature>
            get() = requireNotNull(marketingFeaturesValue) { "marketingFeatures is required" }
            set(`value`) {
                marketingFeaturesValue = value
            }
    ...
    public object Serializer : KSerializer<Product> {
        ...
        override fun deserialize(decoder: Decoder): Product {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("Product must be a JSON object")
            ...
            return Product(
                ...
                updated = updated,
                defaultPrice =
                    rawObject["default_price"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineProductDefaultPriceX5e5c27b1?>(element)
                        }
                    },
                description =
                ...
                packageDimensions =
                    rawObject["package_dimensions"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineProductPackageDimensionsX11badd14?>(element)
                        }
                    },
                shippable =
                    rawObject["shippable"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                taxCode =
                    rawObject["tax_code"]?.let { element ->
                        if (element ==
                            ...
                            json.decodeFromJsonElement<InlineProductTaxCodeXef95fe7f?>(element)
                        }
                    },
                unitLabel =
                    rawObject["unit_label"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
