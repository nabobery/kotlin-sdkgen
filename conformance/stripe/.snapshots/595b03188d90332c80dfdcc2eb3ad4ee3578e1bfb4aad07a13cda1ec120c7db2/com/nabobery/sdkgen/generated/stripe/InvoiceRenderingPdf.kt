...
public class InvoiceRenderingPdf(
    ...
    public class Builder {
        ...
        public fun build(): InvoiceRenderingPdf =
            InvoiceRenderingPdf(
                pageSize = pageSize,
            )
    }
        ...
        override fun deserialize(decoder: Decoder): InvoiceRenderingPdf {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("InvoiceRenderingPdf must be a JSON object")
            return InvoiceRenderingPdf(
                pageSize =
                    rawObject["page_size"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlineInvoiceRenderingPdfPageSizeXa324d13f?>(element)
                        }
                    },
            )
        }
...
public fun invoiceRenderingPdf(block: InvoiceRenderingPdf.Builder.() -> Unit): InvoiceRenderingPdf =
    InvoiceRenderingPdf.build(block)
