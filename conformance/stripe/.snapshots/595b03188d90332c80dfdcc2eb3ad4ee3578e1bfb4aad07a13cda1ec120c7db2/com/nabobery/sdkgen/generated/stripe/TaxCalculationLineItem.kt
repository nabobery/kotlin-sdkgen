        ...
        override fun deserialize(decoder: Decoder): TaxCalculationLineItem {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("TaxCalculationLineItem must be a JSON object")
            ...
            val taxBehavior =
                json.decodeRequired<InlineTaxCalculationLineItemTaxBehaviorX254c641f>(
                    rawObject,
                    "tax_behavior",
                )
            ...
            return TaxCalculationLineItem(
                ...
                taxBreakdown =
                    rawObject["tax_breakdown"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<List<TaxProductResourceLineItemTaxBreakdown>?>(element)
                        }
                    },
            )
        }
...
public fun taxCalculationLineItem(block: TaxCalculationLineItem.Builder.() -> Unit): TaxCalculationLineItem =
    TaxCalculationLineItem.build(block)
...
private inline fun <reified T> Json.decodeRequired(
    ...
    val element =
        raw[name] ?: throw SerializationException("TaxCalculationLineItem is missing required property '" + name + "'")
