...
public class PaymentIntentAmountDetailsLineItem(
     ...
     */
    public val paymentMethodOptions: InlinePaymentIntentAmountDetailsLineItemPaymentMethodOptionsX51c30aed? = null,
    /**
    ...
    public object Serializer : KSerializer<PaymentIntentAmountDetailsLineItem> {
        ...
        override fun deserialize(decoder: Decoder): PaymentIntentAmountDetailsLineItem {
            ...
            val rawObject =
                jsonDecoder.decodeJsonElement() as? JsonObject
                    ?: throw SerializationException("PaymentIntentAmountDetailsLineItem must be a JSON object")
            val id = json.decodeRequired<String>(rawObject, "id")
            val objectValue =
                json.decodeRequired<InlinePaymentIntentAmountDetailsLineItemObjectValueX5064f062>(
                    rawObject,
                    "object",
                )
            ...
            return PaymentIntentAmountDetailsLineItem(
                ...
                unitCost = unitCost,
                discountAmount =
                    rawObject["discount_amount"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                paymentMethodOptions =
                    rawObject["payment_method_options"]?.let { element ->
                        if (element ==
                            ...
                            json
                                .decodeFromJsonElement<InlinePaymentIntentAmountDetailsLineItemPaymentMethodOptionsX51c30aed?>(
                                    element,
                                )
                        }
                    },
                productCode =
                    rawObject["product_code"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                ...
                tax =
                    rawObject["tax"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
                        } else {
                            json.decodeFromJsonElement<InlinePaymentIntentAmountDetailsLineItemTaxX754180e6?>(element)
                        }
                    },
                unitOfMeasure =
                    rawObject["unit_of_measure"]?.let { element ->
                        if (element ==
                            JsonNull
                        ) {
                            null
        ...
        override fun serialize(
            encoder: Encoder,
            `value`: PaymentIntentAmountDetailsLineItem,
        ) {
...
public fun paymentIntentAmountDetailsLineItem(
    block: PaymentIntentAmountDetailsLineItem.Builder.() -> Unit,
): PaymentIntentAmountDetailsLineItem = PaymentIntentAmountDetailsLineItem.build(block)
    ...
    val element =
        raw[name]
            ?: throw SerializationException(
                "PaymentIntentAmountDetailsLineItem is missing required property '" + name + "'",
            )
