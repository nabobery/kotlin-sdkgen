...
public class GelatoSessionLastError(
  ...
  public class Builder {
    ...
    public fun build(): GelatoSessionLastError = GelatoSessionLastError(
      code = code,
      reason = reason,
    )
  }
  ...
  internal object Serializer : KSerializer<GelatoSessionLastError> {
    ...
    override fun deserialize(decoder: Decoder): GelatoSessionLastError {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("GelatoSessionLastError must be a JSON object")
      return GelatoSessionLastError(
        code = rawObject["code"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineGelatoSessionLastErrorCodeX68f6f737?>(element) },
        reason = rawObject["reason"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: GelatoSessionLastError) {
...
public fun gelatoSessionLastError(block: GelatoSessionLastError.Builder.() -> Unit): GelatoSessionLastError = GelatoSessionLastError.build(block)
