...
public class ConnectAccountReference(
  ...
  internal object Serializer : KSerializer<ConnectAccountReference> {
    ...
    override fun deserialize(decoder: Decoder): ConnectAccountReference {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("ConnectAccountReference must be a JSON object")
      val type = json.decodeRequired<InlineConnectAccountReferenceTypeXc6224d9c>(rawObject, "type")
      return ConnectAccountReference(
        type = type,
        account = rawObject["account"]?.let { json.decodeFromJsonElement<InlineConnectAccountReferenceAccountXebc59099>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: ConnectAccountReference) {
...
public fun connectAccountReference(block: ConnectAccountReference.Builder.() -> Unit): ConnectAccountReference = ConnectAccountReference.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("ConnectAccountReference is missing required property '" + name + "'")
