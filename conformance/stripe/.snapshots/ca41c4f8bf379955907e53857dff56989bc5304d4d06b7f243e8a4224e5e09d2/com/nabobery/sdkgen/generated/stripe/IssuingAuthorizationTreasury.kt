...
@Serializable
public data class IssuingAuthorizationTreasuryView internal constructor(
...
public class IssuingAuthorizationTreasury(
  ...
  public class Builder {
    ...
    public var receivedCredits: List<String>
      get() = requireNotNull(receivedCreditsValue) { "receivedCredits is required" }.toList()
      set(`value`) {
        receivedCreditsValue = value.toList()
      }

    private var receivedDebitsValue: List<String>? = null

    public var receivedDebits: List<String>
      get() = requireNotNull(receivedDebitsValue) { "receivedDebits is required" }.toList()
      set(`value`) {
        receivedDebitsValue = value.toList()
      }
  ...
  internal object Serializer : KSerializer<IssuingAuthorizationTreasury> {
    ...
    override fun deserialize(decoder: Decoder): IssuingAuthorizationTreasury {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("IssuingAuthorizationTreasury must be a JSON object")
      ...
      return IssuingAuthorizationTreasury(
        receivedCredits = receivedCredits,
        receivedDebits = receivedDebits,
        transaction = rawObject["transaction"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: IssuingAuthorizationTreasury) {
...
public fun issuingAuthorizationTreasury(block: IssuingAuthorizationTreasury.Builder.() -> Unit): IssuingAuthorizationTreasury = IssuingAuthorizationTreasury.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("IssuingAuthorizationTreasury is missing required property '" + name + "'")
