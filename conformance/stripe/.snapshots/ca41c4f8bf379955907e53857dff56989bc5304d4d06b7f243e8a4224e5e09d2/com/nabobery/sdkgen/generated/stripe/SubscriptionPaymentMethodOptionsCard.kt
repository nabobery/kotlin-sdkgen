...
public class SubscriptionPaymentMethodOptionsCard(
  ...
  public class Builder {
    ...
    public fun build(): SubscriptionPaymentMethodOptionsCard = SubscriptionPaymentMethodOptionsCard(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): SubscriptionPaymentMethodOptionsCard = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<SubscriptionPaymentMethodOptionsCard> {
    ...
    override fun deserialize(decoder: Decoder): SubscriptionPaymentMethodOptionsCard {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("SubscriptionPaymentMethodOptionsCard must be a JSON object")
      return SubscriptionPaymentMethodOptionsCard(
        mandateOptions = rawObject["mandate_options"]?.let { json.decodeFromJsonElement<InvoiceMandateOptionsCard>(it) },
        network = rawObject["network"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSubscriptionPaymentMethodOptionsCardNetworkXe9bf37aa?>(element) },
        requestThreeDSecure = rawObject["request_three_d_secure"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineSubscriptionPaymentMethodOptionsCardRequestThreeDSecureX5bc18e0a?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: SubscriptionPaymentMethodOptionsCard) {
...
public fun subscriptionPaymentMethodOptionsCard(block: SubscriptionPaymentMethodOptionsCard.Builder.() -> Unit): SubscriptionPaymentMethodOptionsCard = SubscriptionPaymentMethodOptionsCard.build(block)
