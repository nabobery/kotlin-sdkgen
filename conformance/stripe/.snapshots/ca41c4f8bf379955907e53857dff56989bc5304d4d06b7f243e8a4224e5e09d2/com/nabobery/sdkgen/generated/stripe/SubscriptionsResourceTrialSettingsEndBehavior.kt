...
public class SubscriptionsResourceTrialSettingsEndBehavior(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): SubscriptionsResourceTrialSettingsEndBehavior = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<SubscriptionsResourceTrialSettingsEndBehavior> {
    ...
    override fun deserialize(decoder: Decoder): SubscriptionsResourceTrialSettingsEndBehavior {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("SubscriptionsResourceTrialSettingsEndBehavior must be a JSON object")
      val missingPaymentMethod = json.decodeRequired<InlineSubscriptionsResourceTrialSettingsEndBehaviorMissingPaymentMethodX98f2e764>(rawObject, "missing_payment_method")
      return SubscriptionsResourceTrialSettingsEndBehavior(
        missingPaymentMethod = missingPaymentMethod,
      )
    }

    override fun serialize(encoder: Encoder, `value`: SubscriptionsResourceTrialSettingsEndBehavior) {
...
public fun subscriptionsResourceTrialSettingsEndBehavior(block: SubscriptionsResourceTrialSettingsEndBehavior.Builder.() -> Unit): SubscriptionsResourceTrialSettingsEndBehavior = SubscriptionsResourceTrialSettingsEndBehavior.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("SubscriptionsResourceTrialSettingsEndBehavior is missing required property '" + name + "'")
