...
public class IssuingCardGooglePay(
  ...
  internal object Serializer : KSerializer<IssuingCardGooglePay> {
    ...
    override fun deserialize(decoder: Decoder): IssuingCardGooglePay {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("IssuingCardGooglePay must be a JSON object")
      val eligible = json.decodeRequired<Boolean>(rawObject, "eligible")
      return IssuingCardGooglePay(
        eligible = eligible,
        ineligibleReason = rawObject["ineligible_reason"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingCardGooglePayIneligibleReasonX01a38feb?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: IssuingCardGooglePay) {
...
public fun issuingCardGooglePay(block: IssuingCardGooglePay.Builder.() -> Unit): IssuingCardGooglePay = IssuingCardGooglePay.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("IssuingCardGooglePay is missing required property '" + name + "'")
