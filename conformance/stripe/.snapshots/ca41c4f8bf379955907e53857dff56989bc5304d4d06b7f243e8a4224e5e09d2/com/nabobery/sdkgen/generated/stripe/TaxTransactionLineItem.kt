...
public class TaxTransactionLineItem(
  ...
  internal object Serializer : KSerializer<TaxTransactionLineItem> {
    ...
    override fun deserialize(decoder: Decoder): TaxTransactionLineItem {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("TaxTransactionLineItem must be a JSON object")
      ...
      val taxBehavior = json.decodeRequired<InlineTaxTransactionLineItemTaxBehaviorXe0d015ed>(rawObject, "tax_behavior")
      ...
      return TaxTransactionLineItem(
        ...
        reversal = rawObject["reversal"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineTaxTransactionLineItemReversalX4c9abeca?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: TaxTransactionLineItem) {
...
public fun taxTransactionLineItem(block: TaxTransactionLineItem.Builder.() -> Unit): TaxTransactionLineItem = TaxTransactionLineItem.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("TaxTransactionLineItem is missing required property '" + name + "'")
