...
public class PaymentMethodDetailsKonbiniStore(
  ...
  public class Builder {
    ...
    public fun build(): PaymentMethodDetailsKonbiniStore = PaymentMethodDetailsKonbiniStore(
      chain = chain,
    )
  }
  ...
  internal object Serializer : KSerializer<PaymentMethodDetailsKonbiniStore> {
    ...
    override fun deserialize(decoder: Decoder): PaymentMethodDetailsKonbiniStore {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentMethodDetailsKonbiniStore must be a JSON object")
      return PaymentMethodDetailsKonbiniStore(
        chain = rawObject["chain"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentMethodDetailsKonbiniStoreChainXb550e3bd?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PaymentMethodDetailsKonbiniStore) {
...
public fun paymentMethodDetailsKonbiniStore(block: PaymentMethodDetailsKonbiniStore.Builder.() -> Unit): PaymentMethodDetailsKonbiniStore = PaymentMethodDetailsKonbiniStore.build(block)
