...
public class ClimateOrder(
  ...
  public class Builder {
    ...
    public var deliveryDetails: List<ClimateRemovalsOrderDeliveries>
      get() = requireNotNull(deliveryDetailsValue) { "deliveryDetails is required" }.toList()
      set(`value`) {
        deliveryDetailsValue = value.toList()
      }
  ...
  internal object Serializer : KSerializer<ClimateOrder> {
    ...
    override fun deserialize(decoder: Decoder): ClimateOrder {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("ClimateOrder must be a JSON object")
      ...
      val deliveryDetails = json.decodeRequired<List<ClimateRemovalsOrderDeliveries>>(rawObject, "delivery_details")
      ...
      return ClimateOrder(
        ...
        product = product,
        status = status,
        beneficiary = rawObject["beneficiary"]?.let { json.decodeFromJsonElement<ClimateRemovalsBeneficiary>(it) },
        canceledAt = rawObject["canceled_at"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
        cancellationReason = rawObject["cancellation_reason"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineClimateOrderCancellationReasonXb8eb996f?>(element) },
        certificate = rawObject["certificate"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        confirmedAt = rawObject["confirmed_at"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
        delayedAt = rawObject["delayed_at"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
        deliveredAt = rawObject["delivered_at"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
        productSubstitutedAt = rawObject["product_substituted_at"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: ClimateOrder) {
