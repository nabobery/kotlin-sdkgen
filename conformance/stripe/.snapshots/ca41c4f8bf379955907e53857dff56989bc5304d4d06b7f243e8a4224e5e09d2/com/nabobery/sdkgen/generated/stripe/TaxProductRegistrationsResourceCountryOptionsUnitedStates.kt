...
public class TaxProductRegistrationsResourceCountryOptionsUnitedStates(
  ...
  public val localAmusementTax:
      TaxProductRegistrationsResourceCountryOptionsUsLocalAmusementTax? = null,
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): TaxProductRegistrationsResourceCountryOptionsUnitedStates = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<TaxProductRegistrationsResourceCountryOptionsUnitedStates> {
    ...
    override fun deserialize(decoder: Decoder): TaxProductRegistrationsResourceCountryOptionsUnitedStates {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("TaxProductRegistrationsResourceCountryOptionsUnitedStates must be a JSON object")
      val state = json.decodeRequired<String>(rawObject, "state")
      val type = json.decodeRequired<InlineTaxProductRegistrationsResourceCountryOptionsUnitedStatesTypeX80e9f5e5>(rawObject, "type")
      return TaxProductRegistrationsResourceCountryOptionsUnitedStates(
        state = state,
        type = type,
        localAmusementTax = rawObject["local_amusement_tax"]?.let { json.decodeFromJsonElement<TaxProductRegistrationsResourceCountryOptionsUsLocalAmusementTax>(it) },
        localLeaseTax = rawObject["local_lease_tax"]?.let { json.decodeFromJsonElement<TaxProductRegistrationsResourceCountryOptionsUsLocalLeaseTax>(it) },
        stateSalesTax = rawObject["state_sales_tax"]?.let { json.decodeFromJsonElement<TaxProductRegistrationsResourceCountryOptionsUsStateSalesTax>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: TaxProductRegistrationsResourceCountryOptionsUnitedStates) {
...
public fun taxProductRegistrationsResourceCountryOptionsUnitedStates(block: TaxProductRegistrationsResourceCountryOptionsUnitedStates.Builder.() -> Unit): TaxProductRegistrationsResourceCountryOptionsUnitedStates = TaxProductRegistrationsResourceCountryOptionsUnitedStates.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("TaxProductRegistrationsResourceCountryOptionsUnitedStates is missing required property '" + name + "'")
