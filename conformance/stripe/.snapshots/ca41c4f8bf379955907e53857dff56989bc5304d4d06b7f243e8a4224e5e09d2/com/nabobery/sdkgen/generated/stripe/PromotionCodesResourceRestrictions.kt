...
public class PromotionCodesResourceRestrictions(
  ...
  currencyOptions: Map<String, PromotionCodeCurrencyOption>? = null,
  /**
   ...
   */
  public val currencyOptions: Map<String, PromotionCodeCurrencyOption>? =
  ...
  public class Builder {
    ...
    private var currencyOptionsValue: Map<String, PromotionCodeCurrencyOption>? = null
     ...
     * currency](https://stripe.com/docs/currencies).
     */
    public var currencyOptions: Map<String, PromotionCodeCurrencyOption>?
  ...
  internal object Serializer : KSerializer<PromotionCodesResourceRestrictions> {
    ...
    override fun deserialize(decoder: Decoder): PromotionCodesResourceRestrictions {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PromotionCodesResourceRestrictions must be a JSON object")
      val firstTimeTransaction = json.decodeRequired<Boolean>(rawObject, "first_time_transaction")
      return PromotionCodesResourceRestrictions(
        firstTimeTransaction = firstTimeTransaction,
        currencyOptions = rawObject["currency_options"]?.let { json.decodeFromJsonElement<Map<String, PromotionCodeCurrencyOption>>(it) },
        minimumAmount = rawObject["minimum_amount"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
        minimumAmountCurrency = rawObject["minimum_amount_currency"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PromotionCodesResourceRestrictions) {
...
public fun promotionCodesResourceRestrictions(block: PromotionCodesResourceRestrictions.Builder.() -> Unit): PromotionCodesResourceRestrictions = PromotionCodesResourceRestrictions.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("PromotionCodesResourceRestrictions is missing required property '" + name + "'")
