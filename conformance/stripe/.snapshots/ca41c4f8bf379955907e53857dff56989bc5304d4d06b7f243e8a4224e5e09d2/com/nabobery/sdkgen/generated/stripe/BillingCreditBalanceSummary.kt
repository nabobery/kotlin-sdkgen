...
public class BillingCreditBalanceSummary(
  ...
  public class Builder {
    ...
    public var balances: List<CreditBalance>
      get() = requireNotNull(balancesValue) { "balances is required" }.toList()
      set(`value`) {
        balancesValue = value.toList()
      }
  ...
  internal object Serializer : KSerializer<BillingCreditBalanceSummary> {
    ...
    override fun deserialize(decoder: Decoder): BillingCreditBalanceSummary {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("BillingCreditBalanceSummary must be a JSON object")
      val balances = json.decodeRequired<List<CreditBalance>>(rawObject, "balances")
      val customer = json.decodeRequired<InlineBillingCreditBalanceSummaryCustomerXd7a27b43>(rawObject, "customer")
      val livemode = json.decodeRequired<Boolean>(rawObject, "livemode")
      val objectValue = json.decodeRequired<InlineBillingCreditBalanceSummaryObjectValueX8dbd6d20>(rawObject, "object")
      return BillingCreditBalanceSummary(
        balances = balances,
        customer = customer,
        livemode = livemode,
    ...
    override fun serialize(encoder: Encoder, `value`: BillingCreditBalanceSummary) {
...
public fun billingCreditBalanceSummary(block: BillingCreditBalanceSummary.Builder.() -> Unit): BillingCreditBalanceSummary = BillingCreditBalanceSummary.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("BillingCreditBalanceSummary is missing required property '" + name + "'")
