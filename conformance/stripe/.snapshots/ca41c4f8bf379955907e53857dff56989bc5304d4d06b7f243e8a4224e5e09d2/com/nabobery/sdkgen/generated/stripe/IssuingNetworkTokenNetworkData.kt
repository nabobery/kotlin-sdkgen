...
public class IssuingNetworkTokenNetworkData(
  ...
  internal object Serializer : KSerializer<IssuingNetworkTokenNetworkData> {
    ...
    override fun deserialize(decoder: Decoder): IssuingNetworkTokenNetworkData {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("IssuingNetworkTokenNetworkData must be a JSON object")
      ...
      return IssuingNetworkTokenNetworkData(
        ...
        mastercard = rawObject["mastercard"]?.let { json.decodeFromJsonElement<IssuingNetworkTokenMastercard>(it) },
        visa = rawObject["visa"]?.let { json.decodeFromJsonElement<IssuingNetworkTokenVisa>(it) },
        walletProvider = rawObject["wallet_provider"]?.let { json.decodeFromJsonElement<IssuingNetworkTokenWalletProvider>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: IssuingNetworkTokenNetworkData) {
...
public fun issuingNetworkTokenNetworkData(block: IssuingNetworkTokenNetworkData.Builder.() -> Unit): IssuingNetworkTokenNetworkData = IssuingNetworkTokenNetworkData.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("IssuingNetworkTokenNetworkData is missing required property '" + name + "'")
