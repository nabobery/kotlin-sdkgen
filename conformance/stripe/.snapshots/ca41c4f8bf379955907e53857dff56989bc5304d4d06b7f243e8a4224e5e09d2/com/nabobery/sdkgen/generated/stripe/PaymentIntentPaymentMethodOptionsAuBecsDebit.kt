...
public class PaymentIntentPaymentMethodOptionsAuBecsDebit(
  ...
  public class Builder {
    ...
    public fun build(): PaymentIntentPaymentMethodOptionsAuBecsDebit = PaymentIntentPaymentMethodOptionsAuBecsDebit(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): PaymentIntentPaymentMethodOptionsAuBecsDebit = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<PaymentIntentPaymentMethodOptionsAuBecsDebit> {
    ...
    override fun deserialize(decoder: Decoder): PaymentIntentPaymentMethodOptionsAuBecsDebit {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentIntentPaymentMethodOptionsAuBecsDebit must be a JSON object")
      return PaymentIntentPaymentMethodOptionsAuBecsDebit(
        setupFutureUsage = rawObject["setup_future_usage"]?.let { json.decodeFromJsonElement<InlinePaymentIntentPaymentMethodOptionsAuBecsDebitSetupFutureUsageXbdbd2b1d>(it) },
        targetDate = rawObject["target_date"]?.let { json.decodeFromJsonElement<String>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PaymentIntentPaymentMethodOptionsAuBecsDebit) {
...
public fun paymentIntentPaymentMethodOptionsAuBecsDebit(block: PaymentIntentPaymentMethodOptionsAuBecsDebit.Builder.() -> Unit): PaymentIntentPaymentMethodOptionsAuBecsDebit = PaymentIntentPaymentMethodOptionsAuBecsDebit.build(block)
