...
public class PaymentIntentNextActionDisplayBankTransferInstructions(
  ...
  financialAddresses: List<FundingInstructionsBankTransferFinancialAddress>? = null,
  /**
  ...
  public val financialAddresses: List<FundingInstructionsBankTransferFinancialAddress>? =
      financialAddresses?.let { collection0 -> collection0.toList() }
  ...
  public class Builder {
    ...
    public var financialAddresses: List<FundingInstructionsBankTransferFinancialAddress>?
      get() = financialAddressesValue?.let { collection0 -> collection0.toList() }
      set(`value`) {
        financialAddressesValue = value?.let { collection0 -> collection0.toList() }
      }
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): PaymentIntentNextActionDisplayBankTransferInstructions = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<PaymentIntentNextActionDisplayBankTransferInstructions> {
    ...
    override fun deserialize(decoder: Decoder): PaymentIntentNextActionDisplayBankTransferInstructions {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentIntentNextActionDisplayBankTransferInstructions must be a JSON object")
      val type = json.decodeRequired<InlinePaymentIntentNextActionDisplayBankTransferInstructionsTypeX06b693fe>(rawObject, "type")
      return PaymentIntentNextActionDisplayBankTransferInstructions(
        type = type,
        amountRemaining = rawObject["amount_remaining"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
        currency = rawObject["currency"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        financialAddresses = rawObject["financial_addresses"]?.let { json.decodeFromJsonElement<List<FundingInstructionsBankTransferFinancialAddress>>(it) },
        hostedInstructionsUrl = rawObject["hosted_instructions_url"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        reference = rawObject["reference"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PaymentIntentNextActionDisplayBankTransferInstructions) {
...
public fun paymentIntentNextActionDisplayBankTransferInstructions(block: PaymentIntentNextActionDisplayBankTransferInstructions.Builder.() -> Unit): PaymentIntentNextActionDisplayBankTransferInstructions = PaymentIntentNextActionDisplayBankTransferInstructions.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("PaymentIntentNextActionDisplayBankTransferInstructions is missing required property '" + name + "'")
