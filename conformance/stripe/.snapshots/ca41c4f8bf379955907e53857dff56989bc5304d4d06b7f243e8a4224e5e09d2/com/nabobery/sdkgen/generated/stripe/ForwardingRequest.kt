...
public class ForwardingRequest(
  ...
  internal object Serializer : KSerializer<ForwardingRequest> {
    ...
    override fun deserialize(decoder: Decoder): ForwardingRequest {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("ForwardingRequest must be a JSON object")
      ...
      val replacements = json.decodeRequired<List<InlineForwardingRequestReplacementsItemXe1b219c9>>(rawObject, "replacements")
      return ForwardingRequest(
        created = created,
        id = id,
        livemode = livemode,
        ...
        requestContext = rawObject["request_context"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineForwardingRequestRequestContextXbafbd94f?>(element) },
        requestDetails = rawObject["request_details"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineForwardingRequestRequestDetailsX8d6ec125?>(element) },
        responseDetails = rawObject["response_details"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineForwardingRequestResponseDetailsXfec234d4?>(element) },
        url = rawObject["url"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: ForwardingRequest) {
...
public fun forwardingRequest(block: ForwardingRequest.Builder.() -> Unit): ForwardingRequest = ForwardingRequest.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("ForwardingRequest is missing required property '" + name + "'")
