...
public class GelatoSelfieReport(
  ...
  internal object Serializer : KSerializer<GelatoSelfieReport> {
    ...
    override fun deserialize(decoder: Decoder): GelatoSelfieReport {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("GelatoSelfieReport must be a JSON object")
      val status = json.decodeRequired<InlineGelatoSelfieReportStatusX07703f81>(rawObject, "status")
      return GelatoSelfieReport(
        status = status,
        document = rawObject["document"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        error = rawObject["error"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineGelatoSelfieReportErrorX46da4547?>(element) },
        selfie = rawObject["selfie"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: GelatoSelfieReport) {
...
public fun gelatoSelfieReport(block: GelatoSelfieReport.Builder.() -> Unit): GelatoSelfieReport = GelatoSelfieReport.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("GelatoSelfieReport is missing required property '" + name + "'")
