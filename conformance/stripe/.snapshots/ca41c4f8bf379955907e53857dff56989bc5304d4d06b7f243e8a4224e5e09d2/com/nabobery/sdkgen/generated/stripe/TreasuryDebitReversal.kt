...
public class TreasuryDebitReversal(
  ...
  internal object Serializer : KSerializer<TreasuryDebitReversal> {
    ...
    override fun deserialize(decoder: Decoder): TreasuryDebitReversal {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("TreasuryDebitReversal must be a JSON object")
      ...
      val statusTransitions = json.decodeRequired<TreasuryReceivedDebitsResourceStatusTransitions>(rawObject, "status_transitions")
      return TreasuryDebitReversal(
        amount = amount,
        created = created,
        currency = currency,
        ...
        linkedFlows = rawObject["linked_flows"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineTreasuryDebitReversalLinkedFlowsX60b89939?>(element) },
        transaction = rawObject["transaction"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineTreasuryDebitReversalTransactionX6d21296f?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: TreasuryDebitReversal) {
...
public fun treasuryDebitReversal(block: TreasuryDebitReversal.Builder.() -> Unit): TreasuryDebitReversal = TreasuryDebitReversal.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("TreasuryDebitReversal is missing required property '" + name + "'")
