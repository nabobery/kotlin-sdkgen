...
public class PaymentMethodDetailsCardInstallmentsPlan(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): PaymentMethodDetailsCardInstallmentsPlan = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<PaymentMethodDetailsCardInstallmentsPlan> {
    ...
    override fun deserialize(decoder: Decoder): PaymentMethodDetailsCardInstallmentsPlan {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentMethodDetailsCardInstallmentsPlan must be a JSON object")
      val type = json.decodeRequired<InlinePaymentMethodDetailsCardInstallmentsPlanTypeX7b8fbc7a>(rawObject, "type")
      return PaymentMethodDetailsCardInstallmentsPlan(
        type = type,
        count = rawObject["count"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Int?>(element) },
        interval = rawObject["interval"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentMethodDetailsCardInstallmentsPlanIntervalXe0ba3ec2?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PaymentMethodDetailsCardInstallmentsPlan) {
...
public fun paymentMethodDetailsCardInstallmentsPlan(block: PaymentMethodDetailsCardInstallmentsPlan.Builder.() -> Unit): PaymentMethodDetailsCardInstallmentsPlan = PaymentMethodDetailsCardInstallmentsPlan.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("PaymentMethodDetailsCardInstallmentsPlan is missing required property '" + name + "'")
