...
public class PaymentMethodConfigResourcePaymentMethodProperties(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): PaymentMethodConfigResourcePaymentMethodProperties = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<PaymentMethodConfigResourcePaymentMethodProperties> {
    ...
    override fun deserialize(decoder: Decoder): PaymentMethodConfigResourcePaymentMethodProperties {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentMethodConfigResourcePaymentMethodProperties must be a JSON object")
      val available = json.decodeRequired<Boolean>(rawObject, "available")
      val displayPreference = json.decodeRequired<PaymentMethodConfigResourceDisplayPreference>(rawObject, "display_preference")
      return PaymentMethodConfigResourcePaymentMethodProperties(
        available = available,
        displayPreference = displayPreference,
      )
    ...
    override fun serialize(encoder: Encoder, `value`: PaymentMethodConfigResourcePaymentMethodProperties) {
...
public fun paymentMethodConfigResourcePaymentMethodProperties(block: PaymentMethodConfigResourcePaymentMethodProperties.Builder.() -> Unit): PaymentMethodConfigResourcePaymentMethodProperties = PaymentMethodConfigResourcePaymentMethodProperties.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("PaymentMethodConfigResourcePaymentMethodProperties is missing required property '" + name + "'")
