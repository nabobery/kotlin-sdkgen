...
public class TerminalConfigurationConfigurationResourceWifiConfig(
  ...
  public val enterpriseEapPeap:
      TerminalConfigurationConfigurationResourceEnterprisePeapWifi? = null,
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): TerminalConfigurationConfigurationResourceWifiConfig = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<TerminalConfigurationConfigurationResourceWifiConfig> {
    ...
    override fun deserialize(decoder: Decoder): TerminalConfigurationConfigurationResourceWifiConfig {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("TerminalConfigurationConfigurationResourceWifiConfig must be a JSON object")
      val type = json.decodeRequired<InlineTerminalConfigurationConfigurationResourceWifiConfigTypeXbf775f83>(rawObject, "type")
      return TerminalConfigurationConfigurationResourceWifiConfig(
        type = type,
        enterpriseEapPeap = rawObject["enterprise_eap_peap"]?.let { json.decodeFromJsonElement<TerminalConfigurationConfigurationResourceEnterprisePeapWifi>(it) },
        enterpriseEapTls = rawObject["enterprise_eap_tls"]?.let { json.decodeFromJsonElement<TerminalConfigurationConfigurationResourceEnterpriseTlsWifi>(it) },
        personalPsk = rawObject["personal_psk"]?.let { json.decodeFromJsonElement<TerminalConfigurationConfigurationResourcePersonalPskWifi>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: TerminalConfigurationConfigurationResourceWifiConfig) {
...
public fun terminalConfigurationConfigurationResourceWifiConfig(block: TerminalConfigurationConfigurationResourceWifiConfig.Builder.() -> Unit): TerminalConfigurationConfigurationResourceWifiConfig = TerminalConfigurationConfigurationResourceWifiConfig.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("TerminalConfigurationConfigurationResourceWifiConfig is missing required property '" + name + "'")
