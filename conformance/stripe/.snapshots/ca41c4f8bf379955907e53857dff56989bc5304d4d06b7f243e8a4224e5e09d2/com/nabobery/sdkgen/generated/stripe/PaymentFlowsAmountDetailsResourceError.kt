...
public class PaymentFlowsAmountDetailsResourceError(
  ...
  public class Builder {
    ...
    public fun build(): PaymentFlowsAmountDetailsResourceError = PaymentFlowsAmountDetailsResourceError(
      code = code,
      message = message,
    )
  }
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): PaymentFlowsAmountDetailsResourceError = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<PaymentFlowsAmountDetailsResourceError> {
    ...
    override fun deserialize(decoder: Decoder): PaymentFlowsAmountDetailsResourceError {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentFlowsAmountDetailsResourceError must be a JSON object")
      return PaymentFlowsAmountDetailsResourceError(
        code = rawObject["code"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlinePaymentFlowsAmountDetailsResourceErrorCodeXaf7724eb?>(element) },
        message = rawObject["message"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PaymentFlowsAmountDetailsResourceError) {
...
public fun paymentFlowsAmountDetailsResourceError(block: PaymentFlowsAmountDetailsResourceError.Builder.() -> Unit): PaymentFlowsAmountDetailsResourceError = PaymentFlowsAmountDetailsResourceError.build(block)
