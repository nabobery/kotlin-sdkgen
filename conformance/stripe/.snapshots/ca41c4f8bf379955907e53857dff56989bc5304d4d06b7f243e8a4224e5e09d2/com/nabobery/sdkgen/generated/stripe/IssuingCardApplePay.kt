...
public class IssuingCardApplePay(
  ...
  internal object Serializer : KSerializer<IssuingCardApplePay> {
    ...
    override fun deserialize(decoder: Decoder): IssuingCardApplePay {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("IssuingCardApplePay must be a JSON object")
      val eligible = json.decodeRequired<Boolean>(rawObject, "eligible")
      return IssuingCardApplePay(
        eligible = eligible,
        ineligibleReason = rawObject["ineligible_reason"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineIssuingCardApplePayIneligibleReasonXbee2eece?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: IssuingCardApplePay) {
...
public fun issuingCardApplePay(block: IssuingCardApplePay.Builder.() -> Unit): IssuingCardApplePay = IssuingCardApplePay.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("IssuingCardApplePay is missing required property '" + name + "'")
