...
public class IssuingTransactionFlightDataLeg(
  ...
  public class Builder {
    ...
    public fun build(): IssuingTransactionFlightDataLeg = IssuingTransactionFlightDataLeg(
  ...
  internal object Serializer : KSerializer<IssuingTransactionFlightDataLeg> {
    ...
    override fun deserialize(decoder: Decoder): IssuingTransactionFlightDataLeg {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("IssuingTransactionFlightDataLeg must be a JSON object")
      return IssuingTransactionFlightDataLeg(
        arrivalAirportCode = rawObject["arrival_airport_code"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        carrier = rawObject["carrier"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        departureAirportCode = rawObject["departure_airport_code"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        flightNumber = rawObject["flight_number"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        serviceClass = rawObject["service_class"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        stopoverAllowed = rawObject["stopover_allowed"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<Boolean?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: IssuingTransactionFlightDataLeg) {
...
public fun issuingTransactionFlightDataLeg(block: IssuingTransactionFlightDataLeg.Builder.() -> Unit): IssuingTransactionFlightDataLeg = IssuingTransactionFlightDataLeg.build(block)
