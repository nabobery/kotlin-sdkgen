...
public class IssuingToken(
  ...
  internal object Serializer : KSerializer<IssuingToken> {
    ...
    override fun deserialize(decoder: Decoder): IssuingToken {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("IssuingToken must be a JSON object")
      ...
      return IssuingToken(
        ...
        objectValue = objectValue,
        status = status,
        deviceFingerprint = rawObject["device_fingerprint"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
        last4 = rawObject["last4"]?.let { json.decodeFromJsonElement<String>(it) },
        networkData = rawObject["network_data"]?.let { json.decodeFromJsonElement<IssuingNetworkTokenNetworkData>(it) },
        walletProvider = rawObject["wallet_provider"]?.let { json.decodeFromJsonElement<InlineIssuingTokenWalletProviderXc4259c1a>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: IssuingToken) {
