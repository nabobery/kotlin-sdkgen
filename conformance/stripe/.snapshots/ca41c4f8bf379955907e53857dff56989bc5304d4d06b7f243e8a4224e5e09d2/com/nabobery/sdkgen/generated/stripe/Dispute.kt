...
@Serializable
public data class DisputeView internal constructor(
...
public class Dispute(
  ...
  public class Builder {
    ...
    public var enhancedEligibilityTypes: List<InlineDisputeEnhancedEligibilityTypesItemXa48b4f02>
      get() = requireNotNull(enhancedEligibilityTypesValue) { "enhancedEligibilityTypes is required" }.toList()
      set(`value`) {
        enhancedEligibilityTypesValue = value.toList()
      }
  ...
  internal object Serializer : KSerializer<Dispute> {
    override val descriptor: SerialDescriptor = JsonElement.serializer().descriptor

    override fun deserialize(decoder: Decoder): Dispute {
      val jsonDecoder = decoder.requireJsonDecoder("Dispute")
      val json = jsonDecoder.json
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("Dispute must be a JSON object")
      ...
      val enhancedEligibilityTypes = json.decodeRequired<List<InlineDisputeEnhancedEligibilityTypesItemXa48b4f02>>(rawObject, "enhanced_eligibility_types")
      ...
      return Dispute(
        ...
        reason = reason,
        status = status,
        paymentIntent = rawObject["payment_intent"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineDisputePaymentIntentXbb1e9cd4?>(element) },
        paymentMethodDetails = rawObject["payment_method_details"]?.let { json.decodeFromJsonElement<DisputePaymentMethodDetails>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: Dispute) {
