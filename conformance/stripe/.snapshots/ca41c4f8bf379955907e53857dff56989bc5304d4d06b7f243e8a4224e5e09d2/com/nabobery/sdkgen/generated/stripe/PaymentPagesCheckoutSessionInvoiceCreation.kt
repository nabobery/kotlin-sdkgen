...
public class PaymentPagesCheckoutSessionInvoiceCreation(
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): PaymentPagesCheckoutSessionInvoiceCreation = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<PaymentPagesCheckoutSessionInvoiceCreation> {
    ...
    override fun deserialize(decoder: Decoder): PaymentPagesCheckoutSessionInvoiceCreation {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PaymentPagesCheckoutSessionInvoiceCreation must be a JSON object")
    ...
    override fun serialize(encoder: Encoder, `value`: PaymentPagesCheckoutSessionInvoiceCreation) {
...
public fun paymentPagesCheckoutSessionInvoiceCreation(block: PaymentPagesCheckoutSessionInvoiceCreation.Builder.() -> Unit): PaymentPagesCheckoutSessionInvoiceCreation = PaymentPagesCheckoutSessionInvoiceCreation.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("PaymentPagesCheckoutSessionInvoiceCreation is missing required property '" + name + "'")
