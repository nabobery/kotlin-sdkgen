...
public class PortalPaymentMethodUpdate(
  ...
  internal object Serializer : KSerializer<PortalPaymentMethodUpdate> {
    ...
    override fun deserialize(decoder: Decoder): PortalPaymentMethodUpdate {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("PortalPaymentMethodUpdate must be a JSON object")
      val enabled = json.decodeRequired<Boolean>(rawObject, "enabled")
      return PortalPaymentMethodUpdate(
        enabled = enabled,
        paymentMethodConfiguration = rawObject["payment_method_configuration"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: PortalPaymentMethodUpdate) {
...
public fun portalPaymentMethodUpdate(block: PortalPaymentMethodUpdate.Builder.() -> Unit): PortalPaymentMethodUpdate = PortalPaymentMethodUpdate.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("PortalPaymentMethodUpdate is missing required property '" + name + "'")
