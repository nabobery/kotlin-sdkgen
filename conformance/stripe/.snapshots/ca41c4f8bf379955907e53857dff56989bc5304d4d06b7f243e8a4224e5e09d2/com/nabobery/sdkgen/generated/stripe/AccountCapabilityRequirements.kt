...
public class AccountCapabilityRequirements(
  ...
  internal object Serializer : KSerializer<AccountCapabilityRequirements> {
    ...
    override fun deserialize(decoder: Decoder): AccountCapabilityRequirements {
      ...
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("AccountCapabilityRequirements must be a JSON object")
      ...
      return AccountCapabilityRequirements(
        ...
        disabledReason = rawObject["disabled_reason"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineAccountCapabilityRequirementsDisabledReasonXbebf8378?>(element) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: AccountCapabilityRequirements) {
...
public fun accountCapabilityRequirements(block: AccountCapabilityRequirements.Builder.() -> Unit): AccountCapabilityRequirements = AccountCapabilityRequirements.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("AccountCapabilityRequirements is missing required property '" + name + "'")
