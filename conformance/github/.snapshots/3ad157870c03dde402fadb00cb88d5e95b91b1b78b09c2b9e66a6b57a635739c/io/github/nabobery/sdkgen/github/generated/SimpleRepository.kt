 ...
 * A GitHub repository.
 *
 * Source: sdkgen://source/openapi.yaml#/components/schemas/simple-repository
 */
@Serializable(with = SimpleRepository.Serializer::class)
public class SimpleRepository(
  ...
  public class Builder {
    ...
    public fun build(): SimpleRepository {
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): SimpleRepository = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<SimpleRepository> {
    override val descriptor: SerialDescriptor = JsonElement.serializer().descriptor

    override fun deserialize(decoder: Decoder): SimpleRepository {
      val jsonDecoder = decoder.requireJsonDecoder("SimpleRepository")
      val json = jsonDecoder.json
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("SimpleRepository must be a JSON object")
      ...
      if (!rawObject.containsKey("description")) {
        throw SerializationException("SimpleRepository is missing required property 'description'")
      }
    ...
    override fun serialize(encoder: Encoder, `value`: SimpleRepository) {
      val jsonEncoder = encoder.requireJsonEncoder("SimpleRepository")
...
public fun simpleRepository(block: SimpleRepository.Builder.() -> Unit): SimpleRepository = SimpleRepository.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("SimpleRepository is missing required property '" + name + "'")
...
private inline fun <reified T> Json.decodeOptional(
  ...
  if (element == JsonNull) {
    if (!nullable) throw SerializationException("SimpleRepository property '" + name + "' is not nullable")
