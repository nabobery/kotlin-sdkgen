 ...
 * Collaborator
 *
 * Source: sdkgen://source/openapi.yaml#/components/schemas/collaborator
 */
@Serializable(with = Collaborator.Serializer::class)
public class Collaborator internal constructor(
  /**
  ...
  private val permissionsState: FieldState<InlineCollaboratorPermissionsXf02d903c>,
  ...
  public val permissions: InlineCollaboratorPermissionsXf02d903c?
  ...
  public class Builder {
    ...
    private var permissionsState: FieldState<InlineCollaboratorPermissionsXf02d903c> =
        FieldState.Absent

    /**
     * Assign a non-null value, or use the unset function to omit the property.
     */
    public var permissions: InlineCollaboratorPermissionsXf02d903c?
      get() = permissionsState.valueOrNull()
      set(`value`) {
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): Collaborator = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<Collaborator> {
    override val descriptor: SerialDescriptor = JsonElement.serializer().descriptor

    override fun deserialize(decoder: Decoder): Collaborator {
      val jsonDecoder = decoder.requireJsonDecoder("Collaborator")
      val json = jsonDecoder.json
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("Collaborator must be a JSON object")
      ...
      if (!rawObject.containsKey("gravatar_id")) {
        throw SerializationException("Collaborator is missing required property 'gravatar_id'")
      }
    ...
    override fun serialize(encoder: Encoder, `value`: Collaborator) {
      val jsonEncoder = encoder.requireJsonEncoder("Collaborator")
...
public fun collaborator(block: Collaborator.Builder.() -> Unit): Collaborator = Collaborator.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("Collaborator is missing required property '" + name + "'")
...
private inline fun <reified T> Json.decodeOptional(
  ...
  if (element == JsonNull) {
    if (!nullable) throw SerializationException("Collaborator property '" + name + "' is not nullable")
