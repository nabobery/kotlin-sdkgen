...
public class ContentTree internal constructor(
  ...
  entriesState: FieldState<List<InlineContentTreeEntriesItemXc0b8f4a8>>,
) {
  private val entriesState: FieldState<List<InlineContentTreeEntriesItemXc0b8f4a8>> =
      entriesState.copyValue { fieldValue -> fieldValue.toList() }
  ...
  public class Builder {
    ...
    public var entries: List<InlineContentTreeEntriesItemXc0b8f4a8>?
      get() = entriesState.valueOrNull()?.let { collection0 -> collection0.toList() }
      set(`value`) {
        val present = requireNotNull(value) { "entries is not nullable; call unsetEntries() to omit it" }
        entriesState = FieldState.Value(present.toList())
      }
  ...
  internal object Serializer : KSerializer<ContentTree> {
