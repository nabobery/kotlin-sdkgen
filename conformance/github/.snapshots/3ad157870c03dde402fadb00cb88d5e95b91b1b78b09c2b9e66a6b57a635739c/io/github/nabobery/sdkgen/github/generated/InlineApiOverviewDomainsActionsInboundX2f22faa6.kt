...
public class InlineApiOverviewDomainsActionsInboundX2f22faa6(
  fullDomains: List<String>? = null,
  wildcardDomains: List<String>? = null,
) {
  public val fullDomains: List<String>? = fullDomains?.let { collection0 -> collection0.toList() }

  public val wildcardDomains: List<String>? =
      wildcardDomains?.let { collection0 -> collection0.toList() }

  public class Builder {
    private var fullDomainsValue: List<String>? = null

    public var fullDomains: List<String>?
      get() = fullDomainsValue?.let { collection0 -> collection0.toList() }
      set(`value`) {
        fullDomainsValue = value?.let { collection0 -> collection0.toList() }
      }

    private var wildcardDomainsValue: List<String>? = null

    public var wildcardDomains: List<String>?
      get() = wildcardDomainsValue?.let { collection0 -> collection0.toList() }
      set(`value`) {
        wildcardDomainsValue = value?.let { collection0 -> collection0.toList() }
      }
  ...
  internal object Serializer : KSerializer<InlineApiOverviewDomainsActionsInboundX2f22faa6> {
