...
public class CopilotOrganizationDetails(
  ...
  public val additionalProperties: Map<String, JsonElement> =
      copyAndValidateCopilotOrganizationDetailsAdditionalProperties(additionalProperties)
  ...
  internal object Serializer : KSerializer<CopilotOrganizationDetails> {
    ...
    override fun deserialize(decoder: Decoder): CopilotOrganizationDetails {
      ...
      return CopilotOrganizationDetails(
        ...
        additionalProperties = rawObject.filterKeys { key -> key !in setOf("cli", "ide_chat", "plan_type", "platform_chat", "public_code_suggestions", "seat_breakdown", "seat_management_setting") }.mapValues { (_, element) -> element }.toMap(),
      )
    }
    ...
    override fun serialize(encoder: Encoder, `value`: CopilotOrganizationDetails) {
      ...
      val raw = buildJsonObject {
        ...
        value.additionalProperties.keys.sorted().forEach { key ->
          ...
          check(key !in setOf("cli", "ide_chat", "plan_type", "platform_chat", "public_code_suggestions", "seat_breakdown", "seat_management_setting")) { "CopilotOrganizationDetails additionalProperties key '" + key + "' collides with a fixed property" }
...
private fun copyAndValidateCopilotOrganizationDetailsAdditionalProperties(additionalProperties: Map<String, JsonElement>): Map<String, JsonElement> {
  val copied = additionalProperties.toMap()
  val collision = copied.keys.sorted().firstOrNull { key -> key in setOf("cli", "ide_chat", "plan_type", "platform_chat", "public_code_suggestions", "seat_breakdown", "seat_management_setting") }
  require(collision == null) { "CopilotOrganizationDetails additionalProperties key '" + collision + "' collides with a fixed property" }
  return copied
}
