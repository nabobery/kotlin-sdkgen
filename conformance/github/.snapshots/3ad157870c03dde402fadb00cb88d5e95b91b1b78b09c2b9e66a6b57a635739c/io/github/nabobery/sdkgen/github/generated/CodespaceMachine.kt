 ...
 *
 * Source: sdkgen://source/openapi.yaml#/components/schemas/codespace-machine
 */
@Serializable(with = CodespaceMachine.Serializer::class)
public class CodespaceMachine(
   ...
   */
  public val prebuildAvailability: InlineCodespaceMachinePrebuildAvailabilityX5eb10e1c?,
  /**
  ...
  public class Builder {
    ...
    private var prebuildAvailabilityState:
        FieldState<InlineCodespaceMachinePrebuildAvailabilityX5eb10e1c?> = FieldState.Absent
     ...
     */
    public var prebuildAvailability: InlineCodespaceMachinePrebuildAvailabilityX5eb10e1c?
      get() = prebuildAvailabilityState.valueOrNull()
      set(`value`) {
        prebuildAvailabilityState = value.toNullableFieldState()
      }

    public fun build(): CodespaceMachine {
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): CodespaceMachine = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<CodespaceMachine> {
    override val descriptor: SerialDescriptor = JsonElement.serializer().descriptor

    override fun deserialize(decoder: Decoder): CodespaceMachine {
      val jsonDecoder = decoder.requireJsonDecoder("CodespaceMachine")
      val json = jsonDecoder.json
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("CodespaceMachine must be a JSON object")
      ...
      if (!rawObject.containsKey("prebuild_availability")) {
        throw SerializationException("CodespaceMachine is missing required property 'prebuild_availability'")
      }
      val prebuildAvailability = rawObject["prebuild_availability"].let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<InlineCodespaceMachinePrebuildAvailabilityX5eb10e1c?>(requireNotNull(element)) }
      return CodespaceMachine(
        cpus = cpus,
        displayName = displayName,
        memoryInBytes = memoryInBytes,
    ...
    override fun serialize(encoder: Encoder, `value`: CodespaceMachine) {
      val jsonEncoder = encoder.requireJsonEncoder("CodespaceMachine")
...
public fun codespaceMachine(block: CodespaceMachine.Builder.() -> Unit): CodespaceMachine = CodespaceMachine.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("CodespaceMachine is missing required property '" + name + "'")
...
private inline fun <reified T> Json.decodeOptional(
  ...
  if (element == JsonNull) {
    if (!nullable) throw SerializationException("CodespaceMachine property '" + name + "' is not nullable")
