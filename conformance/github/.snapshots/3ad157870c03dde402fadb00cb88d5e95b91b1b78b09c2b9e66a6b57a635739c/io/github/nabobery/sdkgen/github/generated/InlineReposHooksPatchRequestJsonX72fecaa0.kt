...
public class InlineReposHooksPatchRequestJsonX72fecaa0 internal constructor(
  ...
  private val removeEventsState: FieldState<List<String>> =
      removeEventsState.copyValue { fieldValue -> fieldValue.toList() }
  ...
  public class Builder {
    ...
    public var addEvents: List<String>?
      get() = addEventsState.valueOrNull()?.let { collection0 -> collection0.toList() }
      set(`value`) {
        val present = requireNotNull(value) { "addEvents is not nullable; call unsetAddEvents() to omit it" }
        addEventsState = FieldState.Value(present.toList())
      }
    ...
    public var removeEvents: List<String>?
      get() = removeEventsState.valueOrNull()?.let { collection0 -> collection0.toList() }
      set(`value`) {
        val present = requireNotNull(value) { "removeEvents is not nullable; call unsetRemoveEvents() to omit it" }
        removeEventsState = FieldState.Value(present.toList())
      }
  ...
  internal object Serializer : KSerializer<InlineReposHooksPatchRequestJsonX72fecaa0> {
