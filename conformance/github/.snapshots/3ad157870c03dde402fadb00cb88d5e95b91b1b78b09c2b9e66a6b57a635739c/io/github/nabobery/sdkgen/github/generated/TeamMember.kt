...
/**
 * A user that is a member of a team, including their role on the team and whether the membership is inherited from a
 * child team.
 *
 * Source: sdkgen://source/openapi.yaml#/components/schemas/team-member
 */
@Serializable(with = TeamMember.Serializer::class)
public class TeamMember internal constructor(
  ...
  private val inheritedState: FieldState<Boolean>,
  private val nameState: FieldState<String?>,
  private val roleState: FieldState<InlineTeamMemberRoleXc77c8cc8>,
  ...
  /**
   * Whether the user is a member of the team only through a child team. `true` means the membership is inherited from a
   * child team; `false` means the user is a direct (immediate) member of the team. Only present on the `List team
   * members` endpoint, and only when the feature is enabled for the organization.
   */
  public val inherited: Boolean?
    get() = inheritedState.valueOrNull()

  public val name: String?
    get() = nameState.valueOrNull()

  /**
   * The member's role on the team. Only present on the `List team members` endpoint, and only when the feature is
   * enabled for the organization.
   */
  public val role: InlineTeamMemberRoleXc77c8cc8?
    get() = roleState.valueOrNull()
  ...
  inheritedState = FieldState.Absent,
  nameState = FieldState.Absent,
  roleState = FieldState.Absent,
  ...
  /**
   * Returns the wire presence of `inherited`.
   */
  public fun inheritedPresence(): FieldPresence = inheritedState.presence
  ...
  /**
   * Returns the wire presence of `role`.
   */
  public fun rolePresence(): FieldPresence = roleState.presence
  ...
  public class Builder {
    ...
    private var inheritedState: FieldState<Boolean> = FieldState.Absent

    /**
     * Whether the user is a member of the team only through a child team. `true` means the membership is inherited from
     * a child team; `false` means the user is a direct (immediate) member of the team. Only present on the `List team
     * members` endpoint, and only when the feature is enabled for the organization.
    ...
    public var inherited: Boolean?
      get() = inheritedState.valueOrNull()
      set(`value`) {
        val present = requireNotNull(value) { "inherited is not nullable; call unsetInherited() to omit it" }
        inheritedState = FieldState.Value(present)
      }
    ...
    private var roleState: FieldState<InlineTeamMemberRoleXc77c8cc8> = FieldState.Absent
     ...
     * Assign a non-null value, or use the unset function to omit the property.
     */
    public var role: InlineTeamMemberRoleXc77c8cc8?
      get() = roleState.valueOrNull()
      set(`value`) {
        val present = requireNotNull(value) { "role is not nullable; call unsetRole() to omit it" }
        roleState = FieldState.Value(present)
      }
    ...
    /**
     * Omits `inherited` from serialized output.
     */
    public fun unsetInherited() {
      inheritedState = FieldState.Absent
    }
    ...
    /**
     * Omits `role` from serialized output.
     */
    public fun unsetRole() {
      roleState = FieldState.Absent
    }
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): TeamMember = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<TeamMember> {
    override val descriptor: SerialDescriptor = JsonElement.serializer().descriptor

    override fun deserialize(decoder: Decoder): TeamMember {
      val jsonDecoder = decoder.requireJsonDecoder("TeamMember")
      val json = jsonDecoder.json
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("TeamMember must be a JSON object")
      ...
      if (!rawObject.containsKey("gravatar_id")) {
        throw SerializationException("TeamMember is missing required property 'gravatar_id'")
      }
      ...
      return TeamMember(
        ...
        inheritedState = json.decodeOptional(rawObject, "inherited", nullable = false),
        nameState = json.decodeOptional(rawObject, "name", nullable = true),
        roleState = json.decodeOptional(rawObject, "role", nullable = false),
    ...
    override fun serialize(encoder: Encoder, `value`: TeamMember) {
      val jsonEncoder = encoder.requireJsonEncoder("TeamMember")
      ...
      val raw = buildJsonObject {
        ...
        putState("inherited", value.inheritedState, json::encodeToJsonElement)
        putState("name", value.nameState, json::encodeToJsonElement)
        putState("role", value.roleState, json::encodeToJsonElement)
...
public fun teamMember(block: TeamMember.Builder.() -> Unit): TeamMember = TeamMember.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("TeamMember is missing required property '" + name + "'")
...
private inline fun <reified T> Json.decodeOptional(
  ...
  if (element == JsonNull) {
    if (!nullable) throw SerializationException("TeamMember property '" + name + "' is not nullable")
