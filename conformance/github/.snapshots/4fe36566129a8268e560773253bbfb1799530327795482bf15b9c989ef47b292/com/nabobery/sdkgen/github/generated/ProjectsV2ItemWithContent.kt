...
public class ProjectsV2ItemWithContent internal constructor(
  ...
  fieldsState: FieldState<List<JsonObject>>,
  ...
  private val fieldsState: FieldState<List<JsonObject>> =
      fieldsState.copyValue { fieldValue -> fieldValue.toList() }
  ...
  public class Builder {
    ...
    public var fields: List<JsonObject>?
      get() = fieldsState.valueOrNull()?.let { collection0 -> collection0.toList() }
      set(`value`) {
        val present = requireNotNull(value) { "fields is not nullable; call unsetFields() to omit it" }
        fieldsState = FieldState.Value(present.toList())
      }
  ...
  internal object Serializer : KSerializer<ProjectsV2ItemWithContent> {
