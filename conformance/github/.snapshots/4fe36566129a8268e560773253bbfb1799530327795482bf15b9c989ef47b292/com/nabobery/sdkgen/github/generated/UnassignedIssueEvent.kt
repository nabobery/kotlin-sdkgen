...
@Serializable
public data class UnassignedIssueEventView(
  public val actor: SimpleUser,
  public val assignee: SimpleUser,
  public val assigner: SimpleUser,
 ...
 *
 * Source: sdkgen://source/openapi.yaml#/components/schemas/unassigned-issue-event
 */
@Serializable(with = UnassignedIssueEvent.Serializer::class)
public class UnassignedIssueEvent(
  ...
  public class Builder {
    ...
    private var assigneeValue: SimpleUser? = null

    public var assignee: SimpleUser
      get() = requireNotNull(assigneeValue) { "assignee is required" }
      set(`value`) {
        assigneeValue = value
      }

    private var assignerValue: SimpleUser? = null

    public var assigner: SimpleUser
      get() = requireNotNull(assignerValue) { "assigner is required" }
      set(`value`) {
        assignerValue = value
      }
    ...
    public fun build(): UnassignedIssueEvent {
      check(actorValue != null) { "actor is required" }
      check(assigneeValue != null) { "assignee is required" }
      check(assignerValue != null) { "assigner is required" }
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): UnassignedIssueEvent = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<UnassignedIssueEvent> {
    override val descriptor: SerialDescriptor = JsonElement.serializer().descriptor

    override fun deserialize(decoder: Decoder): UnassignedIssueEvent {
      val jsonDecoder = decoder.requireJsonDecoder("UnassignedIssueEvent")
      val json = jsonDecoder.json
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("UnassignedIssueEvent must be a JSON object")
      val actor = json.decodeRequired<SimpleUser>(rawObject, "actor")
      val assignee = json.decodeRequired<SimpleUser>(rawObject, "assignee")
      val assigner = json.decodeRequired<SimpleUser>(rawObject, "assigner")
      ...
      if (!rawObject.containsKey("commit_id")) {
        throw SerializationException("UnassignedIssueEvent is missing required property 'commit_id'")
      }
      ...
      if (!rawObject.containsKey("commit_url")) {
        throw SerializationException("UnassignedIssueEvent is missing required property 'commit_url'")
      }
      ...
      if (!rawObject.containsKey("performed_via_github_app")) {
        throw SerializationException("UnassignedIssueEvent is missing required property 'performed_via_github_app'")
      }
    ...
    override fun serialize(encoder: Encoder, `value`: UnassignedIssueEvent) {
      val jsonEncoder = encoder.requireJsonEncoder("UnassignedIssueEvent")
      val json = jsonEncoder.json
      val raw = buildJsonObject {
        put("actor", json.encodeToJsonElement(value.actor))
        put("assignee", json.encodeToJsonElement(value.assignee))
        put("assigner", json.encodeToJsonElement(value.assigner))
...
public fun unassignedIssueEvent(block: UnassignedIssueEvent.Builder.() -> Unit): UnassignedIssueEvent = UnassignedIssueEvent.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("UnassignedIssueEvent is missing required property '" + name + "'")
...
private inline fun <reified T> Json.decodeOptional(
  ...
  if (element == JsonNull) {
    if (!nullable) throw SerializationException("UnassignedIssueEvent property '" + name + "' is not nullable")
