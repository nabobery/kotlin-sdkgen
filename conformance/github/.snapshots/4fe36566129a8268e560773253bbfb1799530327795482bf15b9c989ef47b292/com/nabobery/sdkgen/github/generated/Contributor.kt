 ...
 * Contributor
 *
 * Source: sdkgen://source/openapi.yaml#/components/schemas/contributor
 */
@Serializable(with = Contributor.Serializer::class)
public class Contributor(
  public val contributions: Int,
  ...
  public class Builder {
    private var contributionsValue: Int? = null

    public var contributions: Int
      get() = requireNotNull(contributionsValue) { "contributions is required" }
      set(`value`) {
        contributionsValue = value
      }
    ...
    public fun build(): Contributor {
      check(contributionsValue != null) { "contributions is required" }
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): Contributor = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<Contributor> {
    override val descriptor: SerialDescriptor = JsonElement.serializer().descriptor

    override fun deserialize(decoder: Decoder): Contributor {
      val jsonDecoder = decoder.requireJsonDecoder("Contributor")
      val json = jsonDecoder.json
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("Contributor must be a JSON object")
      val contributions = json.decodeRequired<Int>(rawObject, "contributions")
      ...
      return Contributor(
        ...
        gravatarId = rawObject["gravatar_id"]?.let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(element) },
    ...
    override fun serialize(encoder: Encoder, `value`: Contributor) {
      val jsonEncoder = encoder.requireJsonEncoder("Contributor")
      val json = jsonEncoder.json
      val raw = buildJsonObject {
        put("contributions", json.encodeToJsonElement(value.contributions))
        put("type", value.type)
...
public fun contributor(block: Contributor.Builder.() -> Unit): Contributor = Contributor.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("Contributor is missing required property '" + name + "'")
