...
@Serializable
public data class LockedIssueEventView(
 ...
 * Locked Issue Event
 *
 * Source: sdkgen://source/openapi.yaml#/components/schemas/locked-issue-event
 */
@Serializable(with = LockedIssueEvent.Serializer::class)
public class LockedIssueEvent(
  ...
  public class Builder {
    ...
    private var lockReasonState: FieldState<String?> = FieldState.Absent

    /**
     * Required nullable field; assigning `null` records present-null.
     */
    public var lockReason: String?
      get() = lockReasonState.valueOrNull()
      set(`value`) {
        lockReasonState = value.toNullableFieldState()
      }
    ...
    public fun build(): LockedIssueEvent {
      ...
      check(lockReasonState !== FieldState.Absent) { "lockReason is required, even when null" }
      ...
      return LockedIssueEvent(
        ...
        event = event,
        id = id,
        lockReason = lockReasonState.valueOrNull(),
        nodeId = nodeId,
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): LockedIssueEvent = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<LockedIssueEvent> {
    override val descriptor: SerialDescriptor = JsonElement.serializer().descriptor

    override fun deserialize(decoder: Decoder): LockedIssueEvent {
      val jsonDecoder = decoder.requireJsonDecoder("LockedIssueEvent")
      val json = jsonDecoder.json
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("LockedIssueEvent must be a JSON object")
      ...
      if (!rawObject.containsKey("commit_id")) {
        throw SerializationException("LockedIssueEvent is missing required property 'commit_id'")
      }
      ...
      if (!rawObject.containsKey("commit_url")) {
        throw SerializationException("LockedIssueEvent is missing required property 'commit_url'")
      }
      ...
      if (!rawObject.containsKey("lock_reason")) {
        throw SerializationException("LockedIssueEvent is missing required property 'lock_reason'")
      }
      val lockReason = rawObject["lock_reason"].let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(requireNotNull(element)) }
      if (!rawObject.containsKey("performed_via_github_app")) {
        throw SerializationException("LockedIssueEvent is missing required property 'performed_via_github_app'")
      }
    ...
    override fun serialize(encoder: Encoder, `value`: LockedIssueEvent) {
      val jsonEncoder = encoder.requireJsonEncoder("LockedIssueEvent")
      ...
      val raw = buildJsonObject {
        ...
        put("lock_reason", value.lockReason?.let { json.encodeToJsonElement(it) } ?: JsonNull)
...
public fun lockedIssueEvent(block: LockedIssueEvent.Builder.() -> Unit): LockedIssueEvent = LockedIssueEvent.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("LockedIssueEvent is missing required property '" + name + "'")
...
private inline fun <reified T> Json.decodeOptional(
  ...
  if (element == JsonNull) {
    if (!nullable) throw SerializationException("LockedIssueEvent property '" + name + "' is not nullable")
