...
/**
 * Generated model for sdkgen://source/openapi.yaml#/components/schemas/webhook-star-created.
 *
 * Source: sdkgen://source/openapi.yaml#/components/schemas/webhook-star-created
 */
@Serializable(with = WebhookStarCreated.Serializer::class)
public class WebhookStarCreated internal constructor(
  public val action: InlineWebhookStarCreatedActionXfae698b6,
  ...
  public constructor(
    action: InlineWebhookStarCreatedActionXfae698b6,
  ...
  public class Builder {
    private var actionValue: InlineWebhookStarCreatedActionXfae698b6? = null

    public var action: InlineWebhookStarCreatedActionXfae698b6
    ...
    public fun build(): WebhookStarCreated {
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): WebhookStarCreated = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<WebhookStarCreated> {
    override val descriptor: SerialDescriptor = JsonElement.serializer().descriptor

    override fun deserialize(decoder: Decoder): WebhookStarCreated {
      val jsonDecoder = decoder.requireJsonDecoder("WebhookStarCreated")
      val json = jsonDecoder.json
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("WebhookStarCreated must be a JSON object")
      val action = json.decodeRequired<InlineWebhookStarCreatedActionXfae698b6>(rawObject, "action")
      val repository = json.decodeRequired<RepositoryWebhooks>(rawObject, "repository")
      val sender = json.decodeRequired<SimpleUser>(rawObject, "sender")
      if (!rawObject.containsKey("starred_at")) {
        throw SerializationException("WebhookStarCreated is missing required property 'starred_at'")
      }
      val starredAt = rawObject["starred_at"].let { element -> if (element == JsonNull) null else json.decodeFromJsonElement<String?>(requireNotNull(element)) }
      return WebhookStarCreated(
        action = action,
        repository = repository,
        sender = sender,
    ...
    override fun serialize(encoder: Encoder, `value`: WebhookStarCreated) {
      val jsonEncoder = encoder.requireJsonEncoder("WebhookStarCreated")
...
public fun webhookStarCreated(block: WebhookStarCreated.Builder.() -> Unit): WebhookStarCreated = WebhookStarCreated.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("WebhookStarCreated is missing required property '" + name + "'")
...
private inline fun <reified T> Json.decodeOptional(
  ...
  if (element == JsonNull) {
    if (!nullable) throw SerializationException("WebhookStarCreated property '" + name + "' is not nullable")
