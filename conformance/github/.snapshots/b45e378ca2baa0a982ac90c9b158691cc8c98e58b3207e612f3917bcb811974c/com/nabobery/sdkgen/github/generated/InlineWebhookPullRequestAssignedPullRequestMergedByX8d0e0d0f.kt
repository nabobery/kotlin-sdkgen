 ...
 * ged_by
 */
@Serializable(with = InlineWebhookPullRequestAssignedPullRequestMergedByX8d0e0d0f.Serializer::class)
public class InlineWebhookPullRequestAssignedPullRequestMergedByX8d0e0d0f(
  public val id: Int,
  ...
  public val type: InlineWebhookPullRequestAssignedPullRequestMergedByTypeX9747c16c? = null,
  /**
   * Wire format: `uri`. Represented as `String` in this release; SDKGen does not validate this format.
   */
  ...
  public class Builder {
    ...
    public var type: InlineWebhookPullRequestAssignedPullRequestMergedByTypeX9747c16c? = null
    ...
    public fun build(): InlineWebhookPullRequestAssignedPullRequestMergedByX8d0e0d0f {
      check(idValue != null) { "id is required" }
      check(loginValue != null) { "login is required" }
      return InlineWebhookPullRequestAssignedPullRequestMergedByX8d0e0d0f(
        id = id,
        login = login,
        avatarUrl = avatarUrl,
        deleted = deleted,
  ...
  public companion object {
    public fun build(block: Builder.() -> Unit): InlineWebhookPullRequestAssignedPullRequestMergedByX8d0e0d0f = Builder().apply(block).build()
  }

  internal object Serializer : KSerializer<InlineWebhookPullRequestAssignedPullRequestMergedByX8d0e0d0f> {
    override val descriptor: SerialDescriptor = JsonElement.serializer().descriptor

    override fun deserialize(decoder: Decoder): InlineWebhookPullRequestAssignedPullRequestMergedByX8d0e0d0f {
      val jsonDecoder = decoder.requireJsonDecoder("InlineWebhookPullRequestAssignedPullRequestMergedByX8d0e0d0f")
      val json = jsonDecoder.json
      val rawObject = jsonDecoder.decodeJsonElement() as? JsonObject ?: throw SerializationException("InlineWebhookPullRequestAssignedPullRequestMergedByX8d0e0d0f must be a JSON object")
      ...
      return InlineWebhookPullRequestAssignedPullRequestMergedByX8d0e0d0f(
        ...
        type = rawObject["type"]?.let { json.decodeFromJsonElement<InlineWebhookPullRequestAssignedPullRequestMergedByTypeX9747c16c>(it) },
        url = rawObject["url"]?.let { json.decodeFromJsonElement<String>(it) },
        userViewType = rawObject["user_view_type"]?.let { json.decodeFromJsonElement<String>(it) },
      )
    }

    override fun serialize(encoder: Encoder, `value`: InlineWebhookPullRequestAssignedPullRequestMergedByX8d0e0d0f) {
      val jsonEncoder = encoder.requireJsonEncoder("InlineWebhookPullRequestAssignedPullRequestMergedByX8d0e0d0f")
...
public fun inlineWebhookPullRequestAssignedPullRequestMergedByX8d0e0d0f(block: InlineWebhookPullRequestAssignedPullRequestMergedByX8d0e0d0f.Builder.() -> Unit): InlineWebhookPullRequestAssignedPullRequestMergedByX8d0e0d0f = InlineWebhookPullRequestAssignedPullRequestMergedByX8d0e0d0f.build(block)

private inline fun <reified T> Json.decodeRequired(raw: JsonObject, name: String): T {
  val element = raw[name] ?: throw SerializationException("InlineWebhookPullRequestAssignedPullRequestMergedByX8d0e0d0f is missing required property '" + name + "'")
